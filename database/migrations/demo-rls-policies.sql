...
BEGIN
  ...
  DROP POLICY IF EXISTS "Allow public read for demo organizations" ON core_organizations;
  DROP POLICY IF EXISTS "Allow public read for demo organization entities" ON core_entities;
  DROP POLICY IF EXISTS "Allow public read for demo organization dynamic data" ON core_dynamic_data;
  DROP POLICY IF EXISTS "Allow public read for demo organization relationships" ON core_relationships;
  DROP POLICY IF EXISTS "Allow public read for demo organization transactions" ON universal_transactions;
  DROP POLICY IF EXISTS "Allow public read for demo organization transaction lines" ON universal_transaction_lines;

  -- Core Organizations - Allow public read for demo orgs
  CREATE POLICY "Allow public read for demo organizations"
    ON core_organizations
    FOR SELECT
    USING (id = ANY(demo_org_ids));

  -- Core Entities - Allow public read for demo org entities
  CREATE POLICY "Allow public read for demo organization entities"
    ON core_entities
    FOR SELECT
    USING (organization_id = ANY(demo_org_ids));

  -- Core Dynamic Data - Allow public read for demo org data
  CREATE POLICY "Allow public read for demo organization dynamic data"
    ON core_dynamic_data
    FOR SELECT
    USING (organization_id = ANY(demo_org_ids));

  -- Core Relationships - Allow public read for demo org relationships
  CREATE POLICY "Allow public read for demo organization relationships"
    ON core_relationships
    FOR SELECT
    USING (organization_id = ANY(demo_org_ids));

  -- Universal Transactions - Allow public read for demo org transactions
  CREATE POLICY "Allow public read for demo organization transactions"
    ON universal_transactions
    FOR SELECT
    USING (organization_id = ANY(demo_org_ids));

  -- Universal Transaction Lines - Allow public read for demo org transaction lines
  CREATE POLICY "Allow public read for demo organization transaction lines"
