-- Migration: Add flexible stock-on-hand function
-- Purpose: Generic function to calculate SOH for any entity type with smart code filtering

-- Drop existing functions if they exist
drop function if exists public.fn_entities_with_soh cascade;
drop function if exists public.fn_products_with_soh cascade;
drop function if exists public.smart_is_negative_txn_static cascade;
...
as $$
  select case
    ...
    when smart_code ilike '%.DISPOSAL.%' then true
    when smart_code ilike '%.WRITE_OFF.%' then true
    else false
  end;
$$;
...
soh as (
  select
    ...
    sum(
      case
        -- Use smart code for negative detection if available
        when ut.smart_code is not null and smart_is_negative_txn_static(ut.smart_code)
          then -abs(coalesce(utl.quantity,0))
        -- Fallback to transaction type
        when ut.transaction_type in ('sale', 'stock_adjustment_out', 'stock_transfer_out', 'damage', 'loss', 'disposal')
          then -abs(coalesce(utl.quantity,0))
        when ut.transaction_type in ('goods_receipt', 'stock_adjustment_in', 'stock_transfer_in', 'purchase_return', 'production_output')
          then abs(coalesce(utl.quantity,0))
...
as $$
  ...
  from public.fn_entities_with_soh(
    ...
    smart_prefixes     => array['HERA.SALON.PRODUCT.%', 'HERA.SPA.PRODUCT.%', 'HERA.RETAIL.PRODUCT.%'],
...
create index if not exists idx_utl_org_entity on universal_transaction_lines(organization_id, entity_id);
create index if not exists idx_ut_org_status on universal_transactions(organization_id, status);
create index if not exists idx_dynamic_org_entity on core_dynamic_data(organization_id, entity_id);

-- Grant execute permissions
grant execute on function public.fn_entities_with_soh to authenticated;
grant execute on function public.fn_products_with_soh to authenticated;
grant execute on function public.smart_is_negative_txn_static to authenticated;