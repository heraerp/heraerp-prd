-- Finance DNA v2 - Comprehensive Cascade Cleanup
-- Smart Code: HERA.ACCOUNTING.CLEANUP.CASCADE.COMPREHENSIVE.v2
-- Delete all non-compliant data with proper foreign key handling
...
DECLARE
    ...
    v_transactions_referencing INTEGER;
    v_lines_referencing INTEGER;
    v_relationships_referencing INTEGER;
    v_dynamic_referencing INTEGER;
BEGIN
    ...
    -- Count transactions that reference these entities
    SELECT COUNT(*) INTO v_transactions_referencing
    FROM universal_transactions ut
    WHERE ut.from_entity_id IN (
        SELECT id FROM core_entities 
        WHERE smart_code IS NOT NULL 
        AND NOT (smart_code ~ '^HERA\.[A-Z]+(\.[A-Z]+)*\.v[12]$')
        AND NOT (smart_code ~ '^HERA\.ACCOUNTING\.[A-Z]+(\.[A-Z]+)*\.v2$')
    )
    OR ut.to_entity_id IN (
        SELECT id FROM core_entities 
        WHERE smart_code IS NOT NULL 
        AND NOT (smart_code ~ '^HERA\.[A-Z]+(\.[A-Z]+)*\.v[12]$')
        AND NOT (smart_code ~ '^HERA\.ACCOUNTING\.[A-Z]+(\.[A-Z]+)*\.v2$')
    )
    OR ut.target_entity_id IN (
        SELECT id FROM core_entities 
        WHERE smart_code IS NOT NULL 
        AND NOT (smart_code ~ '^HERA\.[A-Z]+(\.[A-Z]+)*\.v[12]$')
        AND NOT (smart_code ~ '^HERA\.ACCOUNTING\.[A-Z]+(\.[A-Z]+)*\.v2$')
    ...
    RAISE NOTICE '  Non-compliant entities: %', v_entities_to_delete;
    RAISE NOTICE '  Transactions referencing these entities: %', v_transactions_referencing;
    RAISE NOTICE '';
    RAISE NOTICE 'Starting comprehensive cascade cleanup...';
END$$;

-- =============================================================================
-- STEP 3: DELETE ALL REFERENCES FIRST (COMPREHENSIVE CASCADE)
-- =============================================================================

-- Step 3a: Delete transaction lines that reference non-compliant entities
DELETE FROM universal_transaction_lines 
WHERE entity_id IN (
    SELECT entity_code FROM core_entities 
    WHERE smart_code IS NOT NULL 
    AND NOT (smart_code ~ '^HERA\.[A-Z]+(\.[A-Z]+)*\.v[12]$')
    AND NOT (smart_code ~ '^HERA\.ACCOUNTING\.[A-Z]+(\.[A-Z]+)*\.v2$')
)
...
-- Step 3b: Delete transaction lines with non-compliant smart codes
DELETE FROM universal_transaction_lines 
WHERE smart_code IS NOT NULL 
AND NOT (smart_code ~ '^HERA\.[A-Z]+(\.[A-Z]+)*\.v[12]$')
AND NOT (smart_code ~ '^HERA\.ACCOUNTING\.[A-Z]+(\.[A-Z]+)*\.v2$');

-- Step 3c: Delete transactions that reference non-compliant entities
...
-- Step 3d: Delete transactions with non-compliant smart codes
DELETE FROM universal_transactions 
WHERE smart_code IS NOT NULL 
AND NOT (smart_code ~ '^HERA\.[A-Z]+(\.[A-Z]+)*\.v[12]$')
AND NOT (smart_code ~ '^HERA\.ACCOUNTING\.[A-Z]+(\.[A-Z]+)*\.v2$');

-- Step 3e: Delete dynamic data that references non-compliant entities
...
-- Step 3f: Delete dynamic data with non-compliant smart codes
DELETE FROM core_dynamic_data 
WHERE smart_code IS NOT NULL 
AND NOT (smart_code ~ '^HERA\.[A-Z]+(\.[A-Z]+)*\.v[12]$')
AND NOT (smart_code ~ '^HERA\.ACCOUNTING\.[A-Z]+(\.[A-Z]+)*\.v2$');
...
-- =============================================================================
-- STEP 4: NOW SAFELY DELETE NON-COMPLIANT ENTITIES
-- =============================================================================

-- Finally delete entities with non-compliant smart codes
...
-- =============================================================================
-- STEP 5: VERIFY ALL DATA IS NOW COMPLIANT
-- =============================================================================
...
BEGIN
    ...
    RAISE NOTICE 'Comprehensive cascade cleanup successful - all remaining data is compliant';
END$$;

-- =============================================================================
-- STEP 6: UPDATE CONSTRAINTS SAFELY
-- =============================================================================
...
-- =============================================================================
-- STEP 7: CREATE CORRECTED GL BALANCE TRIGGER
-- =============================================================================
...
BEGIN
    ...
    IF v_balance_difference > 0.01 THEN
        ...
        BEGIN
            ...
            -- Only enforce balance for specific transaction types
...
-- =============================================================================
-- STEP 8: DROP CONFLICTING FUNCTIONS AND CREATE CLEAN VERSIONS
-- =============================================================================
...
INSERT INTO universal_transactions (
    ...
    'SYSTEM_UPDATE',
    'HERA.ACCOUNTING.CLEANUP.CASCADE.COMPREHENSIVE.v2',
    jsonb_build_object(
        'cleanup_type', 'comprehensive_cascade_cleanup',
        ...
        'trigger_recreated', 'validate_gl_balance_trigger',
        'helper_functions_created', ARRAY['create_test_gl_accounts_v2'],
        'cleanup_timestamp', NOW()
    )
);

SELECT 'Finance DNA v2 Comprehensive Cascade Cleanup Applied Successfully' as status;