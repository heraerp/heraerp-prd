-- Finance DNA v2 - Ultimate Cascade Cleanup
-- Smart Code: HERA.ACCOUNTING.CLEANUP.ULTIMATE.CASCADE.v2
-- Complete cleanup that handles ALL foreign key dependencies properly
...
-- =============================================================================
-- STEP 2: IDENTIFY AND CLEAN ORGANIZATIONS WITH NON-COMPLIANT DATA
-- =============================================================================
...
-- =============================================================================
-- STEP 3: CLEAN ANY REMAINING NON-COMPLIANT SMART CODES ACROSS ALL TABLES
-- =============================================================================

-- Delete transaction lines with non-compliant smart codes
DELETE FROM universal_transaction_lines 
WHERE smart_code IS NOT NULL 
AND NOT (smart_code ~ '^HERA\.[A-Z]+(\.[A-Z]+)*\.v[12]$')
AND NOT (smart_code ~ '^HERA\.ACCOUNTING\.[A-Z]+(\.[A-Z]+)*\.v2$');

-- Delete transactions with non-compliant smart codes
DELETE FROM universal_transactions 
WHERE smart_code IS NOT NULL 
AND NOT (smart_code ~ '^HERA\.[A-Z]+(\.[A-Z]+)*\.v[12]$')
AND NOT (smart_code ~ '^HERA\.ACCOUNTING\.[A-Z]+(\.[A-Z]+)*\.v2$');

-- Delete dynamic data with non-compliant smart codes
DELETE FROM core_dynamic_data 
WHERE smart_code IS NOT NULL 
AND NOT (smart_code ~ '^HERA\.[A-Z]+(\.[A-Z]+)*\.v[12]$')
AND NOT (smart_code ~ '^HERA\.ACCOUNTING\.[A-Z]+(\.[A-Z]+)*\.v2$');

-- Delete relationships that reference entities with non-compliant smart codes
...
-- Now delete any remaining transaction lines that reference non-compliant entities
DELETE FROM universal_transaction_lines 
WHERE entity_id IN (
    SELECT id FROM core_entities 
    WHERE smart_code IS NOT NULL 
...
BEGIN
    ...
    RAISE NOTICE 'Ultimate cleanup successful - all remaining data is compliant';
END$$;
...
INSERT INTO universal_transactions (
    ...
    'SYSTEM_UPDATE',
    'HERA.ACCOUNTING.CLEANUP.ULTIMATE.CASCADE.V2',
    jsonb_build_object(
        'cleanup_type', 'ultimate_cascade_cleanup',
        'strategy', 'proper_fk_dependency_handling',
...
SELECT 'Finance DNA v2 Ultimate Cascade Cleanup Applied Successfully' as status;