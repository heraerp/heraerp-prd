-- HERA User Identity Resolution RPC Functions - FIXED FIELD NAMES
-- =================================================================
-- Uses actual schema field names: from_entity_id/to_entity_id

-- Drop existing functions to recreate with correct field names
...
-- Compatible with verifyAuth expectations - FIXED VERSION
...
-- Function to resolve user roles in a specific organization - FIXED VERSION
...
COMMENT ON FUNCTION resolve_user_identity_v1() IS 'Resolve user identity and return organization IDs where user has membership - FIXED for legacy schema';
COMMENT ON FUNCTION resolve_user_roles_in_org(UUID) IS 'Get user roles within a specific organization - FIXED for legacy schema';