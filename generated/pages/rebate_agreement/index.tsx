...
interface Rebate AgreementData {
  id: string
  entity_name: string
  entity_code?: string
  created_at: string
...
export default function Rebate AgreementListPage() {
  const { organization, user } = useHERAAuth()
  const [entities, setEntities] = useState<Rebate AgreementData[]>([])
  ...
  const loadEntities = async () => {
    try {
      ...
      console.error('Failed to load rebate_agreements:', error)
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleCreate = () => {
    window.location.href = `/rebate_agreement/new`
  }

  const handleEdit = (entityId: string) => {
    window.location.href = `/rebate_agreement/${entityId}/edit`
  }

  const handleView = (entityId: string) => {
    window.location.href = `/rebate_agreement/${entityId}`
  }

  const handleDelete = async (entityId: string) => {
    if (!confirm('Are you sure you want to delete this rebate_agreement?')) return
    ...
    try {
      ...
      console.error('Failed to delete rebate_agreement:', error)
      alert('Failed to delete rebate_agreement')
    }
  }
  ...
  return (
    <ProtectedPage requiredSpace="rebate_agreement">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="p-2 bg-blue-100 rounded-lg">
                  <File-contract className="w-6 h-6 text-blue-600" />
                </div>
                <div>
                  <h1 className="text-2xl font-semibold text-gray-900">Rebate Agreements</h1>
                  <p className="text-gray-600">Supplier rebate agreement header</p>
                </div>
              </div>
        ...
        <div className="p-6">
              ...
              <div className="flex items-center gap-4">
                <div className="flex-1">
                  <div className="relative">
                    ...
                    <Input
                      placeholder="Search rebate_agreements..."
                      value={searchQuery}
          ...
          <Card>
            <CardHeader>
              <div className="flex items-center justify-between">
                <CardTitle>
                  {filteredEntities.length} Rebate Agreement{filteredEntities.length !== 1 ? 's' : ''}
                </CardTitle>
            ...
            <CardContent>
              {loading ? (
                <div className="text-center py-8">
                  ...
                  <p className="text-gray-600">Loading rebate_agreements...</p>
                </div>
              ) : filteredEntities.length === 0 ? (
                <div className="text-center py-8">
                  <File-contract className="w-12 h-12 text-gray-400 mx-auto mb-4" />
                  <h3 className="text-lg font-medium text-gray-900 mb-2">No rebate_agreements found</h3>
                  <p className="text-gray-600 mb-4">
                    {searchQuery 
                      ? `No rebate_agreements match your search criteria`
                      : `Get started by creating your first rebate_agreement`
                    }
                  </p>
                  {!searchQuery && (
                    <Button onClick={handleCreate}>
