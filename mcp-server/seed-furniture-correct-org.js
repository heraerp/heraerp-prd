...
// Correct furniture organization ID
...
async function seedFurnitureData() {
  console.log('🪑 Seeding furniture data for Kerala Furniture Works (Demo)...');
  ...
  try {
    ...
    if (createdCustomers.length > 0 && createdProducts.length > 0) {
      ...
      const order2 = {
        ...
        source_entity_id: createdCustomers[1]?.id || createdCustomers[0].id,
        total_amount: 185000,
        smart_code: 'HERA.FURNITURE.SALES.ORDER.v1',
        metadata: {
          customer_po: 'TM-PO-2025-5678',
      ...
      const { data: order2Data, error: order2Error } = await supabase
        .from('universal_transactions')
        .insert({
          ...order2,
          organization_id: FURNITURE_ORG_ID
      ...
      if (!order2Error) {
        console.log(`✅ Created sales order: ${order2.transaction_code}`);
        ...
        await supabase
          ...
          .insert({
            organization_id: FURNITURE_ORG_ID,
            transaction_id: order2Data.id,
            line_number: 1,
            line_type: 'product',
            entity_id: createdProducts[1]?.id || createdProducts[0].id,
            quantity: '10',
            unit_amount: 18500,
            line_amount: 185000,
            smart_code: 'HERA.FURNITURE.SALES.ORDER.LINE.v1'
    ...
    console.log('🎉 Furniture data seeding completed for Kerala Furniture Works (Demo)!');
