...
console.log('=== CREATING HAIR TALKZ APPOINTMENTS ===')
  ...
  try {
    ...
    for (let dayOffset = 0; dayOffset < 7; dayOffset++) {
      ...
      for (let i = 0; i < appointmentsPerDay; i++) {
        ...
        const appointment = {
          ...
          from_entity_id: randomCustomer.id, // Customer
          to_entity_id: randomStaff.id, // Staff member
          total_amount: randomService.metadata.base_price || 85.00,
          currency: 'AED',
          status: dayOffset === 0 ? 'completed' : (dayOffset <= 1 ? 'confirmed' : 'scheduled'),
          metadata: {
    ...
    for (const appointment of appointments) {
      ...
      if (apptError) {
        ...
        console.log(`✅ ${appointment.status}: ${appointment.metadata.customer_name} → ${appointment.metadata.service_name} (${date} ${time})`)
      }
    }
