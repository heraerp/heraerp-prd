#!/usr/bin/env node
/**
 * HERA APP MANAGEMENT RPC TEST SUITE
 * Tests all 11 app management RPC functions deployed to Supabase
 *
 * Prerequisites:
 ...
 *
 * Usage:
 *   node test-app-management-rpcs.mjs
 */
...
console.log('📋 Testing App Management RPC Functions\n')
...
async function resolveTestOrganization() {
  if (!TEST_ORG_ID) {
    ...
    if (error || !data) {
      ...
      console.error('   Please ensure at least one organization exists')
      process.exit(1)
    }
...
/**
 * Test 1: hera_apps_list_v1 - List all PLATFORM apps
 */
async function test_hera_apps_list_v1() {
  const result = await callRPC('hera_apps_list_v1', {
    p_actor_user_id: TEST_USER_ID,
    p_options: { limit: 10 }
  })

  printResult('hera_apps_list_v1 - List PLATFORM apps', result)
  return result
}

/**
 * Test 2: hera_apps_get_v1 - Get single app by code
 */
...
/**
 * Test 3: hera_org_apps_list_v1 - List apps installed in organization
 */
async function test_hera_org_apps_list_v1() {
  const result = await callRPC('hera_org_apps_list_v1', {
    p_actor_user_id: TEST_USER_ID,
    p_organization_id: TEST_ORG_ID,
    p_options: {}
  })

  printResult('hera_org_apps_list_v1 - List org installed apps', result)
  return result
}

/**
 * Test 4: hera_org_link_app_v1 - Install app to organization
 */
...
/**
 * Test 5: hera_org_set_default_app_v1 - Set default app for org
 */
...
/**
 * Test 6: hera_auth_introspect_v1 - Get user auth snapshot with apps
 */
...
/**
 * Test 7: hera_org_update_app_subscription_v1 - Update app subscription
 */
async function test_hera_org_update_app_subscription_v1() {
  const result = await callRPC('hera_org_update_app_subscription_v1', {
    p_actor_user_id: TEST_USER_ID,
    p_organization_id: TEST_ORG_ID,
    p_app_code: SALON_APP_CODE,
    p_subscription_data: {
  ...
  printResult(`hera_org_update_app_subscription_v1 - Update ${SALON_APP_CODE} subscription`, result)
  return result
}

/**
 * Test 8: hera_org_update_app_config_v1 - Update app configuration
 */
async function test_hera_org_update_app_config_v1() {
  const result = await callRPC('hera_org_update_app_config_v1', {
    p_actor_user_id: TEST_USER_ID,
    p_organization_id: TEST_ORG_ID,
    p_app_code: SALON_APP_CODE,
    p_config_data: {
  ...
  printResult(`hera_org_update_app_config_v1 - Update ${SALON_APP_CODE} config`, result)
  return result
}

/**
 * Test 9: hera_org_unlink_app_v1 - Soft uninstall app (is_active=false)
 */
...
/**
 * Test 10: Re-install app after soft uninstall (idempotency test)
 */
...
/**
 * Test 11: Error handling - Invalid app code
 */
...
/**
 * Test 12: Error handling - Lowercase app code (should fail validation)
 */
...
async function runAllTests() {
  ...
  console.log('HERA APP MANAGEMENT RPC TEST SUITE')
  console.log('=' .repeat(60))
  console.log()
  ...
  try {
    ...
    await test_hera_apps_list_v1()
    await test_hera_apps_get_v1()
    await test_hera_org_apps_list_v1()
    await test_hera_org_link_app_v1()
    await test_hera_org_set_default_app_v1()
    await test_hera_auth_introspect_v1()
    await test_hera_org_update_app_subscription_v1()
    await test_hera_org_update_app_config_v1()
    ...
    console.log('=' .repeat(60))
    console.log('\n📋 Summary:')
    console.log('   - Tested 11 RPC functions')
    ...
    console.log('   - Tested subscription and config updates')
