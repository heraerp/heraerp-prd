/**
 * Fix Michele's relationships CORRECTLY
 * Keep the older one pointing to ORG entity, deactivate the newer one pointing to ORGANIZATION entity
 */
...
async function fixMicheleRelationshipsCorrectly() {
  console.log('🔧 Fixing Michele relationships CORRECTLY...')
  ...
  try {
    ...
    console.log(`\\nFound ${relationships.length} MEMBER_OF relationships:`)
    ...
    if (correctRelationships.length === 0) {
      console.log(`\\n🆕 Creating correct MEMBER_OF relationship...`)
    ...
    if (correctActiveCount === 1) {
      console.log('🎯 Perfect! Michele has exactly 1 active relationship to the correct ORG entity')
    } else {
...
// Run the fix
fixMicheleRelationshipsCorrectly()
  .then(success => {
    if (success) {
      console.log('\\n🚀 Fix completed! Michele should authenticate correctly now.')
    } else {
