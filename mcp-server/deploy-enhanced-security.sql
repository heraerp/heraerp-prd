-- DEPLOY ENHANCED SECURITY VALIDATION
-- This will replace the existing enforce_actor_requirement function with enhanced validation
...
BEGIN
  ...
  RAISE DEBUG 'Enhanced actor validation passed: % (USER) in org % (ORGANIZATION) via %', 
              p_actor_user_id, p_organization_id, p_function_name;
...
'ENHANCED SECURITY FUNCTION v2.1: Validates actor membership with comprehensive error handling.
Enhanced Features:
- Blocks null UUIDs (00000000-0000-0000-0000-000000000000)
- Prevents platform organization business operations
- Validates entity types (USER, ORGANIZATION)
- Checks active membership relationships
- Provides detailed error messages for debugging
- Comprehensive cross-organization access prevention
...
SELECT 'TESTING ENHANCED SECURITY VALIDATION' as test_phase;
...
BEGIN
  BEGIN
    ...
    RAISE NOTICE '✅ Test 1 PASSED: Valid Michele actor accepted';
  EXCEPTION 
    WHEN OTHERS THEN
      RAISE NOTICE '❌ Test 1 FAILED: Valid Michele rejected: %', SQLERRM;
  END;
END;
$$;
...
BEGIN
  BEGIN
    ...
    RAISE NOTICE '❌ Test 2 FAILED: NULL UUID actor should be rejected';
  EXCEPTION 
    WHEN OTHERS THEN
      IF SQLERRM LIKE '%INVALID_ACTOR_NULL_UUID%' THEN
        RAISE NOTICE '✅ Test 2 PASSED: NULL UUID actor correctly rejected';
      ELSE
        RAISE NOTICE '❌ Test 2 FAILED: Wrong error for NULL UUID: %', SQLERRM;
      END IF;
  END;
END;
$$;

-- Test 3: Platform org (should fail)
DO $$
BEGIN
  BEGIN
    PERFORM enforce_actor_requirement(
    ...
    RAISE NOTICE '❌ Test 3 FAILED: Platform org access should be rejected';
  EXCEPTION 
    WHEN OTHERS THEN
      IF SQLERRM LIKE '%INVALID_ORGANIZATION_PLATFORM_UUID%' THEN
        RAISE NOTICE '✅ Test 3 PASSED: Platform org access correctly rejected';
      ELSE
        RAISE NOTICE '❌ Test 3 FAILED: Wrong error for platform org: %', SQLERRM;
      END IF;
  END;
END;
$$;

-- Test 4: Fake actor (should fail)
DO $$
BEGIN
  BEGIN
    PERFORM enforce_actor_requirement(
    ...
    RAISE NOTICE '❌ Test 4 FAILED: Fake actor should be rejected';
  EXCEPTION 
    WHEN OTHERS THEN
      IF SQLERRM LIKE '%ACTOR_ENTITY_NOT_FOUND%' OR SQLERRM LIKE '%ACTOR_NOT_MEMBER%' THEN
        RAISE NOTICE '✅ Test 4 PASSED: Fake actor correctly rejected';
      ELSE
        RAISE NOTICE '❌ Test 4 FAILED: Wrong error for fake actor: %', SQLERRM;
      END IF;
  END;
END;
$$;

SELECT 'ENHANCED SECURITY DEPLOYMENT COMPLETE' as deployment_status;