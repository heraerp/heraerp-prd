...
async function checkTransactionLines() {
  ...
  try {
    // Step 1: Get the most recent SALE transaction
    console.log('📊 Step 1: Finding most recent SALE transaction...')
    const { data: recentTxn, error: txnError } = await supabase
      .from('universal_transactions')
      .select('id, transaction_code, transaction_date, total_amount, source_entity_id, target_entity_id')
    ...
    console.log('\n✅ Found recent transaction:')
    console.log(JSON.stringify(recentTxn, null, 2))
    ...
    const { data: lines, error: linesError } = await supabase
      ...
      .select('*')
      .eq('transaction_id', recentTxn.id)
