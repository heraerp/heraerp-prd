...
export class ContractFirstEnforcer {
     ...
     */
    async createContractFirstPlan(plan) {
        const errors = [];
        try {
     ...
     */
    async createMissingContracts(planId) {
     ...
     */
    async createFailingTests(planId) {
        const plan = this.loadPlan(planId);
        if (!plan) {
            return { success: false, errors: ['Plan not found'] };
        }
     ...
     */
    async validateImplementationReadiness(planId) {
        const plan = this.loadPlan(planId);
        if (!plan) {
     ...
     */
    async blockImplementationIfNotReady(planId) {
    ...
    async generateContractFile(contract) {
...
`;
    ...
    async generateTestFile(test, plan) {
        const testTemplate = `/**
...
export async function runContractFirstEnforcer() {
