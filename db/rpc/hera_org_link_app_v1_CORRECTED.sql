-- ===================================================================
-- RPC: hera_org_link_app_v1 (CORRECTED - 3 Syntax Bugs Fixed)
-- Purpose: Link a tenant organization to a PLATFORM APP (ORG_HAS_APP)
-- Status: ✅ READY FOR DEPLOYMENT
-- Version: 1.1 (Syntax bugs fixed)
--
-- BUGS FIXED:
-- 1. Line 41: Added missing closing quote and $ anchor to regex
-- 2. Line 70: Added missing closing quote and $ anchor to regex
-- 3. Line 78: Fixed REGEXP_REPLACE syntax (missing quote, wrong backslash count, missing anchor)
-- ===================================================================
...
BEGIN
  ...
  -- 🔧 BUG FIX #1: Added missing closing quote and $ anchor
  ...
  -- 🔧 BUG FIX #2: Added missing closing quote and $ anchor
  ...
  -- 🔧 BUG FIX #3: Fixed REGEXP_REPLACE syntax
  -- - Added missing closing quote after pattern
  -- - Changed \\1 to \1 (single backslash for capture group)
  -- - Added missing $ anchor at end of pattern
  -- Segment-5 must match p_app_code
...
COMMENT ON FUNCTION public.hera_org_link_app_v1(uuid, uuid, text, timestamptz, jsonb, jsonb, boolean) IS
  'Link/install a PLATFORM APP to tenant organization. Creates ORG_HAS_APP relationship with subscription and config.
   Idempotent: re-linking updates existing relationship. v1.1 - Syntax bugs fixed (regex quotes and anchors).';
...
SELECT hera_org_link_app_v1(
  '09b0b92a-d797-489e-bc03-5ca0a6272674'::uuid,  -- actor
  '378f24fb-d496-4ff7-8afa-ea34895a0eb8'::uuid,  -- org (Hairtalkz)
  'SALON',                                        -- app code
  now(),
  '{"plan":"premium","status":"active"}'::jsonb,
  '{"enable_appointments":true}'::jsonb,
  true
);
*/
