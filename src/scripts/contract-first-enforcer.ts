...
const ContractFirstPlanSchema = z.object({
  feature_name: z.string().min(1, 'Feature name is required'),
  smart_code: z.string().regex(/^HERA\.[A-Z]+\.[A-Z]+\.[A-Z]+\.[A-Z]+\.v\d+$/, 'Must be valid HERA smart code'),
  description: z.string().min(10, 'Description must be at least 10 characters'),
  contracts_required: z.array(z.object({
  ...
  tests_required: z.array(z.object({
    test_path: z.string(),
    test_name: z.string(),
...
export class ContractFirstEnforcer {
   ...
   */
  async createContractFirstPlan(plan: ContractFirstPlan): Promise<{ success: boolean; errors: string[]; planId: string }> {
    const errors: string[] = [];
    ...
    try {
      ...
      for (const contract of validatedPlan.contracts_required) {
        ...
        if (!contractExists && !contract.exists) {
          errors.push(`Contract ${contract.contract_name} does not exist at ${contract.contract_path}`);
        }
      }
    ...
    for (const contract of plan.contracts_required) {
      if (!contract.exists || contract.needs_update) {
        try {
          ...
          errors.push(`Failed to create contract ${contract.contract_name}: ${error instanceof Error ? error.message : String(error)}`);
        }
      }
    }
  ...
  async createFailingTests(planId: string): Promise<{ success: boolean; errors: string[] }> {
    ...
    for (const test of plan.tests_required) {
      if (!test.exists) {
        try {
          ...
          errors.push(`Failed to create test ${test.test_name}: ${error instanceof Error ? error.message : String(error)}`);
        }
      }
    }
    ...
    try {
      ...
      errors.push('Tests should be failing but all tests are passing. This violates TDD principles.');
    } catch {
   ...
   */
  async blockImplementationIfNotReady(planId: string): Promise<{ blocked: boolean; reason: string }> {
  ...
  private loadPlan(planId: string): ContractFirstPlan & { planId: string; createdAt: string; status: string } | null {
    const planPath = path.join(this.plansDir, `${planId}.json`);
  ...
  private async generateContractFile(contract: { contract_path: string; contract_name: string; purpose: string }): Promise<void> {
...
`;
  ...
  private async generateTestFile(
    test: { test_path: string; test_name: string; test_type: string; should_fail_initially: boolean }, 
...
export async function runContractFirstEnforcer(): Promise<void> {
  ...
  console.log('This tool ensures TDD compliance and prevents implementation without contracts and failing tests.');
  console.log('');
