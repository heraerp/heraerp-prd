...
type ClaudeHandshake = z.infer<typeof ClaudeHandshakeSchema>

export class ClaudeContractHandshakeSystem {
  private readonly enforcer: ContractFirstEnforcer
  private readonly handshakesDir = './handshakes'
   ...
   */
  async validateClaudeHandshake(handshake: ClaudeHandshake): Promise<{
    success: boolean
    errors: string[]
    warnings: string[]
    handshakeId: string
    ...
    try {
      ...
      if (uncheckedItems.length > 0) {
        errors.push(
          `DOOD checklist incomplete. Missing: ${uncheckedItems.map(([key]) => key).join(', ')}`
        )
      }
      ...
      const nonContractImports = validatedHandshake.plan.implementation_files.some(file =>
        file.depends_on_contracts.some(dep => !contractPaths.includes(dep))
      )

      if (nonContractImports) {
        warnings.push(
          'Some implementation files depend on non-contract imports - ensure all types come from @/contracts'
        )
      }
   ...
   */
  async createContractFirstPlanFromHandshake(handshakeId: string): Promise<{
    success: boolean
    errors: string[]
    planId: string
  }> {
    ...
    const plan = {
      ...
      contracts_required: handshake.plan.contracts_to_create.map(
        (contract: { contract_path: string; contract_name: string; purpose: string }) => ({
      ...
      tests_required: handshake.plan.tests_to_create.map(
        (test: {
          test_path: string
          test_name: string
          test_type: string
   ...
   */
  async executeContractFirstWorkflow(handshakeId: string): Promise<{
    success: boolean
    errors: string[]
    status: string
    nextActions: string[]
    ...
    try {
      ...
      const validationResult = await this.enforcer.validateImplementationReadiness(
        planResult.planId
      )
   ...
   */
  createHandshakeTemplate(userRequest: string): ClaudeHandshake {
    return {
      ...
      commitment: {
        ...
        claude_signature:
          '[CLAUDE: I commit to following this contract-first development process and acknowledge that implementation will be blocked if requirements are not met. Signed: Claude]'
      }
    }
  }

  private loadHandshake(
    handshakeId: string
  ): (ClaudeHandshake & { handshakeId: string; createdAt: string; status: string }) | null {
    const handshakePath = path.join(this.handshakesDir, `${handshakeId}.json`)
    if (!fs.existsSync(handshakePath)) {
      return null
    }

    return JSON.parse(fs.readFileSync(handshakePath, 'utf-8')) as ClaudeHandshake & {
      handshakeId: string
      createdAt: string
      status: string
    }
