...
export function computeRecommendation(input: {
  ...
  score += input.integrations === "0" ? 0 : input.integrations === "1-2" ? 2 : input.integrations === "3-5" ? 4 : 6;
...
export const PACKAGES: Array<{ name: RecommendedPackage; summary: string; bullets: string[]; ctaHint: string }> = [
  {
    name: "Essential",
