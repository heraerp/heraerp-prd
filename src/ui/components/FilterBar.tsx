    ...
    <div className="mb-3 grid grid-cols-1 gap-2 sm:grid-cols-4">
      ...
      {fields.includes('date_from') && (
        <input
          ...
          onChange={(e) => update('date_from', e.target.value ? new Date(e.target.value).toISOString() : undefined)}
        />
      )}
      {fields.includes('date_to') && (
        <input
          ...
          onChange={(e) => update('date_to', e.target.value ? new Date(e.target.value).toISOString() : undefined)}
        />
      )}
    </div>
  );
