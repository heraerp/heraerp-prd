...
export function LinesEditor({
  ...
  const update = (idx: number, patch: Partial<TxnLine>) => {
    const next = value.map((l, i) =>
      i === idx ? { ...l, ...patch, line_amount: computeAmount({ ...l, ...patch }) } : l
    )
    onChange(next)
  }
      ...
      {value.map((l, i) => (
        <div key={i} className="grid grid-cols-12 items-end gap-2 rounded-xl border p-3">
          ...
          <div className="col-span-2">
            ...
            <div className="mt-1 w-full rounded-lg border bg-gray-50 px-3 py-2">
          ...
          <div className="col-span-2">
            ...
            <input
              ...
              onChange={e =>
                update(i, { tax_amount: parseFloat(e.target.value || '0') || undefined })
              }
            />
          </div>
