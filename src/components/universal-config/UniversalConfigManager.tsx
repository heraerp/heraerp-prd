...
interface UniversalConfigManagerProps {
  ...
  additionalFields?: {
    name: string
    label: string
    type: 'text' | 'number' | 'boolean' | 'checkbox' | 'textarea' | 'currency' | 'select' | 'multiselect' | 'date' | 'color'
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const endpoint = selectedItem 
        ? `${apiEndpoint}?id=${selectedItem.id}`
        : apiEndpoint
          ...
          <Table>
            ...
            <TableBody>
              {loading ? (
                ...
                <TableRow>
                  <TableCell colSpan={4 + customColumns.length} className="text-center py-8 text-muted-foreground">
              ...
              <div className="space-y-6 pb-4">
                ...
                {additionalFields.map((field, index) => {
                  ...
                  const shouldShowSectionHeader = (
                    (index === 0) ||
                    (field.name === 'service_commission_rate' && additionalFields[index-1]?.name !== 'service_commission_rate') ||
                    (field.name === 'can_accept_tips' && additionalFields[index-1]?.name !== 'can_accept_tips') ||
                    (field.name === 'permissions' && additionalFields[index-1]?.name !== 'permissions')
                  )
                  
                  const sectionTitle = 
                    field.name.includes('commission') ? 'Commission Settings' :
                    field.name === 'can_accept_tips' || field.name === 'requires_license' ? 'Professional Settings' :
                    field.name === 'permissions' ? 'Permissions & Access' :
                    index === 0 ? 'Basic Information' : null
                          ...
                          <div className="space-y-3">
                            {(() => {
                              ...
                              field.options?.forEach(option => {
                                const category = option.label.includes('📅') ? 'Appointments' :
                                              option.label.includes('👤') ? 'Client Management' :
                                              option.label.includes('✂️') || option.label.includes('🛍️') ? 'Services & Products' :
                                              option.label.includes('💳') || option.label.includes('💸') || option.label.includes('💰') ? 'Financial' :
                                              option.label.includes('📦') ? 'Inventory' :
                                              option.label.includes('📊') ? 'Reports' :
                                              option.label.includes('⚙️') ? 'Administration' : 'Other'
                              ...
                              return Object.entries(groups).map(([category, options]) => (
                                <div key={category} className="space-y-2">
                                  ...
                                  <div className="space-y-1 pl-2">
                                    {options.map(option => (
                                      <label key={option.value} className="flex items-center space-x-2 cursor-pointer hover:bg-muted/50 p-1.5 rounded-md transition-colors">
                                        <Checkbox
      ...
      <Dialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Are you sure you want to delete "{selectedItem?.entity_name}"? This action cannot be undone.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button
