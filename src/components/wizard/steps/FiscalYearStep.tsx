...
export const FiscalYearStep: React.FC<FiscalYearStepProps> = ({
  ...
  const systemAccounts = coaAccounts.filter(acc => 
    acc.account_type === 'EQUITY' || 
    (acc.account_type === 'ASSET' && acc.account_subtype === 'CURRENT_ASSET')
  )

  const retainedEarningsAccounts = coaAccounts.filter(acc => 
    acc.account_type === 'EQUITY' && 
  ...
  const currentEarningsAccounts = coaAccounts.filter(acc => 
    acc.account_type === 'EQUITY' && 
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="space-y-2">
          ...
          >
            ...
            <SelectContent className="hera-select-content">
              {MONTHS.map((month) => (
                <SelectItem key={month.value} value={month.value.toString()} className="hera-select-item">
                  {month.name}
                </SelectItem>
              ))}
            </SelectContent>
        ...
        <div className="space-y-2">
          ...
          >
            ...
            <SelectContent className="hera-select-content">
              <SelectItem value="12" className="hera-select-item">12 Periods (Monthly)</SelectItem>
              <SelectItem value="13" className="hera-select-item">13 Periods (4-4-5 Calendar)</SelectItem>
            </SelectContent>
          </Select>
        </div>
        ...
        <div className="space-y-2">
          ...
          />
          <p className="text-xs text-muted-foreground">
            Additional adjustment periods (0-4)
          </p>
        </div>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Select GL accounts for automatic year-end closing
          </CardDescription>
        </CardHeader>
        <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="space-y-2">
            ...
            >
              ...
              <SelectContent className="hera-select-content">
                {retainedEarningsAccounts.map((account) => (
                  <SelectItem key={account.entity_code} value={account.entity_code} className="hera-select-item">
            ...
            <p className="text-xs text-muted-foreground">
              Prior year P&L closes to this account
            </p>
          </div>
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-muted-foreground">
              Current year P&L accumulates here
            </p>
          </div>
        </CardContent>
      </Card>
      ...
      {fiscalPeriods.length > 0 && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Generated fiscal periods based on your configuration
            </CardDescription>
          </CardHeader>
          <CardContent>
      ...
      <div className="mt-4 p-3 border rounded bg-blue-50 dark:bg-blue-950/30">
        ...
        <div className="text-xs font-mono space-y-1">
          <div>Fiscal Config: HERA.{data.organizationBasics.industry_classification}.UCR.CONFIG.FISCAL_YEAR.v1</div>
          <div>Fiscal Periods: HERA.{data.organizationBasics.industry_classification}.UCR.CONFIG.FISCAL_PERIODS.v1</div>
          <div>Year-End Close: HERA.{data.organizationBasics.industry_classification}.UCR.CONFIG.YEAR_END_CLOSE.v1</div>
        </div>
      </div>
    </div>
  )
