...
const DEFAULT_SEQUENCES = [
  ...
  { document_type: 'AR_INVOICE', prefix: 'AR-{YYYY}-', suffix: '', next_number: 1, min_length: 6, reset_frequency: 'YEARLY' },
  { document_type: 'AP_INVOICE', prefix: 'AP-{YYYY}-', suffix: '', next_number: 1, min_length: 6, reset_frequency: 'YEARLY' },
        ...
        {data.documentNumbering.sequences.map((seq, index) => (
          <Card key={seq.document_type}>
            <CardHeader className="pb-3">
              <CardTitle className="flex items-center space-x-2 text-base">
                ...
                <Badge variant="outline" className="ml-auto">
                  {seq.prefix}{seq.next_number.toString().padStart(seq.min_length, '0')}{seq.suffix}
                </Badge>
              </CardTitle>
            </CardHeader>
            <CardContent className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <div>
                <Label>Reset</Label>
                <Select value={seq.reset_frequency} onValueChange={(value) => updateSequence(index, 'reset_frequency', value)}>
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent className="hera-select-content">
                    <SelectItem value="NEVER" className="hera-select-item">Never</SelectItem>
                    <SelectItem value="YEARLY" className="hera-select-item">Yearly</SelectItem>
                    <SelectItem value="MONTHLY" className="hera-select-item">Monthly</SelectItem>
                  </SelectContent>
                </Select>
              </div>
            </CardContent>
