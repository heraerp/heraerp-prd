  ...
  return (
    <div className="space-y-2">
      ...
      <div className="relative">
        ...
        <input
          className={`w-full ${icon ? 'pl-10' : 'pl-4'} pr-4 py-3 bg-background/5 border border-border/10 
                     rounded-xl text-foreground placeholder-white/40 
...
export function ISPSelect({ label, error, options, className = '', ...props }: ISPSelectProps) {
  return (
    <div className="space-y-2">
      <label className="text-sm font-medium text-foreground/80">{label}</label>
      <select
        className={`w-full px-4 py-3 bg-background/5 border border-border/10 rounded-xl 
...
export function ISPTextarea({ label, error, className = '', ...props }: ISPTextareaProps) {
  return (
    <div className="space-y-2">
      ...
      <textarea
        className={`w-full px-4 py-3 bg-background/5 border border-border/10 rounded-xl 
                   text-foreground placeholder-white/40 resize-none
                   focus:outline-none focus:ring-2 focus:ring-[#00DDFF]/50 focus:border-[#00DDFF]/50 
                   transition-all duration-300 ${error ? 'border-[#ff1d58]/50' : ''} ${className}`}
        rows={4}
        {...props}
