  ...
  ({ label, icon, error, className = '', ...props }, ref) => {
    return (
      <div className="space-y-1">
        {label && (
          <label className="text-sm font-medium text-[#F5E6C8]">
            {label}
          </label>
        )}
...
export const ISPButton = forwardRef<HTMLButtonElement, ISPButtonProps>(
  ({ variant = 'primary', size = 'md', loading, className = '', children, disabled, ...props }, ref) => {
    const variants = {
      primary: 'bg-gradient-to-r from-[#0099CC] to-[#0049B7] text-white hover:shadow-lg hover:shadow-[#0099CC]/40',
      secondary: 'bg-slate-800/50 border border-slate-700/50 text-slate-300 hover:bg-slate-700/50 hover:text-white',
      danger: 'bg-gradient-to-r from-red-600 to-red-700 text-white hover:shadow-lg hover:shadow-red-600/40'
    }
