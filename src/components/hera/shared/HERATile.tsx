 ...
 * HERA TILE COMPONENT
 *
 * Enterprise-grade tile/card component with HERA platform styling.
 * Features glassmorphism, indigo borders, and mouse-tracking animations.
 *
 * Features:
 * - Glassmorphism with 8px backdrop blur
 * - Indigo-purple gradient borders with hover effects
 ...
 * @example
 * // Grid tile
 * <HERATile mode="grid" className="p-6">
 *   <h3>Dashboard Card</h3>
 *   <p>Content here</p>
 * </HERATile>
 *
 * // List tile
 * <HERATile mode="list" className="flex items-center p-4">
 *   <div>Content</div>
 * </HERATile>
 */
export function HERATile({
  mode = 'grid',
  ...
  const defaultBackground =
    'linear-gradient(135deg, rgba(99, 102, 241, 0.08) 0%, rgba(147, 51, 234, 0.05) 50%, rgba(168, 85, 247, 0.03) 100%)'
  ...
  const handleMouseEnter = (e: MouseEvent<HTMLDivElement>) => {
    ...
    if (enableHoverEffects && tileRef.current) {
      ...
      tileRef.current.style.boxShadow =
        '0 20px 40px rgba(99, 102, 241, 0.25), inset 0 1px 0 rgba(255, 255, 255, 0.15)'
      tileRef.current.style.borderColor = `${borderColor || 'rgba(99, 102, 241, 0.60)'}`
    }
  ...
  const handleMouseLeave = (e: MouseEvent<HTMLDivElement>) => {
    ...
    if (tileRef.current) {
      ...
      tileRef.current.style.borderColor = borderColor || 'rgba(99, 102, 241, 0.25)'
