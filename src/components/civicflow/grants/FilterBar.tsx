      ...
      {hasActiveFilters && (
        <div className="flex flex-wrap gap-2">
          ...
          {(filters.amount_min !== undefined || filters.amount_max !== undefined) && (
            <Badge variant="secondary" className="text-xs">
              Amount: {filters.amount_min ? new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(filters.amount_min) : '0'} - {filters.amount_max ? new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(filters.amount_max) : '∞'}
              <X
                className="h-3 w-3 ml-1 cursor-pointer"
                onClick={() => {
