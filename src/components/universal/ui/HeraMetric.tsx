    ...
    >
      <div className="space-y-3">
        ...
        <div className="flex items-center justify-between">
          ...
          {icon && <div className="flex-shrink-0 text-hera-400">{icon}</div>}
        </div>
        ...
        {/* Change Indicator */}
        {change && (
          <div className={cn('flex items-center gap-2 text-sm font-medium', getTrendClasses())}>
            {getTrendIcon()}
            <span>
              {change.percentage
              ...
              {change.period && <span className="text-muted-foreground ml-1">{change.period}</span>}
            </span>
          </div>
        )}
      </div>
...
export function HeraCompactMetric({ className, ...props }: Omit<HeraMetricProps, 'variant'>) {
  return <HeraMetric variant="compact" className={cn('p-4', className)} {...props} />
}
