...
export function HeraDnaUniversalCalendar({
  ...
  if (!orgId) {
    return (
      <Alert>
        ...
        <AlertDescription>Organization context required for calendar access.</AlertDescription>
      </Alert>
    )
  }
        ...
        <div className="fc-event-main">
          <div className="fc-event-title-container">
            <div className="fc-event-title fc-sticky">
              {extProps.privacy && <Shield className="inline w-3 h-3 mr-1 text-amber-500" />}
              {displayTitle}
            </div>
          </div>
          {extProps.locationId && (
  ...
  const determineEventColor = useCallback(
    (event: HeraDnaCalendarEvent) => {
      ...
      switch (industryType) {
        ...
        case 'restaurant':
          if (event.extendedProps.smartCode?.includes('RESERVATION'))
            return industryColors.reservation
    ...
    async (start: Date, end: Date) => {
      ...
      try {
        const response = await fetch(
          `/api/v1/calendar/events?start=${start.toISOString()}&end=${end.toISOString()}&org=${orgId}`
        )
