...
import { useUniversalEndpointData } from '@/lib/dna/patterns/universal-api-endpoint-pattern'
...
export default function NewProductionOrderModal({
  ...
  // Load data using API endpoint
  const { data: responseData } = useUniversalEndpointData({
    endpoint: '/api/furniture/production',
    organizationId,
    enabled: open && !!organizationId
  })

  const products = responseData?.entities?.filter((e: any) => e.entity_type === 'product') || []
  const workCenters =
    responseData?.entities?.filter((e: any) => e.entity_type === 'work_center') || []
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const orderData = {
        ...
        source_entity_id: formData.productId, // Product to produce
        target_entity_id: formData.workCenterId, // Work center
        total_amount: formData.plannedQuantity,
        description: `Production order for ${products.find(p => p.id === formData.productId)?.entity_name}`,
        ...
        line_items: [
          {
            ...
            unit_price: 0, // Internal production
            line_amount: 0,
            description: `Output: ${products.find(p => p.id === formData.productId)?.entity_name}`,
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              >
                ...
                <SelectContent className="bg-muted border-border z-[9999]">
                  {products.map((product: any) => (
                    <SelectItem
                      key={product.id}
                      value={product.id}
                      className="text-gray-200 hover:bg-muted-foreground/10 focus:bg-muted-foreground/10 focus:text-foreground cursor-pointer"
                    >
            ...
            <div className="space-y-2">
              <Label htmlFor="workcenter" className="text-gray-300">
                Work Center
              </Label>
              <Select
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={e =>
                  setFormData({ ...formData, plannedQuantity: parseInt(e.target.value) || 1 })
                }
          ...
          <div className="space-y-2">
            ...
            >
              ...
              <SelectContent className="bg-muted border-border z-[9999]">
                ...
                >
                  <div className="flex items-center gap-2">
                    <Badge className="bg-muted-foreground/20 text-muted-foreground border-0">
                      Low
                    </Badge>
                  </div>
                </SelectItem>
                ...
                >
                  <div className="flex items-center gap-2">
                    <Badge className="bg-blue-600/20 text-blue-400 border-0">Medium</Badge>
                  </div>
                </SelectItem>
                <SelectItem
                  value="high"
                ...
                >
                  <div className="flex items-center gap-2">
                    <Badge className="bg-amber-600/20 text-amber-400 border-0">High</Badge>
                  </div>
                </SelectItem>
                <SelectItem
                  value="urgent"
                ...
                >
                  <div className="flex items-center gap-2">
                    <Badge className="bg-red-600/20 text-red-400 border-0">Urgent</Badge>
                  </div>
                </SelectItem>
              </SelectContent>
            </Select>
          ...
          {formData.productId && (
            <div className="p-4 bg-card/50 rounded-lg border border-border">
              <h4 className="text-sm font-medium text-gray-300 mb-2">Production Summary</h4>
              <div className="space-y-1 text-sm">
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Product:</span>
                  <span className="text-foreground font-medium">
                    {products.find(p => p.id === formData.productId)?.entity_name}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Quantity:</span>
                  <span className="text-foreground font-medium">
                    {formData.plannedQuantity} units
                  </span>
                </div>
                <div className="flex justify-between">
                  ...
                  <span className="text-foreground font-medium">
                    {Math.ceil(
                      (new Date(formData.plannedEndDate).getTime() -
                        new Date(formData.plannedStartDate).getTime()) /
                        (1000 * 60 * 60 * 24)
                    )}{' '}
                    days
                  </span>
          ...
          <div className="flex justify-end gap-3 pt-4 border-t border-border">
            <Button
              ...
              variant="outline"
              onClick={onClose}
              className="bg-transparent border-border text-gray-300 hover:bg-muted hover:text-foreground"
            >
              Cancel
            </Button>
            <Button
              type="submit"
              disabled={loading}
              className="bg-gradient-to-r from-amber-600 to-orange-600 hover:from-amber-700 hover:to-orange-700 text-foreground border-0"
            >
              {loading ? (
                <>
