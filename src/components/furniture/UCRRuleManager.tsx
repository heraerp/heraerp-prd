...
import { 
  Settings, Shield, Calculator, DollarSign, CheckCircle, Calendar, 
  AlertTriangle, Code, Play, Plus, Edit
} from 'lucide-react'
...
export function UCRRuleManager({ organizationId, className }: UCRRuleManagerProps) {
  ...
  return field.field_value_json } }
    return field.field_value_number || field.field_value_text;
        ...
        return { id: rule.id, name: rule.entity_name, type: (rule.metadata as any)?.rule_type || 'unknown', smartCode: rule.smart_code, status: rule.status, priority: parseField('rule_priority') || 100, scope: parseField('rule_scope'), condition: parseField('rule_condition'), action: parseField('rule_action'), parameters: parseField('rule_parameters')
        };
      });
