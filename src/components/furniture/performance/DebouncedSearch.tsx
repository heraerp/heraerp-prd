...
const value = controlledValue !== undefined ? controlledValue : internalValue // Debounced search function const performSearch = useCallback( async (query: string) => { if (query.length < minLength) {
  setIsSearching(false) return } setIsSearching(true) try { await onSearch(query  ) finally { setIsSearching(false  ) }, [onSearch, minLength] ) // Handle input change const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => { const newValue = e.target.value if (controlledValue === undefined) {
  ...
  clearTimeout(searchTimeoutRef.current  ) // Set new timeout for debounced search searchTimeoutRef.current = setTimeout(() => { performSearch(newValue  ), delay  ) // Handle clear button const handleClear = () => { if (controlledValue === undefined) {
  setInternalValue(''  ) onChange?.('') onSearch('') inputRef.current?.focus() setShowSuggestions(false  ) // Handle suggestion click const handleSuggestionClick = (suggestion: string) => { if (controlledValue === undefined) {
  setInternalValue(suggestion  ) onChange?.(suggestion) onSuggestionClick?.(suggestion) performSearch(suggestion) setShowSuggestions(false) inputRef.current?.focus(  ) // Handle keyboard navigation const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => { if (!showSuggestions || suggestions.length === 0) return switch (e.key) {
  ...
  handleSuggestionClick(suggestions[focusedSuggestion]  ) else { performSearch(value) setShowSuggestions(false  ) break case 'Escape': setShowSuggestions(false) setFocusedSuggestion(-1) break } } // Handle click outside useEffect(() => { const handleClickOutside = (e: MouseEvent) => { if ( inputRef.current && !inputRef.current.contains(e.target as Node) && suggestionsRef.current && !suggestionsRef.current.contains(e.target as Node) ) {
