...
import { Plus, Trash2, Calculator, CheckCircle, AlertCircle }
from 'lucide-react'
import { universalApi }
...
const calculateTotal = () => { return lineItems.reduce((sum, item) => sum + item.quantity * item.unit_amount, 0  ) return ( <div className="bg-[var(--color-body)] space-y-6"> <Card> <CardHeader> <CardTitle className="flex items-center gap-2"> <Calculator className="w-5 h-5" /> New Sales Order </CardTitle> </CardHeader> <CardContent className="space-y-6"> {/* Customer Selection */} <div className="grid grid-cols-2 gap-4"> <div className="space-y-2"> <Label>Customer *</Label> <Select value={selectedCustomer} onValueChange={setSelectedCustomer}> <SelectTrigger> <SelectValue placeholder="Select customer" /> </SelectTrigger> <SelectContent> {customers.map(customer => ( <SelectItem key={customer.id} value={customer.id}> {customer.entity_name} </SelectItem> ))} </SelectContent> </Select> </div> <div className="space-y-2"> <Label>Customer PO Number</Label> <Input value={customerPO} onChange={e => setCustomerPO(e.target.value)} placeholder="CUST-PO-2025-001" /> </div> </div> {/* Line Items */} <div className="space-y-2"> <div className="flex justify-between items-center"> <Label>Products</Label> <Button type="button" variant="outline" size="sm" onClick={addLineItem}> <Plus className="w-4 h-4 mr-1" /> Add Product </Button> </div> {lineItems.length > 0 && ( <Table> <TableHeader> <TableRow> <TableHead>Product</TableHead> <TableHead className="w-24">Quantity</TableHead> <TableHead className="w-32">Unit Price</TableHead> <TableHead className="w-32">Line Total</TableHead> <TableHead className="w-20">Actions</TableHead> </TableRow> </TableHeader> <TableBody> {lineItems.map((item, index) => ( <TableRow key={index}> <TableCell> <Select value={item.entity_id} onValueChange={value => updateLineItem(index, 'entity_id', value)} > <SelectTrigger> <SelectValue placeholder="Select product" /> </SelectTrigger> <SelectContent> {products.map(product => ( <SelectItem key={product.id} value={product.id}> {product.entity_name} </SelectItem> ))} </SelectContent> </Select> </TableCell> <TableCell> <Input type="number" min="1" value={item.quantity} onChange={e => updateLineItem(index, 'quantity', parseInt(e.target.value)  ) /> </TableCell> <TableCell> <Input type="number" value={item.unit_amount} onChange={e => updateLineItem(index, 'unit_amount', parseFloat(e.target.value)  ) /> </TableCell> <TableCell>{(item.quantity * item.unit_amount).toFixed(2)}</TableCell> <TableCell> <Button type="button" variant="ghost" size="sm" onClick={() => removeLineItem(index)} > <Trash2 className="w-4 h-4" /> </Button> </TableCell> </TableRow> ))} </TableBody> </Table> )} {lineItems.length === 0 && ( <div className="bg-[var(--color-body)] text-center py-8 text-[var(--color-text-secondary)]"> No products added. Click"Add Product" to start. </div> )} </div> {/* Delivery Address */} <div className="bg-[var(--color-body)] space-y-2"> <Label>Delivery Address</Label> <div className="bg-[var(--color-body)] grid grid-cols-2 gap-4"> <Input placeholder="Address Line 1" value={deliveryAddress.line1} onChange={e => setDeliveryAddress({ ...deliveryAddress, line1: e.target.value })} /> <Input placeholder="Address Line 2" value={deliveryAddress.line2} onChange={e => setDeliveryAddress({ ...deliveryAddress, line2: e.target.value })} /> <Input placeholder="City" value={deliveryAddress.city} onChange={e => setDeliveryAddress({ ...deliveryAddress, city: e.target.value })} /> <Input placeholder="State" value={deliveryAddress.state} onChange={e => setDeliveryAddress({ ...deliveryAddress, state: e.target.value })} /> <Input placeholder="Country" value={deliveryAddress.country} onChange={e => setDeliveryAddress({ ...deliveryAddress, country: e.target.value })} /> <Input placeholder="Postal Code" value={deliveryAddress.postal} onChange={e => setDeliveryAddress({ ...deliveryAddress, postal: e.target.value })} /> </div> </div> {/* Special Instructions */} <div className="bg-[var(--color-body)] space-y-2"> <Label>Special Instructions</Label> <Textarea value={specialInstructions} onChange={e => setSpecialInstructions(e.target.value)} placeholder="Any special delivery or packaging instructions..." rows={3} /> </div> {/* Validation Errors */} {validationErrors.length > 0 && ( <Alert variant="destructive"> <AlertCircle className="h-4 w-4" /> <AlertDescription> <ul className="bg-[var(--color-body)] list-disc list-inside"> {validationErrors.map((error, index) => ( <li key={index}>{error}</li> ))} </ul> </AlertDescription> </Alert> )} {/* Pricing Summary */} {pricingSummary && ( <Card className="bg-[var(--color-surface-raised)] border-[var(--color-border)] bg-[var(--color-body)]/50"> <CardContent className="pt-6"> <div className="bg-[var(--color-body)] space-y-2"> <div className="flex justify-between text-sm"> <span>Subtotal:</span> <span>AED {calculateTotal().toFixed(2)}</span> </div> {pricingSummary.lineItems.some(item => item.discount_amount) && ( <div className="flex justify-between text-sm text-green-600"> <span>Total Discounts:</span> <span> - AED{' '} {pricingSummary.lineItems .reduce((sum, item) => sum + (item.discount_amount || 0), 0) .toFixed(2)} </span> </div> )} <div className="flex justify-between font-semibold text-lg pt-2 border-t"> <span>Total Amount:</span> <span>AED {pricingSummary.totalAmount.toFixed(2)}</span> </div> </div> </CardContent> </Card> )} {/* Approval Required */} {approvalRequired?.required && ( <Alert> <AlertCircle className="h-4 w-4" /> <AlertDescription> <strong>Approval Required:</strong> {approvalRequired.rule} <br /> This order exceeds AED {approvalRequired.threshold} and requires approval from:{' '} {approvalRequired.approvers?.join(', ')} </AlertDescription> </Alert> )} {/* Promised Delivery Date */} {promisedDeliveryDate && ( <Alert className="border-[#6b6975] bg-[var(--color-body)]"> <CheckCircle className="h-4 w-4 text-primary" /> <AlertDescription className="text-[var(--color-text-primary)]"> <strong>Promised Delivery Date:</strong> {formatDate(promisedDeliveryDate, 'PPP')} </AlertDescription> </Alert> )} {/* Success/Error Messages */} {error && ( <Alert variant="destructive"> <AlertCircle className="h-4 w-4" /> <AlertDescription>{error}</AlertDescription> </Alert> )} {success && ( <Alert className="border-green-200 bg-green-50"> <CheckCircle className="h-4 w-4 text-green-600" /> <AlertDescription className="text-green-800">{success}</AlertDescription> </Alert> )} {/* Actions */} <div className="flex justify-between"> <div className="text-lg font-semibold">Total: AED {calculateTotal().toFixed(2)}</div> <div className="space-x-2"> <Button type="button" variant="outline" onClick={validateAndCalculate} disabled={loading || lineItems.length === 0} > Validate & Calculate </Button> <Button onClick={handleSubmit} disabled={loading || lineItems.length === 0 || !selectedCustomer} > {loading ? 'Creating...' : 'Create Sales Order'} </Button> </div> </div> </CardContent> </Card> </div> )
}
