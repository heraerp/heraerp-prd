...
import { TrendingUp, TrendingDown, DollarSign, Package, Users, Calendar, BarChart3 } from 'lucide-react'
    ...
    const fetchAnalytics = async () => {
      ...
      try {
        ...
        const salesRes = await fetch(
          `/api/playbook/analytics/daily-sales?` + new URLSearchParams({
        ...
        const topRes = await fetch(
          `/api/playbook/analytics/top-items?` + new URLSearchParams({
            orgId: organizationId,
            startDate: format(dateRange.from, 'yyyy-MM-dd'),
            endDate: format(dateRange.to, 'yyyy-MM-dd'),
            limit: '5'
        ...
        <Card className="border-0" style={{
          ...
          <CardContent>
            <div className="space-y-2 max-h-[300px] overflow-y-auto">
              {isLoading ? (
                <div className="text-center py-8" style={{ color: COLORS.bronze }}>Loading...</div>
              ) : dailySales.length === 0 ? (
                <div className="text-center py-8" style={{ color: COLORS.bronze }}>No sales data for this period</div>
              ) : (
                dailySales.map((day) => (
                  <div key={day.sale_date} className="flex items-center justify-between py-2 border-b" style={{ borderColor: `${COLORS.bronze}33` }}>
            ...
            <div className="space-y-2">
              {isLoading ? (
                ...
                topItems.map((item, idx) => (
                  <div key={item.item_id} className="flex items-center justify-between py-2 border-b" style={{ borderColor: `${COLORS.bronze}33` }}>
                    <div className="flex items-center gap-3">
                      <span className="text-lg font-bold" style={{ color: COLORS.gold }}>#{idx + 1}</span>
                      <div>
                        <div className="font-medium" style={{ color: COLORS.champagne }}>
                          {item.item_name}
                        </div>
                        <div className="text-xs" style={{ color: COLORS.bronze }}>
                          {item.entity_type === 'salon_service' ? 'Service' : 'Product'} • Sold {item.times_sold} times
                        </div>
                      </div>
                    </div>
