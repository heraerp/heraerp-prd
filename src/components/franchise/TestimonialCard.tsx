...
export function TestimonialCard({
  ...
  if (variant === 'compact') {
    return (
      <div
        className={cn(
          'bg-white dark:bg-slate-800 rounded-lg p-6 border border-slate-200 dark:border-slate-700 shadow-sm',
          className
        )}
      >
        <div className="flex items-center mb-4">
          <div className="flex mr-2">{renderStars(rating)}</div>
          <span className="text-sm text-slate-600 dark:text-slate-400">{rating}/5</span>
        </div>
  ...
  if (variant === 'featured') {
    return (
      <div
        className={cn(
          'bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 dark:from-slate-800 dark:via-slate-700 dark:to-slate-800 rounded-2xl p-8 border border-blue-200 dark:border-slate-600 shadow-lg relative overflow-hidden',
          className
        )}
      >
        <Quote className="absolute top-6 right-6 h-12 w-12 text-blue-200 dark:text-slate-600" />

        <div className="flex items-center mb-6">
          <div className="flex mr-3">{renderStars(rating)}</div>
        </div>

        <blockquote className="text-lg text-slate-800 dark:text-slate-200 leading-relaxed mb-6 font-medium">
          "{testimonial}"
        </blockquote>
  ...
  return (
    <div
      className={cn(
        'bg-white dark:bg-slate-800 rounded-xl p-6 border border-slate-200 dark:border-slate-700 shadow-sm hover:shadow-md transition-shadow duration-200',
        className
      )}
    >
      <div className="flex items-center mb-4">
