        ...
        {totalDuration > 0 && (
          <div className="mt-4 p-3 bg-blue-50 rounded-md">
            <p className="text-sm text-blue-700">
              Total duration: <strong>{Math.floor(totalDuration / 60)}h {totalDuration % 60}m</strong>
            </p>
          </div>
        )}
      </Card>
          ...
          {loadingSlots ? (
            ...
            <div className="grid grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
              {availableSlots.map((slot, idx) => {
                const isSelected = 
                  form.watch('start_time') === slot.start &&
                  form.watch('end_time') === slot.end
          ...
          {form.watch('start_time') && (
            <div className="mt-4 p-3 bg-green-50 rounded-md">
              <p className="text-sm text-green-700">
                Selected: <strong>{formatTime(form.watch('start_time'))} - {formatTime(form.watch('end_time'))}</strong>
              </p>
            </div>
          )}
        </Card>
