...
export function BOMExplorer({ productId, organizationId, className }: BOMExplorerProps) {
      ...
      for (const rel of bomComponentRels) {
        ...
        if (component) {
          ...
          components.push({
            ...
            code: component.entity_code,
            type: component.entity_type,
            quantity: rel.relationship_data?.quantity || 1,
            unit: rel.relationship_data?.unit_of_measure || 'EA',
            scrapPercentage: rel.relationship_data?.scrap_percentage,
            unitCost: rel.relationship_data?.unit_price,
            leadTime: supplierRel?.relationship_data?.lead_time_days,
            supplier: supplier?.entity_name,
            children: [] // Could recursively load sub-components
          })
        }
      }
  ...
  const renderBOMNode = (node: BOMComponent, level: number = 0) => {
    ...
    return (
      ...
      >
        ...
        >
          {hasChildren && (
            <Button variant="ghost" size="icon" className="h-5 w-5 hover:bg-[var(--color-hover)]">
              {isExpanded ? (
                ...
                {node.scrapPercentage && (
                  <TooltipProvider>
                    <Tooltip>
                      ...
                      <TooltipContent>
                        <p>Material includes {node.scrapPercentage}% scrap allowance</p>
                      </TooltipContent>
                    </Tooltip>
                  </TooltipProvider>
                )}
      ...
      <Card className={cn('bg-[var(--color-body)]/50 border-[var(--color-border)]', className)}>
        ...
        <CardContent>
          <div className="text-center py-8">
            ...
            <Button
              onClick={loadBOMData}
              className="bg-[var(--color-button-bg)] text-[var(--color-button-text)] hover:bg-[var(--color-button-hover)] mt-4"
            >
              Retry
            </Button>
          </div>
        ...
        {bomData.children && bomData.children.length > 0 && (
          <div className="mt-6 pt-4 border-t border-[var(--color-border)]">
            <div className="flex justify-between items-center text-sm">
              <span className="text-[var(--color-text-secondary)]">Total Components</span>
              <span className="font-semibold">{bomData.children.length}</span>
            </div>
            {bomData.totalCost && (
              <div className="flex justify-between items-center text-sm mt-2">
                <span className="text-[var(--color-text-secondary)]">Total Material Cost</span>
