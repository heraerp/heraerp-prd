...
const loadingRef = useRef(false) // Load initial data const loadData = useCallback( async (reset = false) => { if (loadingRef.current && !reset) return try { loadingRef.current = true
  const page = reset ? 1 : currentPage if (page === 1) {
...
const result = await fetchData(page, pageSize, sortBy, sortOrder) if (reset) {
  ...
  setError(err instanceof Error ? err.message : 'Failed to load data'  ) finally { setLoading(false) setLoadingMore(false) loadingRef.current = false } }, [currentPage, fetchData, pageSize, sortBy, sortOrder] ) // Initial load useEffect(() => { loadData(true  ), [sortBy, sortOrder]) // Handle scroll for lazy loading useEffect(() => {
  const container = tableContainerRef.current if (!container) 
    return const handleScroll = () => { if (!hasMore || loadingRef.current) return const { scrollTop, scrollHeight, clientHeight } = container const distanceFromBottom = scrollHeight - scrollTop - clientHeight if (distanceFromBottom < loadMoreThreshold) {
  loadData(false  ) } container.addEventListener('scroll', handleScroll, { passive: true }) 
    return () => container.removeEventListener('scroll', handleScroll  ), [hasMore, loadData, loadMoreThreshold]) // Handle sorting
  const handleSort = (columnKey: string) => { if (sortBy === columnKey) {
