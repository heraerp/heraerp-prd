...
export function UCRRuleManager({ organizationId, className }: UCRRuleManagerProps) {
  ...
  const loadRules = async () => {
    try {
      setLoading(true)
      // Get UCR rule entities
      const { data: entities } = await universalApi.read({ table: 'core_entities', organizationId })
      ...
      const rulesWithLogic = ucrRules.map((rule: any) => {
        ...
        return {
          ...
          scope: parseField('rule_scope'),
          condition: parseField('rule_condition'),
          action: parseField('rule_action'),
  ...
  const testRule = async (rule: UCRRule) => {
  ...
  return (
    <div className={cn('space-y-6', className)}>
      ...
      <div className="flex items-center justify-between">
        ...
        <div>
          {' '}
          <h2 className="bg-[var(--color-body)] text-2xl font-bold text-[var(--color-text-primary)]">
            Universal Configuration Rules
          </h2>{' '}
          <p className="text-[var(--color-text-secondary)] mt-1">
            Business logic as data - zero code changes required
          </p>{' '}
        </div>{' '}
        ...
        <Card className="bg-[var(--color-surface-raised)] border-[var(--color-border)] bg-[var(--color-body)]/50 border-[var(--color-border)]">
          ...
          <CardContent className="p-4">
            ...
            <div className="flex items-center justify-between">
              ...
              <div>
                {' '}
                <p className="text-sm text-[var(--color-text-secondary)]">Total Rules</p>{' '}
                <p className="text-2xl font-bold text-[var(--color-text-primary)]">
                  {rules.length}
                </p>{' '}
              </div>{' '}
              <Settings className="h-8 w-8 text-[var(--color-text-primary)]" />{' '}
            </div>{' '}
          </CardContent>{' '}
        </Card>{' '}
        <Card className="bg-[var(--color-surface-raised)] border-[var(--color-border)] bg-[var(--color-body)]/50 border-[var(--color-border)]">
        ...
        <Card className="bg-[var(--color-surface-raised)] border-[var(--color-border)] bg-[var(--color-body)]/50 border-[var(--color-border)]">
          ...
          <CardContent className="p-4">
            ...
            <div className="flex items-center justify-between">
              ...
              <div>
                {' '}
                <p className="text-sm text-[var(--color-text-secondary)]">Rule Types</p>{' '}
                <p className="text-2xl font-bold text-[var(--color-text-primary)]">
                  {' '}
                  {new Set(rules.map(r => r.type)).size}{' '}
                </p>{' '}
              </div>{' '}
              <Code className="h-8 w-8 text-[var(--color-text-primary)]" />{' '}
            </div>{' '}
          </CardContent>{' '}
        </Card>{' '}
        <Card className="bg-[var(--color-surface-raised)] border-[var(--color-border)] bg-[var(--color-body)]/50 border-[var(--color-border)]">
          ...
          <CardContent className="p-4">
            ...
            <div className="flex items-center justify-between">
              ...
              <div>
                {' '}
                <p className="text-sm text-[var(--color-text-secondary)]">Executions Today</p>{' '}
                <p className="text-2xl font-bold text-[var(--color-text-primary)]">
                  {executionResults.length}
                </p>{' '}
              </div>{' '}
              <Play className="h-8 w-8 text-[var(--color-text-primary)]" />{' '}
            </div>{' '}
          </CardContent>{' '}
        </Card>{' '}
      </div>{' '}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {' '}
        {/* Rule List */}{' '}
        <Card className="bg-[var(--color-surface-raised)] border-[var(--color-border)] bg-[var(--color-body)]/50 border-[var(--color-border)] lg:col-span-1">
          {' '}
          <CardHeader>
            {' '}
            ...
            <div className="space-y-1 p-4">
              ...
              {rules.map(rule => (
                <div
                  key={rule.id}
                  className={cn(
                    'p-3 rounded-lg cursor-pointer transition-all hover:bg-[var(--color-sidebar)]/30/50',
                    selectedRule?.id === rule.id &&
                      'bg-muted-foreground/10/50 ring-1 ring-amber-500'
                  )}
        ...
        {/* Rule Details */}{' '}
        {selectedRule && (
          <Card className="bg-[var(--color-surface-raised)] border-[var(--color-border)] bg-[var(--color-body)]/50 border-[var(--color-border)] lg:col-span-2">
            {' '}
            <CardHeader>
              {' '}
            ...
            <CardContent>
              ...
              <Tabs defaultValue="logic">
                ...
                <TabsContent value="logic" className="bg-[var(--color-body)] space-y-4 mt-4">
                  {' '}
                  {/* Scope */}{' '}
                  <div>
                    {' '}
                    <h4 className="bg-[var(--color-body)] text-sm font-medium text-[var(--color-text-secondary)] mb-2">
                      Scope
                    </h4>{' '}
                    <div className="bg-[var(--color-body)]/50 rounded-lg p-3">
                      {' '}
                      <pre className="bg-[var(--color-body)] text-xs text-[var(--color-text-primary)] overflow-x-auto">
                        {' '}
                ...
                <TabsContent value="parameters" className="bg-[var(--color-body)] mt-4">
                  {' '}
                  <div className="bg-[var(--color-body)]/50 rounded-lg p-3">
                    {' '}
                    <pre className="bg-[var(--color-body)] text-xs text-[var(--color-text-primary)] overflow-x-auto">
                ...
                <TabsContent value="execution" className="bg-[var(--color-body)] mt-4">
                  {' '}
                  {executionResults.length === 0 ? (
                    <Alert className="bg-[var(--color-body)]/50 border-[var(--color-border)]">
                      {' '}
                    ...
                    <div className="space-y-3">
                      {' '}
                      {executionResults
                        .filter(r => r.ruleId === selectedRule.id)
                        .map((result, idx) => (
                          <div
                            key={idx}
                            className={cn(
                          ...
                          >
                            ...
                            <pre className="bg-[var(--color-body)] text-xs text-[var(--color-text-primary)]">
                              {' '}
