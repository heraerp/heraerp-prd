...
export function DependencyGraph({ modules, relationships }: DependencyGraphProps) {
  ...
  return (
    <section aria-label="Dependency Graph" className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-6">
        ...
        {selectedNode && (
          ...
          >
            <h3 className="font-semibold text-gray-900 dark:text-white flex items-center gap-2">
              {selectedNode.type === 'module' ? <Package className="w-4 h-4" /> : <Info className="w-4 h-4" />}
              {selectedNode.name}
            </h3>
            ...
            {selectedNode.type === 'module' && (
              <div className="mt-3 space-y-2">
                <p className="text-xs font-medium text-gray-700 dark:text-gray-300">Dependencies:</p>
                {edges
