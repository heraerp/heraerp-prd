...
const SUPPORTED_DOMAINS = [
  { value: 'CA', label: 'Chartered Accountancy', description: 'Professional accounting and finance' },
...
export function UniversalLearningInterface({ className }: UniversalLearningInterfaceProps) {
  ...
  const updateStepStatus = useCallback((stepId: string, status: ProcessingStep['status'], progress: number = 0, result?: any) => {
    setProcessingSteps(prev => prev.map(step => 
      step.id === stepId 
        ? { ...step, status, progress, result, duration: status === 'completed' ? Date.now() : step.duration }
        : step
    ));
  }, []);

  const simulateStepProcessing = useCallback(async (stepId: string, duration: number = 2000) => {
  ...
  const handleFileUpload = useCallback((event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (selectedFiles && selectedFiles.length > 0) {
      ...
      if (file.type.startsWith('text/') || file.name.endsWith('.md') || file.name.endsWith('.txt')) {
        ...
        setContent(`📄 Word Document Selected: ${file.name} (${(file.size / 1024).toFixed(1)} KB)\n\nDocument content will be extracted during processing...`);
      } else {
  ...
  const processUniversalLearning = useCallback(async () => {
    ...
    try {
      ...
      console.log(`📝 Content being sent: "${content.substring(0, 100)}..." (${content.length} chars)`);
      ...
      if (files && files.length > 0) {
        ...
        if (content && content.trim().length > 0 && !content.includes('File Selected:') && !content.includes('PDF content will be extracted')) {
      ...
      if (!response.ok) {
        ...
        throw new Error(apiResult.error || `API request failed: ${response.status} ${response.statusText}`);
      }
  ...
  }, [content, files, selectedDomain, options, metadata, currentStep, processingSteps, simulateStepProcessing, updateStepStatus]);
    ...
    <div className={`max-w-7xl mx-auto p-6 space-y-6 ${className}`}>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
        <TabsList className="grid w-full grid-cols-4">
          ...
          <TabsTrigger value="processing" disabled={!isProcessing && currentStep === 0}>Processing</TabsTrigger>
          <TabsTrigger value="results" disabled={!result}>Results</TabsTrigger>
          <TabsTrigger value="export" disabled={!result}>Export & Share</TabsTrigger>
        </TabsList>
          ...
          <div className="grid lg:grid-cols-2 gap-6">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Upload files or paste content to process
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                ...
                <div>
                  ...
                  {content && (
                    <div className="mt-2 text-sm text-muted-foreground">
                      {content.length} characters, ~{Math.ceil(content.split(' ').length / 250)} minutes to read
                    </div>
                  )}
                </div>
              </CardContent>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Configure domain and processing options
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                {/* Domain Selection */}
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <Select value={options.target_audience} onValueChange={(value) => setOptions(prev => ({ ...prev, target_audience: value }))}>
                      <SelectTrigger>
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent>
                    ...
                    <Select value={options.learning_style} onValueChange={(value) => setOptions(prev => ({ ...prev, learning_style: value }))}>
                      <SelectTrigger>
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent>
                    ...
                    <AlertDescription>
                      ...
                      {error.includes('Content or files are required') && (
                        <div className="mt-2 text-sm">
                          💡 <strong>Tip:</strong> Make sure you've entered some educational content in the text area before clicking "Process Universal Learning"
                        </div>
                      )}
                    </AlertDescription>
                  </Alert>
                ...
                <Card>
                  <CardContent className="p-4">
                    <div className="flex items-center space-x-2">
                      ...
                      <div>
                        <div className="text-2xl font-bold">{result.pipeline_summary?.total_universal_elements || 0}</div>
                ...
                <Card>
                  <CardContent className="p-4">
                    <div className="flex items-center space-x-2">
                      ...
                      <div>
                        <div className="text-2xl font-bold">{result.pipeline_summary?.total_entities_created || 0}</div>
                ...
                <Card>
                  <CardContent className="p-4">
                    <div className="flex items-center space-x-2">
                      ...
                      <div>
                        <div className="text-2xl font-bold">{result.pipeline_summary?.total_learning_paths || 0}</div>
                ...
                <Card>
                  <CardContent className="p-4">
                    <div className="flex items-center space-x-2">
                      ...
                      <div>
                        <div className="text-2xl font-bold">{result.pipeline_summary?.cross_domain_insights || 0}</div>
                  ...
                  <Card>
                    ...
                    <CardContent>
                      <ScrollArea className="h-[400px]">
                        {result.ai_analysis?.universalElements?.map((element: any, index: number) => (
                          <div key={index} className="mb-4 p-4 border rounded-lg">
                            <div className="flex items-center justify-between mb-2">
                              <Badge variant="outline">{element.type}</Badge>
                              <Badge variant="secondary">
                                {((element.universalApplicability || 0) * 100).toFixed(0)}% Universal
                              </Badge>
                            </div>
                            ...
                            <div className="flex items-center space-x-4 text-xs text-muted-foreground">
                              ...
                              <span>Style: {element.learningScience?.learningStyle || 'N/A'}</span>
                              <span>Difficulty: {element.learningScience?.difficulty || 'N/A'}</span>
                            </div>
                          </div>
                        ))}
                      </ScrollArea>
                ...
                <TabsContent value="entities">
                  <Card>
                    <CardHeader>
                      ...
                      <CardDescription>Educational content converted to HERA's universal 6-table architecture</CardDescription>
                    </CardHeader>
                    <CardContent>
                      <ScrollArea className="h-[400px]">
                        {result.entity_creation?.coreEntities?.map((entity: any, index: number) => (
                          <div key={index} className="mb-4 p-4 border rounded-lg">
                            ...
                            <p className="text-sm text-muted-foreground mb-2">Code: {entity.entity_code}</p>
                            <p className="text-xs text-muted-foreground">Smart Code: {entity.smart_code}</p>
                          </div>
                        ))}
                      </ScrollArea>
                    </CardContent>
                ...
                <TabsContent value="paths">
                  <Card>
                    <CardHeader>
                      ...
                      <CardDescription>Adaptive learning paths for different learner types</CardDescription>
                    </CardHeader>
                    <CardContent>
                      <ScrollArea className="h-[400px]">
                        {result.learning_paths?.map((path: any, index: number) => (
                          <div key={index} className="mb-4 p-4 border rounded-lg">
                            <div className="flex items-center justify-between mb-2">
                              ...
                              <Badge variant="secondary">{path.estimatedDuration || 0} minutes</Badge>
                            </div>
                            <h4 className="font-medium mb-2">{path.pathName || 'Learning Path'}</h4>
                            <p className="text-sm text-muted-foreground mb-2">{path.description || 'No description available'}</p>
                            <div className="flex items-center space-x-4 text-xs text-muted-foreground">
                ...
                <TabsContent value="specialization">
                  <Card>
                    <CardHeader>
                      ...
                      <CardDescription>{selectedDomain} specific enhancements and professional context</CardDescription>
                    </CardHeader>
                    <CardContent>
                      <ScrollArea className="h-[400px]">
                          ...
                          <div className="space-y-4">
                            <div className="p-4 border rounded-lg">
                              ...
                              <p className="text-sm text-muted-foreground">
                                Enhanced with {selectedDomain}-specific professional context and industry standards
                              </p>
                            </div>
                            <div className="p-4 border rounded-lg">
                            ...
                            <div className="p-4 border rounded-lg">
                              ...
                              <p className="text-sm text-muted-foreground">
                                {result.domain_specialization.practicalApplications?.length || 0} real-world scenarios identified
                              </p>
                            </div>
                          </div>
                        )}
                ...
                <TabsContent value="intelligence">
                  <Card>
                    <CardHeader>
                      ...
                      <CardDescription>Insights from other domains that can enhance learning</CardDescription>
                    </CardHeader>
                    <CardContent>
                      <ScrollArea className="h-[400px]">
                        {result.cross_domain_intelligence && (
                          <div className="space-y-4">
                            <div className="p-4 border rounded-lg">
                              ...
                              <p className="text-sm text-muted-foreground">
                                Identified {result.cross_domain_intelligence.learningTransfers?.length || 0} opportunities 
                                to apply insights from other domains
                              </p>
                            </div>
                            <div className="p-4 border rounded-lg">
                              <h4 className="font-medium mb-2">Universal Patterns</h4>
                              <p className="text-sm text-muted-foreground">
                                {result.cross_domain_intelligence.universalPatterns?.length || 0} universal learning patterns 
                                detected across domains
                              </p>
                            </div>
                            <div className="p-4 border rounded-lg">
                              <h4 className="font-medium mb-2">Predictive Insights</h4>
                              <p className="text-sm text-muted-foreground">
                                {result.cross_domain_intelligence.predictiveInsights?.length || 0} predictive insights 
                                for optimizing future learning
                              </p>
                            </div>
                          </div>
                        )}
