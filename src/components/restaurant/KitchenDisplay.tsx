...
export function KitchenDisplay({
  ...
  const loadData = async () => {
    try {
      ...
      const newOrders = todayOrders.filter(
        o => !(o.metadata as any)?.status || o.metadata.status === 'new'
      ).length
      const inProgress = todayOrders.filter(
        o => (o.metadata as any)?.status === 'in_progress'
      ).length
      const ready = todayOrders.filter(o => (o.metadata as any)?.status === 'ready').length
      const avgPrepTime =
        todayOrders
          .filter(
            o => (o.metadata as any)?.status === 'ready' || (o.metadata as any)?.status === 'served'
          )
    ...
    try {
      ...
      setOrders(prevOrders =>
        prevOrders.map(order =>
          order.id === orderId ? { ...order, metadata: { ...order.metadata, status } } : order
        )
      )
  ...
  const filterOrdersByStation = (orders: Order[], station: string) => {
    ...
    return orders.filter(order =>
      order.items?.some(item => (item.metadata as any)?.station === station)
    )
  }
    ...
    <div className="space-y-6">
      ...
      <Tabs value={activeStation} onValueChange={setActiveStation}>
        <TabsList className="grid w-full grid-cols-5">
          <TabsTrigger value="all">All Stations ({orders.length})</TabsTrigger>
