...
export function OrderManager() {
  ...
  const loadOrders = async () => {
    ...
    if (result?.success && result.data && Array.isArray(result.data)) {
      ...
      const transformedOrders = result.data.map((transaction: any) => ({
        id: transaction.id,
        reference_number: transaction.reference_number || `#${transaction.id.slice(-6).toUpperCase()}`,
        ...
        estimated_ready_time: new Date(new Date(transaction.transaction_date).getTime() + 20 * 60000),
  ...
  const getOrderTypeIcon = (type: string) => {
    switch (type) {
      case 'dine_in': return <User className="w-4 h-4" />
      case 'takeout': return <Plus className="w-4 h-4" />
            ...
            <div className="p-4">
              ...
              <div className="flex justify-between items-center mb-4 text-sm">
                ...
                <span className={`font-medium ${
                  ...
                  {order.status === 'approved' ? 'Ready!' : getTimeUntilReady(order.estimated_ready_time)}
                </span>
              </div>
        ...
        <div className="text-center">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs text-green-700">
            <div className="bg-white/50 p-3 rounded-lg">
              ...
              Each order is a transaction with type "order", storing header info like customer, total, status
            </div>
            <div className="bg-white/50 p-3 rounded-lg">
