...
import { PnLRow, ReportCalculations } from '@/src/lib/schemas/reports'
...
export function PnLTable({
  ...
  const [expandedGroups, setExpandedGroups] = React.useState<Set<string>>(new Set(['revenue', 'expenses']))
  ...
  const getAmountColor = (amount: number, group: string) => {
    ...
    if (group === 'revenue' || group.includes('profit') || group.includes('income')) {
      return isPositive ? 'text-emerald-700 dark:text-emerald-400' : 'text-red-700 dark:text-red-400'
    }
  ...
  const getRowStyles = (row: PnLRow, index: number) => {
    ...
    if (row.is_subtotal) {
      const subtotalClasses = {
        ...
        'net_income': 'bg-emerald-50 dark:bg-emerald-950/30 border-l-2 border-emerald-400 font-bold text-lg'
      }
                ...
                <div className="flex items-center justify-end gap-1">
                  ...
                  {row.variance_percent && (
                    <Badge 
                      ...
                      className={cn(
                        "text-xs ml-1",
                        row.variance_percent > 0 ? "text-emerald-700 border-emerald-300" : "text-red-700 border-red-300"
                      )}
                    >
        ...
        <div className="rounded-md border border-violet-200 dark:border-violet-800">
          <Table>
            <caption className="sr-only">
              Profit and loss statement showing revenue, expenses, and net income with hierarchical grouping
            </caption>
            {renderTableHeader()}
            <TableBody>
