...
export function SummaryCard({
  ...
  const formatValue = (val: number | string, type: 'currency' | 'percentage' | 'number' = 'currency') => {
            ...
            {trend && (
              <div className="flex items-center gap-1">
                ...
                >
                  ...
                  <span className="ml-1">{trend.value > 0 ? '+' : ''}{trend.value.toFixed(1)}%</span>
                </Badge>
    ...
    return (
      <div className={cn('grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4', className)}>
        ...
        <SummaryCard
          ...
          icon={summary.net_income && summary.net_income >= 0 ? <TrendingUp className="h-4 w-4" /> : <TrendingDown className="h-4 w-4" />}
  ...
  return (
    <div className={cn('grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4', className)}>
      ...
      <SummaryCard
        ...
        icon={summary.balance_check?.is_balanced ? <TrendingUp className="h-4 w-4" /> : <TrendingDown className="h-4 w-4" />}
      ...
      {summary.current_ratio && (
        <SummaryCard
          ...
          icon={<Percent className="h-4 w-4" />}
          variant={summary.current_ratio >= 2 ? 'revenue' : summary.current_ratio >= 1 ? 'default' : 'expense'}
        />
      )}
      ...
      {summary.debt_to_equity && (
        <SummaryCard
          ...
          icon={<Percent className="h-4 w-4" />}
          variant={summary.debt_to_equity <= 1 ? 'revenue' : summary.debt_to_equity <= 2 ? 'default' : 'expense'}
        />
      )}
