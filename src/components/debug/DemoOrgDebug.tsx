  ...
  return (
    <div className="fixed top-4 right-4 bg-black/80 text-white p-4 rounded-lg text-xs z-50 max-w-md">
      ...
      <div className="space-y-1">
        ...
        <p>
          <strong>User:</strong> {user?.name || 'None'} ({user?.email || 'No email'})
        </p>
        <p>
          <strong>Current Org:</strong> {currentOrganization?.name || 'None'}
        </p>
        <p>
          <strong>Org ID:</strong> {currentOrganization?.id || 'None'}
        </p>
        <p>
          <strong>Total Orgs:</strong> {organizations.length}
        </p>
        <p>
          <strong>Pathname:</strong> {pathname || 'Loading...'}
        </p>
      </div>
    </div>
  )
