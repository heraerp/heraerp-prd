...
export class FinancialSmartCodeService {
  ...
  async generateSmartCode(request: SmartCodeGenerationRequest): Promise<{
    ...
    try {
      ...
      const validation = await this.validateSmartCode(smartCode, request.organizationId, 'L4_INTEGRATION')
  ...
  async searchSmartCodes(
    ...
    try {
      ...
      if (filters.includeSystem) {
        query = query.in('organization_id', [organizationId, '719dfed1-09b4-4ca8-bfda-f682460de945'])
      } else {
  ...
  private generateMetadata(smartCode: string, request: SmartCodeGenerationRequest): FinancialSmartCode['metadata'] {
    const parts = smartCode.split('.')
    const [, , subModule, functionType, entityType] = parts
  ...
  private validateSemantics(smartCode: string): { isValid: boolean; errors: string[]; warnings: string[] } {
    const errors: string[] = []
    const warnings: string[] = []
    ...
    if (subModule === 'GL' && functionType === 'ENT' && !['ACC', 'JE', 'CC', 'DEP'].includes(entityType)) {
      warnings.push(`Entity type '${entityType}' is uncommon for GL entities`)
    }
    
    if (subModule === 'AR' && functionType === 'TXN' && !['SAL', 'RCP', 'ADJ'].includes(entityType)) {
  ...
  private async validatePerformance(smartCode: string, organizationId: string): Promise<{ warnings: string[]; suggestions: string[] }> {
    const warnings: string[] = []
    const suggestions: string[] = []
  ...
  private async validateIntegration(smartCode: string, organizationId: string): Promise<{ 
  ...
  private async validateBusinessRule(rule: string, smartCode: string, organizationId: string): Promise<boolean> {
