...
export const UniversalFinanceEventSchema = z.object({
  ...
  transaction_type: z.string()
    .regex(/^TX\.FINANCE\.[A-Z_]+\.V\d+$/, 'Transaction type must follow TX.FINANCE.{KIND}.V{N} pattern')
  ...
  smart_code: z.string()
    .regex(/^HERA\.SALON\.FINANCE\.TXN\.[A-Z_]+\.[A-Z_]+\.V\d+$/, 'Smart code must follow HERA.SALON.FINANCE.TXN.{KIND}.{SUBKIND}.V{N} pattern')
  ...
  source_entity_id: z.string().uuid().optional()
    .describe('Optional entity reference (customer for sales, vendor for purchases, staff for payroll)'),
  ...
  // Currency handling
  transaction_currency_code: z.string()
  ...
  business_context: z.object({
    channel: z.enum(['MCP', 'POS', 'BANK', 'MANUAL', 'IMPORT'])
      .describe('Source channel for the transaction'),
    
    note: z.string().optional()
      .describe('Free text description for the transaction'),
    
    branch_id: z.string().uuid().optional()
      .describe('Optional branch/location reference'),
    
    category: z.string().optional()
      .describe('Business category (Payroll, Rent, POS, etc.) for posting rule lookup'),
    
    // POS-specific context
    pos_terminal: z.string().optional()
      .describe('POS terminal identifier for EOD summaries'),
    
    shift_id: z.string().uuid().optional()
      .describe('Shift reference for POS transactions'),
    
    // Additional context for complex transactions
    is_eod_summary: z.boolean().default(false)
      .describe('Flag indicating this is an end-of-day summary transaction'),
    
    vat_inclusive: z.boolean().default(true)
      .describe('Whether the total_amount includes VAT')
  }).describe('Business context for automated posting rule resolution'),
  
  // Technical metadata
  metadata: z.object({
    ingest_source: z.string()
      .describe('Technical source system (MCP_Bank, POS_Terminal, etc.)'),
    
    original_ref: z.string().optional()
      .describe('Original reference from source system'),
    
    correlation_id: z.string().uuid().optional()
      .describe('Optional correlation ID for tracking related transactions'),
    
    batch_id: z.string().uuid().optional()
      .describe('Batch identifier for bulk processing'),
    
    reconciliation_key: z.string().optional()
      .describe('Key for bank reconciliation matching')
  ...
  lines: z.array(z.any()).max(0, 'Lines must be empty - Auto-Posting Engine will generate based on rules')
    .default([])
    .describe('Must be empty array - APE generates lines from posting rules'),
  
  // POS-specific totals for EOD summaries
  totals: z.object({
    gross_sales: z.number().optional()
      .describe('Total sales before taxes and discounts'),
    
    vat: z.number().optional()
      .describe('Total VAT amount'),
    
    tips: z.number().optional()
      .describe('Total tips amount'),
    
    fees: z.number().optional()
      .describe('Processing fees (card fees, etc.)'),
    
    cash_collected: z.number().optional()
      .describe('Cash payment total'),
    
    card_settlement: z.number().optional()
      .describe('Card payment settlement amount'),
    
    discounts: z.number().optional()
      .describe('Total discount amount'),
    
    commission: z.number().optional()
      .describe('Staff commission amount')
  }).optional().describe('Breakdown totals for POS EOD summaries')
})
...
export function createSalonUFE(
  ...
  return {
    ...
    transaction_type: type === 'EXPENSE' ? FINANCE_TRANSACTION_TYPES.EXPENSE : 
                     type === 'REVENUE' ? FINANCE_TRANSACTION_TYPES.REVENUE :
