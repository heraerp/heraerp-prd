...
import { testDemoAPI, DEMO_ORG_ID, DEMO_USER, calculateDemoWealthOverview } from './demo'
...
export class PWMTestSuite {
  ...
  private async testComponentLoading(): Promise<any> {
    ...
    const avgLoadTime =
      componentTests.reduce((sum, c) => sum + c.loadTime, 0) / componentTests.length
  ...
  private async testUserAuthentication(): Promise<any> {
    ...
    if (DEMO_USER.organization_id.length !== 36) {
      // UUID length
  ...
  private generateTestSummary(): TestResult {
    ...
    return {
      ...
      summary: {
        ...
        criticalIssues: this.testResults.filter(
          t => t.status === 'failed' && ['API Integration', 'Wealth Calculations'].includes(t.name)
        ).length,
        warnings: this.testResults.filter(
          t => t.result?.performanceIssues?.length > 0 || t.result?.slowComponents > 0
        ).length
      }
    }
  }
