...
class DocumentSearchIndex {
  ...
  trackSearchSelection(searchId: string, selectedPath: string) {
    const search = this.analytics.find(a => 
      a.timestamp.toISOString() === searchId
    )
    if (search) {
  ...
  private trackSearch(query: string, userId?: string) {
    ...
    this.popularSearches.set(
      normalizedQuery,
      (this.popularSearches.get(normalizedQuery) || 0) + 1
    )
...
export function extractDocumentSections(markdown: string, path: string, category: string): DocumentSection[] {
