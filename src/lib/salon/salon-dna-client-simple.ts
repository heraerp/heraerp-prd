...
export class SimpleSalonDNAClient {
  ...
  async getServices(): Promise<any[]> {
    try {
      const response = await fetch(`/api/v1/universal?action=read&table=core_entities&organization_id=${this.organizationId}`);
      ...
      // Filter for services
      const services = (result.data || []).filter((entity: any) => 
