...
export async function createIntegratedAppointment(bookingData: BookingData) {
  ...
  try {
    ...
    const isStylistUUID = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(
    ...
    if (appointmentError || !appointment) {
      throw new Error(
        `Failed to create appointment: ${appointmentError?.message || 'Unknown error'}`
      )
    }
    ...
    if (isValidUUID) {
      const { error: lineError } = await supabase.from('universal_transaction_lines').insert({
