...
export const salonQueryKeys = {
  ...
  services: (orgId: string, categoryId?: string) => ['salon', 'services', orgId, categoryId].filter(Boolean),
  ...
  branchFilter: (userId: string, component: string) => ['salon', 'branch-filter', userId, component],
...
export const salonCacheHelpers = {
  ...
  invalidateEntity: (queryClient: QueryClient, entityType: keyof typeof salonQueryKeys, orgId: string) => {
    return queryClient.invalidateQueries({ queryKey: [entityType, orgId] })
  },
  
  // Smart invalidation after mutations
  invalidateAfterMutation: (queryClient: QueryClient, mutationType: string, entityType: string, orgId: string) => {
    switch (mutationType) {
      case 'create':
      case 'update':
      case 'delete':
