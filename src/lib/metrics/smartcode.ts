...
export const SMARTCODE_RE = new RegExp('^HERA\\.[A-Z0-9]{3,15}(?:\\.[A-Z0-9_]{2,30}){3,8}\\.v[0-9]+$');
...
export function validateSmartCode(code: string): { valid: boolean; error?: string } {
  ...
  if (!isSmartCode(code)) {
    return { 
      valid: false, 
      error: 'Invalid smart code format. Expected: HERA.{INDUSTRY}.{MODULE}.{TYPE}.{SUBTYPE}.v{VERSION}' 
    };
  }
