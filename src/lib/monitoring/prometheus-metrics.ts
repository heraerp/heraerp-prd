...
class HERAMetrics extends PrometheusMetrics {
  ...
  trackProgressiveConversion(businessType: string, duration_seconds: number, success: boolean) {
    ...
    this.histogram('hera_conversion_duration_seconds', {
      business_type: businessType
    }, duration_seconds);
  }
  ...
  trackAPICall(endpoint: string, method: string, status_code: number, duration_ms: number) {
    ...
    this.histogram('hera_api_duration_milliseconds', {
      endpoint,
      method
    }, duration_ms);
  }
  ...
  trackProgressiveBehavior(action: string, business_type: string, session_duration: number) {
    ...
    this.gauge('hera_progressive_session_duration_seconds', {
      business_type
    }, session_duration);
  }
   ...
   */
  trackSystemHealth(component: string, status: 'healthy' | 'degraded' | 'down', response_time_ms?: number) {
    this.gauge('hera_system_health', {
      component,
      status
    }, status === 'healthy' ? 1 : 0);
    
    if (response_time_ms !== undefined) {
      this.histogram('hera_system_response_time_milliseconds', {
        component
      }, response_time_ms);
    }
  }
  ...
  trackBusinessValue(metric: string, value: number, business_type: string, currency?: string) {
    this.gauge('hera_business_value', {
