...
export interface CreateTransactionCompleteRequest {
  ...
  // Transaction lines
  lines?: Omit<TransactionLineDataComplete, 'id' | 'transaction_id' | 'created_at' | 'updated_at' | 'version'>[]
...
export class UniversalAPITransactionsComplete extends UniversalAPISacredSix {
  ...
  async createTransactionComplete(request: CreateTransactionCompleteRequest): Promise<any> {
    ...
    return this.execute({
      ...
      ai_requests: request.ai_processing ? {
        enrich: request.ai_processing.auto_classify ? ['transaction_classification', 'risk_analysis'] : [],
   ...
   */
  async queryTransactionsComplete(organizationId: string, filters: TransactionQueryFilters = {}): Promise<any> {
    const queryFilters: any = {}
    
    // Basic field filters
    if (filters.id) queryFilters.id = Array.isArray(filters.id) ? { in: filters.id } : filters.id
    if (filters.transaction_type) queryFilters.transaction_type = Array.isArray(filters.transaction_type) ? { in: filters.transaction_type } : filters.transaction_type
    if (filters.transaction_code) queryFilters.transaction_code = filters.transaction_code
    if (filters.transaction_status) queryFilters.transaction_status = Array.isArray(filters.transaction_status) ? { in: filters.transaction_status } : filters.transaction_status
    ...
    // Entity relationship filters
    if (filters.source_entity_id) queryFilters.source_entity_id = Array.isArray(filters.source_entity_id) ? { in: filters.source_entity_id } : filters.source_entity_id
    if (filters.target_entity_id) queryFilters.target_entity_id = Array.isArray(filters.target_entity_id) ? { in: filters.target_entity_id } : filters.target_entity_id
    ...
    if (filters.approved_by) queryFilters.approved_by = Array.isArray(filters.approved_by) ? { in: filters.approved_by } : filters.approved_by
    
    // Amount filters
    if (filters.total_amount) {
      if (typeof filters.total_amount === 'object') {
        if (filters.total_amount.min !== undefined) queryFilters.total_amount = { '>=': filters.total_amount.min }
        if (filters.total_amount.max !== undefined) queryFilters.total_amount = { ...queryFilters.total_amount, '<=': filters.total_amount.max }
        if (filters.total_amount.equals !== undefined) queryFilters.total_amount = filters.total_amount.equals
      }
    }
    ...
    if (filters.amount_tier) {
      ...
      queryFilters.total_amount = { '>=': tier.min, '<=': tier.max === Infinity ? 999999999 : tier.max }
    }
    
    // Date range filters
    if (filters.transaction_date) {
      if (filters.transaction_date.from) queryFilters.transaction_date = { '>=': filters.transaction_date.from }
      if (filters.transaction_date.to) queryFilters.transaction_date = { ...queryFilters.transaction_date, '<=': filters.transaction_date.to }
    }
    ...
    if (filters.approved_at) {
      ...
      if (filters.approved_at.to) queryFilters.approved_at = { ...queryFilters.approved_at, '<=': filters.approved_at.to }
    }
    
    // Approval filters
    if (filters.approval_required !== undefined) queryFilters.approval_required = filters.approval_required
    ...
    // Fiscal period filters
    if (filters.fiscal_year) queryFilters.fiscal_year = Array.isArray(filters.fiscal_year) ? { in: filters.fiscal_year } : filters.fiscal_year
    if (filters.fiscal_period) queryFilters.fiscal_period = Array.isArray(filters.fiscal_period) ? { in: filters.fiscal_period } : filters.fiscal_period
    if (filters.posting_period_code) queryFilters.posting_period_code = Array.isArray(filters.posting_period_code) ? { in: filters.posting_period_code } : filters.posting_period_code
    
    // Currency filters
    if (filters.base_currency_code) queryFilters.base_currency_code = Array.isArray(filters.base_currency_code) ? { in: filters.base_currency_code } : filters.base_currency_code
    if (filters.transaction_currency_code) queryFilters.transaction_currency_code = Array.isArray(filters.transaction_currency_code) ? { in: filters.transaction_currency_code } : filters.transaction_currency_code
    ...
    // JSONB field queries
    if (filters.business_context_query) queryFilters.business_context = { 'jsonb_contains': filters.business_context_query }
  ...
  async advancedTransactionOperations() {
    return {
      ...
      approveTransaction: async (organizationId: string, transactionId: string, approverId: string, comments?: string): Promise<any> => {
        return this.execute({
          entity: 'universal_transactions',
          organization_id: organizationId,
      ...
      getEntityTransactionHistory: async (organizationId: string, entityId: string, relationshipType?: 'source' | 'target'): Promise<any> => {
        const filters: any = {}
      ...
      assignFiscalPeriod: async (organizationId: string, transactionId: string, fiscalData: { fiscal_year: number; fiscal_period: number; posting_period_code?: string }): Promise<any> => {
        return this.execute({
          entity: 'universal_transactions',
          organization_id: organizationId,
      ...
      getFiscalPeriodTransactions: async (organizationId: string, fiscalYear: number, fiscalPeriod?: number): Promise<any> => {
      ...
      // Currency operations
      updateExchangeRate: async (organizationId: string, transactionId: string, rateData: { exchange_rate: number; exchange_rate_type: string; rate_date?: string }): Promise<any> => {
        return this.execute({
          entity: 'universal_transactions',
          organization_id: organizationId,
      ...
      linkExternalReference: async (organizationId: string, transactionId: string, externalRef: string, system?: string): Promise<any> => {
        return this.execute({
          entity: 'universal_transactions',
          organization_id: organizationId,
      ...
      generateComprehensiveAuditReport: async (organizationId: string, options: {
        ...
        return this.query({
          ...
          query: {
            filters: {
              ...
              transaction_type: options.transaction_types ? { in: options.transaction_types } : undefined
            },
            aggregations: [
              {
      ...
      getTransactionVersionHistory: async (organizationId: string, transactionId: string): Promise<any> => {
        return this.query({
          entity: 'universal_transactions',
          organization_id: organizationId,
      ...
      analyzeTransactionPatterns: async (organizationId: string, entityId?: string, timeframe?: string): Promise<any> => {
        return this.query({
          entity: 'universal_transactions',
          organization_id: organizationId,
  ...
  private determineApprovalRequired(request: CreateTransactionCompleteRequest): boolean {
    ...
    if (request.transaction_config?.require_approval_above && 
  ...
  private async determineFiscalPeriod(transactionDate?: string): Promise<{ fiscal_year: number; fiscal_period: number; posting_period_code: string }> {
    ...
    return { fiscal_year: fiscalYear, fiscal_period: fiscalPeriod, posting_period_code: postingPeriodCode }
  }
  ...
  async bulkCreateTransactionsComplete(organizationId: string, transactions: CreateTransactionCompleteRequest[]): Promise<any> {
    const operations = await Promise.all(
      transactions.map(async (txnRequest, index) => {
        ...
        if (txnRequest.fiscal_config?.auto_assign_period) {
          const fiscalData = await this.determineFiscalPeriod(txnRequest.transaction.transaction_date)
          transactionData.fiscal_year = fiscalData.fiscal_year
          transactionData.fiscal_period = fiscalData.fiscal_period
          transactionData.posting_period_code = fiscalData.posting_period_code
        }
  ...
  async validateTransactionSchema(organizationId: string): Promise<any> {
    return {
      ...
      auditFields: ['created_at', 'updated_at', 'created_by', 'updated_by', 'version', 'approved_by', 'approved_at'],
      entityFields: ['source_entity_id', 'target_entity_id'],
      fiscalFields: ['fiscal_year', 'fiscal_period', 'fiscal_period_entity_id', 'posting_period_code'],
      currencyFields: ['base_currency_code', 'transaction_currency_code', 'exchange_rate', 'exchange_rate_date', 'exchange_rate_type'],
...
export function createUniversalAPITransactionsComplete(config: any): UniversalAPITransactionsComplete {
