...
import { summarizeSalesByBranchDay, buildDailyJournalPayload, postDailySalesJournal } from './dailySales';
  ...
  for (let attempt = 1; attempt <= retryAttempts; attempt++) {
    try {
      ...
      if (attempt < retryAttempts) {
        console.warn(`Attempt ${attempt} failed for ${organization_id}/${branch_id}/${day}, retrying...`, error);
 ...
 */
export async function postDailySalesForAllOrganizations(
  day?: string
): Promise<PostingResult[]> {
  ...
  console.log(`Starting daily posting for ${targetDay} across ${config.organizations.length} organizations`);
  ...
  for (const organizationId of config.organizations) {
    try {
      ...
      for (const branchId of branchIds) {
        ...
        if (result.success) {
          console.log(`✓ Posted daily sales for ${organizationId}/${branchId}/${targetDay}: ${result.total_amount} ${result.transaction_count} txns`);
        } else {
          console.error(`✗ Failed to post daily sales for ${organizationId}/${branchId}/${targetDay}: ${result.error}`);
        }
      }
    } catch (error) {
  ...
  console.log(`Daily posting completed for ${targetDay}: ${successful} successful, ${failed} failed, total ${totalAmount.toFixed(2)}`);
