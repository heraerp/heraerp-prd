...
const salonTheme: SalonThemeContextType = {
  ...
  getStatusClass: (status: string) => {
    const statusClasses: Record<string, string> = {
      pending:
        'bg-amber-100 dark:bg-amber-950/30 text-amber-900 dark:text-amber-100 border border-amber-300 dark:border-amber-800',
      confirmed:
        'bg-blue-100 dark:bg-blue-950/30 text-blue-900 dark:text-blue-100 border border-blue-300 dark:border-blue-800',
      arrived:
        'bg-purple-100 dark:bg-purple-950/30 text-purple-900 dark:text-purple-100 border border-purple-300 dark:border-purple-800',
      in_service:
        'bg-pink-100 dark:bg-pink-950/30 text-pink-900 dark:text-pink-100 border border-pink-300 dark:border-pink-800',
      in_progress:
        'bg-pink-100 dark:bg-pink-950/30 text-pink-900 dark:text-pink-100 border border-pink-300 dark:border-pink-800',
      completed:
        'bg-green-100 dark:bg-green-950/30 text-green-900 dark:text-green-100 border border-green-300 dark:border-green-800',
      cancelled:
        'bg-gray-100 dark:bg-gray-950/30 text-gray-900 dark:text-gray-100 border border-gray-300 dark:border-gray-800',
      no_show:
        'bg-red-100 dark:bg-red-950/30 text-red-900 dark:text-red-100 border border-red-300 dark:border-red-800'
    }
...
export function SalonThemeProvider({ children }: { children: React.ReactNode }) {
  return <SalonThemeContext.Provider value={salonTheme}>{children}</SalonThemeContext.Provider>
}
