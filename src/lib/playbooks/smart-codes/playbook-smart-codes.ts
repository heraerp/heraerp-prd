...
export class PlaybookSmartCodeService {
   ...
   */
  generateContractCode(contractType: 'input' | 'output' | 'step_input' | 'step_output', version = '1'): string {
    const patterns = {
   ...
   */
  generatePolicyCode(policyType: 'sla' | 'quorum' | 'segregation' | 'approval' | 'retry', version = '1'): string {
  ...
  validateSmartCode(smartCode: string): SmartCodeValidationResult {
    ...
    if (parts.length > 1) {
      ...
      if (!Object.values(INDUSTRY_CODES).includes(industry as any)) {
        suggestions.push(`Consider using standard industry codes: ${Object.values(INDUSTRY_CODES).join(', ')}`);
      }
    }
  ...
  private validatePlaybookPattern(parts: string[]): { valid: boolean; errors: string[]; suggestions: string[] } {
    const errors: string[] = [];
    const suggestions: string[] = [];
...
export const PlaybookSmartCodes = {
  ...
  forPlaybookDefinition: (industry: string, name: string, version = '1') =>
    playbookSmartCodeService.generatePlaybookDefinitionCode({ industry, module: 'PLAYBOOK', name, version }),
    
  forStepDefinition: (industry: string, stepName: string, version = '1') =>
    playbookSmartCodeService.generateStepDefinitionCode({ industry, module: 'PLAYBOOK', stepName, version }),
    
  forPlaybookRun: (industry: string, name: string, version = '1') =>
    playbookSmartCodeService.generatePlaybookRunCode({ industry, module: 'PLAYBOOK', name, version }),
    
  forStepExecution: (industry: string, stepName: string, version = '1') =>
    playbookSmartCodeService.generateStepExecutionCode({ industry, module: 'PLAYBOOK', stepName, version }),
