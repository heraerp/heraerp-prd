...
export class GrantsIntakeOrchestrator extends PlaybookOrchestrator {
  ...
  private async processApplicationRegistration(
    ...
    try {
      ...
      const totalPreviousFunding = previousGrants.data?.reduce(
        (sum, grant) => sum + (grant.total_amount || 0), 0
      ) || 0;
  ...
  private async processEligibilityScreening(
    ...
    try {
      ...
      // Amount limits check
      const amountInRange = inputs.amount_requested >= eligibilityRules.min_amount && 
      ...
      if (amountInRange) {
        eligibilityReasons.push(`Amount within limits: $${inputs.amount_requested.toLocaleString()} ≤ $${eligibilityRules.max_amount.toLocaleString()} ✓`);
      } else {
        eligibilityReasons.push(`Amount outside limits: $${inputs.amount_requested.toLocaleString()} (min: $${eligibilityRules.min_amount.toLocaleString()}, max: $${eligibilityRules.max_amount.toLocaleString()}) ✗`);
        allPassed = false;
      }
      
      // Organization type check
      const orgTypeEligible = eligibilityRules.organization_types.includes(inputs.applicant_profile.type);
      ...
      if (orgTypeEligible) {
        eligibilityReasons.push(`Organization type '${inputs.applicant_profile.type}' is eligible ✓`);
      } else {
        eligibilityReasons.push(`Organization type '${inputs.applicant_profile.type}' not eligible ✗`);
        allPassed = false;
      }
  ...
  private async processCommitteeReview(
    ...
    try {
      ...
      const consensusScore = simulatedVotes.reduce((sum, vote) => sum + vote.score, 0) / simulatedVotes.length;
  ...
  private async processAwardDecision(
    ...
    try {
      ...
      const shouldAward = finalScore >= approvalThreshold && 
  ...
  private calculateInnovationScore(inputs: any): number {
    ...
    const innovativeKeywords = ['ai', 'machine learning', 'blockchain', 'iot', 'vr', 'ar', 'quantum'];
    const description = (inputs.project_description || '').toLowerCase();
    const keywordMatches = innovativeKeywords.filter(keyword => description.includes(keyword)).length;
  ...
  private assessProposalRisks(inputs: any, criteria: Record<string, any>): Record<string, string> {
    return {
      technical_risk: criteria.feasibility?.score > 80 ? 'low' : criteria.feasibility?.score > 60 ? 'medium' : 'high',
  ...
  private generateScoringRecommendations(criteria: Record<string, any>, risks: Record<string, string>): string[] {
