...
export class ContractValidationService {
   ...
   */
  async validatePlaybookInput(playbookId: string, inputData: any): Promise<ValidationResult> {
   ...
   */
  async validatePlaybookOutput(playbookId: string, outputData: any): Promise<ValidationResult> {
    const startTime = Date.now()
   ...
   */
  async validateStepInput(stepId: string, inputData: any): Promise<ValidationResult> {
    const startTime = Date.now()
   ...
   */
  async validateStepOutput(stepId: string, outputData: any): Promise<ValidationResult> {
    const startTime = Date.now()
      ...
      if (context.stepName) {
        ...
        if (stepSLA) {
          ...
          if (stepSLA.business_hours_only && context.businessHoursOnly) {
            allowedMinutes = this.adjustForBusinessHours(
              allowedMinutes,
              context.startTime,
              currentTime
            )
          ...
          return {
            ...
            severity: compliant
              ? 'info'
              : elapsedMinutes > allowedMinutes * 1.5
                ? 'critical'
                : 'warning',
            metrics: {
  ...
  async validateSegregationPolicy(
    ...
    try {
      ...
      if (stepRules.requires_different_user_from && context.previousStepUsers) {
        ...
        const conflictingSteps = context.previousStepUsers.filter(
          prevStep =>
      ...
      if (stepRules.prohibited_combinations && context.previousStepUsers) {
        for (const combination of stepRules.prohibited_combinations) {
          ...
          const hasRole1InHistory = context.previousStepUsers.some(
            prevStep => prevStep.userId === context.currentUserId && prevStep.roles.includes(role1)
          )
      ...
      if (context.relationshipChecks) {
        ...
        if (prohibitedConflicts) {
          const detectedProhibitions = context.relationshipChecks.filter(
            check => check.detected && prohibitedConflicts[check.type] === 'prohibited'
          )
          ...
          switch (rule) {
            case 'budget_total_equals_amount':
              if (data.budget_breakdown && data.amount_requested) {
                const total = Object.values(data.budget_breakdown).reduce(
            ...
            case 'project_duration_reasonable':
              return (
                ...
                (data.project_duration_months >= 6 && data.project_duration_months <= 36)
              )
