...
export class PermissionService {
  ...
  async checkPermission(
    ...
    if (context) {
      return this.checkContextualPermission(userId, permission, context, userPermissions)
    }
...
export class IdempotencyService {
   ...
   */
  private async findRecord(key: string, endpoint: string): Promise<any | null> {
...
export class SecureQueryBuilder {
   ...
   */
  buildEntityQuery(entityType: string, additionalFilters?: Record<string, any>): any {
    const query = {
   ...
   */
  buildTransactionQuery(transactionType: string, additionalFilters?: Record<string, any>): any {
    const query = {
  ...
  return {
    ...
    requirePermission: (...permissions: string[]) => {
      return async (req: NextRequest, context: any) => {
        ...
        try {
          await permissionService.enforcePermissions(userId, permissions, {
    ...
    validateRequest: (schema: z.ZodSchema) => {
      return async (req: NextRequest, context: any) => {
        try {
          ...
          throw new SecurityError('INVALID_REQUEST', 'Request validation failed', 400)
        }
      }
    },
    ...
    auditLog: (action: string) => {
      return async (req: NextRequest, context: any, response: any) => {
        await auditService.logAction(action, context.userId, context.resourceId, {
          method: req.method,
          path: req.nextUrl.pathname,
