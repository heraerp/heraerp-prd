...
export class PlaybookAIService {
  ...
  async summarizeRunProgress(runId: string): Promise<RunSummary> {
    ...
    try {
      ...
      const recommendations = await this.generateRunRecommendations(
        run,
        stepData,
        anomalies
      );
  ...
  async detectRunAnomalies(
    ...
    const avgStepDuration = await this.getAverageStepDuration(
    ...
    if (failureRate > 0.2) {
      anomalies.push({
        ...
        metrics: {
          ...
          failed_steps: steps.filter(s => s.metadata?.status === 'failed')
  ...
  private calculateWeightedConfidence(confidences: number[]): number {
    ...
    const weights = confidences.map((_, i) => 
      Math.pow(0.9, confidences.length - i - 1)
    );
    const weightSum = weights.reduce((a, b) => a + b, 0);
    
    return confidences.reduce(
      (sum, conf, i) => sum + conf * weights[i], 0
    ) / weightSum;
  }
  ...
  private estimateCompletion(run: any, steps: any[]): Date | undefined {
    const inProgressSteps = steps.filter(s => 
      ['pending', 'running'].includes(s.metadata?.status)
    );
  ...
  private checkStalledRun(run: any): ProactiveNudge | null {
    const lastActivity = new Date(
      run.metadata?.last_activity_at || run.created_at
    );
  ...
  private async detectInputAnomalies(
    run: any,
    steps: any[]
  ): Promise<AIAnomaly[]> {
...
class MockAIProvider implements AIProvider {
  async analyze(request: any): Promise<AIAnalysis> {
    return {
      ...
      insights: 'Analysis indicates normal execution patterns with minor optimization opportunities.',
      model: 'mock-ai-v1',
      version: '1.0.0',
