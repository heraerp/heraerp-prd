...
export class PlaybookOrchestratorDaemon {
  ...
  private async processOrchestrationCycle(): Promise<void> {
    ...
    // Log status
    console.log(`Orchestration cycle: ${this.activeRuns.size} active runs, ${pendingSteps.length} pending steps, ${readySteps.length} ready steps, ${this.processingSteps.size} processing`);
  }
  ...
  private async executeReadySteps(readySteps: PendingStepLine[]): Promise<void> {
    ...
    for (const step of readySteps) {
      ...
      const stepsInRunBeingProcessed = Array.from(this.processingSteps)
        .filter(stepId => {
  ...
  private async handleStepFailure(
    ...
    try {
      ...
      if (canRetry) {
        // Schedule retry
        const retryDelay = this.config.retry_delays_ms[Math.min(newRetryCount - 1, this.config.retry_delays_ms.length - 1)];
        ...
        console.log(`Step scheduled for retry (${newRetryCount}/${step.max_retries}): ${step.step_name}`);
  ...
  private async checkStepDependencies(step: PendingStepLine): Promise<boolean> {
    ...
    try {
      ...
      for (const dependency of step.dependencies) {
        const dependentStep = stepLinesResult.data.find(line => 
          line.line_number === dependency.step_number
        );
  ...
  private async cleanupCompletedRuns(): Promise<void> {
    const completedRuns = Array.from(this.activeRuns.entries())
      .filter(([_, runContext]) => 
        runContext.status === 'completed' || runContext.status === 'failed'
      );
