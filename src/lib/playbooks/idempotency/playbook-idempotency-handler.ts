...
export class PlaybookIdempotencyHandler {
   ...
   */
  async markOperationCompleted(recordId: string, result: any): Promise<void> {
    try {
   ...
   */
  async markOperationFailed(recordId: string, error: any): Promise<void> {
    try {
      await universalApi.updateTransaction(recordId, {
        metadata: {
          status: 'failed',
   ...
   */
  async cleanupExpiredRecords(organizationId: string, expirationDays: number = 7): Promise<number> {
    try {
      ...
      keyGroups.forEach((group, key) => {
        ...
        group.forEach(record => {
          ...
          if (
            ...
            const duration =
              new Date(record.metadata.completed_at).getTime() -
