...
export class PlaybookOrchestrator {
  ...
  private async processPendingSteps(): Promise<void> {
    ...
    try {
      ...
      for (const step of pendingSteps) {
        ...
        this.processStep(step).catch(error => this.handleStepError(step, error))
      }
    } catch (error) {
   ...
   */
  private async completeStep(stepId: string, completion: StepCompletion): Promise<void> {
  ...
  private async handleStepFailure(
    ...
    if (attempt < retryPolicy.max_attempts) {
      ...
      console.log(
        `[Orchestrator] Scheduling retry ${attempt}/${retryPolicy.max_attempts} in ${backoff}s`
      )
    } else {
      // Fail step
  ...
  private async updateRunProgress(runId: string): Promise<void> {
    ...
    const completed = lines.filter(l => l.metadata?.status === 'completed').length

    const failed = lines.filter(l => l.metadata?.status === 'failed').length
  ...
  private async updateStepMetadata(stepId: string, metadata: Record<string, any>): Promise<void> {
    const step = await universalApi.getTransactionLine(stepId)
    await universalApi.updateTransactionLine(stepId, {
      metadata: {
        ...step.metadata,
  ...
  private async validateOutputs(step: StepExecutionLine, outputs: any): Promise<void> {
  ...
  private async buildAIPrompt(step: StepExecutionLine, inputs: any): Promise<string> {
...
`
  ...
  private async createHumanTask(step: StepExecutionLine, inputs: any): Promise<any> {
    const stepDef = await universalApi.getEntity(step.line_entity_id)
  ...
  private async getPreviousSteps(step: StepExecutionLine): Promise<any[]> {
    ...
    return lines.filter(l => (l.metadata?.sequence || 0) < sequence)
  }

  private async getStepBySequence(runId: string, sequence: number): Promise<any | null> {
    const lines = await universalApi.queryTransactionLines({
      transaction_id: runId
    })
  ...
  private async findDependentSteps(runId: string, completedSequence: number): Promise<any[]> {
    const lines = await universalApi.queryTransactionLines({
      transaction_id: runId
    })
