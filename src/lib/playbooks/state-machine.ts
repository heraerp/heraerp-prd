...
export class PlaybookStateMachine {
    ...
    rules.set('playbook', [
      {
        ...
        postActions: [this.notifyPlaybookPublished, this.createVersionRelationship]
      },
      {
        fromState: 'published',
        toState: 'archived',
        smartCode: 'HERA.PLAYBOOK.TRANSITION.ARCHIVE.V1',
        validations: [this.validateNoActiveRuns],
        postActions: [this.cancelActiveRuns, this.notifyPlaybookArchived]
      }
    ])
    ...
    rules.set('run', [
      {
        ...
        postActions: [this.initializeFirstStep, this.notifyRunStarted]
      },
      {
        ...
        validations: [],
        postActions: [this.startBlockedTimer, this.sendEscalationNotification]
      },
      {
        fromState: 'in_progress',
        toState: 'completed',
        smartCode: 'HERA.PLAYBOOK.RUN.TRANSITION.COMPLETE.V1',
        validations: [this.validateAllStepsComplete, this.validateOutputContract],
        postActions: [this.calculateTotalCost, this.triggerDownstreamWorkflows]
      },
      {
        fromState: 'in_progress',
        toState: 'failed',
        ...
        validations: [],
        postActions: [this.executeFailureHandlers, this.notifyRunFailed]
      }
    ])
    ...
    rules.set('step', [
      {
        ...
        postActions: [this.assignWorker, this.lockResources, this.startExecutionTimer]
      },
      {
        fromState: 'running',
        ...
        validations: [],
        postActions: [this.createInputRequest, this.pauseExecutionTimer]
      },
      {
        fromState: 'running',
        toState: 'completed',
        smartCode: 'HERA.PLAYBOOK.STEP.TRANSITION.COMPLETE.V1',
        validations: [this.validateStepOutputs, this.validateSideEffects],
        postActions: [this.updateRunProgress, this.triggerNextSteps]
      },
      {
        fromState: 'running',
        toState: 'failed',
        ...
        validations: [],
        postActions: [this.checkRetryPolicy, this.notifyStepFailed]
      }
    ])
  ...
  async canTransition(
    ...
    const rule = rules.find(r => r.fromState === currentState && r.toState === toState)
  ...
  async transition(
    ...
    // Validate transition
    const validation = await this.canTransition(entityType, entityId, toState, context)
  ...
  private async notifyPlaybookPublished(
    entityId: string,
    context: TransitionContext
  ): Promise<void> {
    // Send notifications
  ...
  private async createVersionRelationship(
    entityId: string,
    context: TransitionContext
  ): Promise<void> {
  ...
  private async notifyPlaybookArchived(
    entityId: string,
    context: TransitionContext
  ): Promise<void> {
    // Send notifications
  ...
  private async triggerDownstreamWorkflows(
    entityId: string,
    context: TransitionContext
  ): Promise<void> {
