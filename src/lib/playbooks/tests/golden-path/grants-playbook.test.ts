...
const mockUniversalApi = {
  ...
  setDynamicField: async (entityId: string, field: string, value: any) => ({
    data: { success: true },
    error: null
  })
}
...
async function updateRunStep(stepId: string, status: StepStatus, outputData?: any): Promise<void> {
  const updates: any = {
    status,
  ...
  await supabase.from('playbook_run_steps').update(updates).eq('id', stepId)
}

async function updateRunStatus(runId: string, status: RunStatus, outputData?: any): Promise<void> {
  const updates: any = {
    status,
    updated_at: new Date().toISOString()
  }
  ...
  await supabase.from('playbook_runs').update(updates).eq('id', runId)
}
