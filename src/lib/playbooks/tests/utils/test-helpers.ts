...
export function createTestContract(
  ...
  return {
    ...
    examples: [
      type === 'input' ? { value: 'test input' } : { result: 'test output', success: true }
    ]
  }
}
 ...
 */
export function createTestPolicy(type: PolicyType, overrides?: Partial<StepPolicy>): StepPolicy {
  ...
  switch (type) {
    ...
    case 'validation':
      return {
        ...basePolicy,
        config: {
          rules: [{ field: 'amount', operator: 'lt', value: 10000 }],
          ...overrides?.config
        }
      }
...
export function advanceRunToStep(
  ...
  if (completedSteps) {
    updatedRun.stepExecutions = [...run.stepExecutions, ...completedSteps]
  }
...
export function assertRunStatus(
  ...
  if (run.status !== expectedStatus) {
    throw new Error(
      message || `Expected run status to be '${expectedStatus}', but got '${run.status}'`
    )
  }
}
 ...
 */
export function assertContractValid(data: any, contract: StepContract, message?: string): void {
  ...
  for (const field of required) {
    if (!(field in data)) {
      throw new Error(message || `Contract validation failed: missing required field '${field}'`)
    }
  }
  ...
  switch (policy.type) {
    ...
    case 'approval':
      if (execution.status === 'completed' && !execution.output?.approved) {
        throw new Error(message || 'Approval policy violated: step completed without approval')
      }
      break
...
export function assertSignalHandled(
  ...
  if (expectedOutcome?.hasOutput && !run.output) {
    throw new Error(
      message || `Signal '${signalType}' handling failed: expected output to be present`
    )
  }
}
 ...
 */
export async function resetDatabase(options?: { resetFn?: () => Promise<void> }): Promise<void> {
