...
class PlaybookTestRunner {
  ...
  private async generateReports(): Promise<void> {
    ...
    for (const result of this.results) {
      ...
      console.log(
        `       Tests: ${result.tests}, Failures: ${result.failures}, Duration: ${this.formatDuration(result.duration)}`
      )
...
<body>
  ...
  private async cleanupTestEnvironment(): Promise<void> {
    ...
    const tempFiles = [path.join(__dirname, 'test-results', 'jest-results.json')]
  ...
  private calculateOverallStatus(): boolean {
    ...
    const requiredResults = this.results.filter(r => requiredSuites.some(s => s.name === r.suite))
