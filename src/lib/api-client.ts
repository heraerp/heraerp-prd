...
// Organizations API
export const orgApi = {
  async listOrganizations(params: { q?: string; type?: string; stage?: string; rm?: string; tag?: string; limit?: number; cursor?: string; orgId?: OrgId }): Promise<Paginated<any>> {
    const { orgId, ...queryParams } = params;
    const query = new URLSearchParams();
  ...
  async logActivity(orgId: string, entityId: string, activity: any): Promise<void> {
    const smartCode = activity.type === 'email' ? 'HERA.CRM.ACTIVITY.EMAIL.SENT.v1'
      : activity.type === 'call' ? 'HERA.CRM.ACTIVITY.CALL.LOG.v1'
      : activity.type === 'meeting' ? 'HERA.CRM.ACTIVITY.MEETING.HELD.v1'
...
const crmPrograms = {
  ...
  async createGrantRound(programId: string, data: CreateGrantRoundRequest): Promise<GrantRoundLite> {
