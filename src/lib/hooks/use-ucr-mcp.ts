...
export function useUCRMCP() {
  ...
  const callTool = useCallback(async (tool: string, args: any = {}) => {
  ...
  const listTemplates = useCallback(async (industry?: string, module?: string) => {
    const result = await callTool('list_templates', { industry, module })
    return result.templates as UCRTemplate[]
  }, [callTool])

  const cloneTemplate = useCallback(async (templateId: string, targetSmartCode: string) => {
  ...
  const searchRules = useCallback(async (
    query: string,
    tags?: string[],
  ...
  const getAuditLog = useCallback(async (objectRef: string, from?: string, to?: string) => {
