...
export function useFactoryDashboard() {
  ...
  const client = isMockMode ? MockAdapter : createClient({
    baseUrl: process.env.NEXT_PUBLIC_API_BASE || '',
    token: process.env.NEXT_PUBLIC_API_TOKEN,
  });

  // Fetch transactions
  const { data: transactions = [], error: txnError, mutate: mutateTxns } = useSWR(
  ...
  // Fetch modules
  const { data: modules = [], error: modError, mutate: mutateModules } = useSWR(
  ...
  const { data: allLines = [], error: linesError } = useSWR(
    ['transaction-lines', transactions.map(t => t.id)],
    async () => {
  ...
  const transactionLines = new Map<string, UniversalTransactionLine[]>();
  allLines.forEach(line => {
    if (!transactionLines.has(line.transaction_id)) {
      transactionLines.set(line.transaction_id, []);
    }
    transactionLines.get(line.transaction_id)!.push(line);
  });

  // Calculate KPIs
  const kpis = calculateModuleKPIs(transactions, allLines, fiscalPeriods);
...
export function useModuleDetails(moduleId: string) {
  const { data, actions } = useFactoryDashboard();
  
  const module = data.modules.find(m => m.id === moduleId);
  const moduleTransactions = data.transactions.filter(t => 
    t.smart_code === module?.smart_code
  );
  ...
  const dependencies = data.relationships.filter(r => 
    r.from_entity_id === moduleId && r.relationship_type === 'DEPENDS_ON'
  );

  const dependents = data.relationships.filter(r => 
    r.to_entity_id === moduleId && r.relationship_type === 'DEPENDS_ON'
  );
