...
export async function updateOpportunityOwner(input: {
  ...
  return await insertAudit(
    ...
    input.actor_user_id,
    {
      event: 'OPPORTUNITY_OWNER_CHANGED',
      opportunity_id: input.opportunity_id,
...
export async function updateOpportunityProbability(input: {
  orgId: string
  smart_code: string
  idempotency_key: string
  actor_user_id: string
  ...
  return await insertAudit(
    ...
    input.actor_user_id,
    {
      event: 'OPPORTUNITY_PROBABILITY_CHANGED',
      opportunity_id: input.opportunity_id,
...
export async function createLead(input: {
  ...
  await insertAudit(input.orgId, input.smart_code, input.idempotency_key, input.actor_user_id, {
    event: 'LEAD_CREATED',
    lead_id: leadId
  })
  return { id: leadId }
...
export async function createAccount(input: {
  ...
  if (input.revenue !== undefined)
    await upsertDynamic(input.orgId, accountId, 'revenue', input.revenue)
  if (input.employees !== undefined)
    await upsertDynamic(input.orgId, accountId, 'employees', input.employees)
...
export async function updateAccount(input: {
  ...
  if (input.account_name) {
    await supabase
      .from('core_entities')
      .update({ entity_name: input.account_name })
      .eq('id', input.account_id)
  }
  if (input.industry !== undefined)
    await upsertDynamic(input.orgId, input.account_id, 'industry', input.industry)
  if (input.segment !== undefined)
    await upsertDynamic(input.orgId, input.account_id, 'segment', input.segment)
  if (input.website !== undefined)
    await upsertDynamic(input.orgId, input.account_id, 'website', input.website)
  if (input.revenue !== undefined)
    await upsertDynamic(input.orgId, input.account_id, 'revenue', input.revenue)
  if (input.employees !== undefined)
    await upsertDynamic(input.orgId, input.account_id, 'employees', input.employees)
  if (input.status !== undefined)
    await upsertDynamic(input.orgId, input.account_id, 'status', input.status)
  return await insertAudit(
    input.orgId,
    input.smart_code,
    input.idempotency_key,
...
export async function qualifyLead(input: {
  ...
  const { data: opp, error } = await supabase
    ...
    .insert({
      ...
      entity_name: input.opportunity_name,
      smart_code: input.smart_code
    })
    .select()
    .single()
