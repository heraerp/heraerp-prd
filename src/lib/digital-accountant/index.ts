...
export class DigitalAccountantService implements IDigitalAccountantService {
  ...
  constructor(
    ...
    // Initialize services
    this.validationService = repositories?.validation || new ValidationService(organizationId, userId);
    ...
    this.analyticsIntegration = new AccountingAnalyticsIntegration(
      this,
      {
        organizationId,
        userId,
  ...
  async postJournal(journalId: string, options?: any): Promise<AccountingToolResponse> {
    try {
      ...
      if (!validation.is_valid && (!options?.validation_override || options.validation_override.length === 0)) {
        return {
          success: false,
   ...
   */
  async startReconciliation(glAccountId: string, statementDate: string): Promise<ReconciliationSession> {
    try {
      // Get GL account
  ...
  async performAutoMatch(sessionId: string): Promise<any> {
    try {
      ...
      const { data: transactions, error: txError } = await supabase
        ...
        .select('*')
        .eq('organization_id', this.organizationId)
        .or(`source_entity_id.eq.${session.metadata.gl_account_id},target_entity_id.eq.${session.metadata.gl_account_id}`)
