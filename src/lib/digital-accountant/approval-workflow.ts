...
export const STANDARD_APPROVAL_RULES: ApprovalRule[] = [
  {
    ...
    conditions: [
      { field: 'metadata.total_debits', operator: 'less_than', value: 1000 }
    ],
    workflow_steps: [
      {
        step_number: 1,
  ...
  },
  {
    ...
    entity_type: 'journal_entry',
    conditions: [
      { field: 'metadata.total_debits', operator: 'greater_than', value: 10000 }
    ],
    workflow_steps: [
      {
        step_number: 1,
      ...
      },
      {
        ...
        skip_conditions: [
          { field: 'metadata.total_debits', operator: 'less_than', value: 50000 }
        ]
      },
      {
        step_number: 3,
        ...
        skip_conditions: [
          { field: 'metadata.total_debits', operator: 'less_than', value: 100000 }
        ]
      }
    ],
    is_active: true,
  ...
  },
  {
    ...
    entity_type: 'journal_entry',
    conditions: [
      { field: 'smart_code', operator: 'contains', value: '.ADJUST.' }
    ],
    workflow_steps: [
      {
        step_number: 1,
...
export class ApprovalWorkflowService {
  ...
  private async findMatchingRule(entity: any, entityType: string): Promise<ApprovalRule | null> {
    ...
    const rules = STANDARD_APPROVAL_RULES
      .filter(rule => rule.entity_type === entityType && rule.is_active)
  ...
  private async evaluateRuleConditions(conditions: ApprovalCondition[], entity: any): Promise<boolean> {
    let result = true;
  ...
  private async initializeWorkflowSteps(
    ...
    for (const stepTemplate of rule.workflow_steps) {
      ...
      if (stepTemplate.skip_conditions &&
