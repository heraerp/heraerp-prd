...
const INTENT_PATTERNS: Record<AccountingIntent, RegExp[]> = {
  ...
  period_status: [/period.*status/i, /close.*period/i, /open.*periods/i, /accounting.*period/i],
  compliance_check: [
    /compliance.*check/i,
    /audit.*trail/i,
    /validation.*errors/i,
  ...
  general_help: [/help/i, /what can.*do/i, /how.*work/i, /explain/i]
}
...
export class AccountingAnalyticsIntegration {
  ...
  constructor(accountantService: IDigitalAccountantService, context: AccountingChatContext) {
...
Would you like to see recent transactions for this account?`
  ...
  private async handleReportGeneration(query: string): Promise<string> {
    ...
    else if (query.includes('income statement') || query.includes('p&l'))
    ...
    if (periodMatch) {
      ...
      period_start = new Date(currentDate.getFullYear(), currentDate.getMonth(), 1)
        .toISOString()
        .split('T')[0]
...
Try asking a specific question about your accounting data!`
  ...
  private async calculateAccountBalance(accountId: string): Promise<number> {
    ...
    lines.forEach(line => {
      balance +=
        ((line.metadata as any)?.debit_amount || 0) - ((line.metadata as any)?.credit_amount || 0)
    })
...
Ensure debits equal credits. Output as JSON.`
      ...
      if (result.lines && result.description) {
        // Calculate totals
        result.totalDebits = result.lines.reduce(
          (sum: number, line: any) => sum + (line.debit || 0),
          0
        )
        result.totalCredits = result.lines.reduce(
