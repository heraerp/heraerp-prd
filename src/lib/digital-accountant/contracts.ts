...
export interface IJournalRepository {
  ...
  updateJournalMetadata(id: string, metadata: Partial<JournalEntry['metadata']>, organizationId: string): Promise<void>;
  
  // Posting operations
  postJournal(id: string, organizationId: string, validationOverrides?: string[]): Promise<AccountingToolResponse>;
  reverseJournal(id: string, organizationId: string, reversalDate: string, reason: string): Promise<JournalEntry>;
...
export interface IReconciliationRepository {
  ...
  updateSession(sessionId: string, updates: Partial<ReconciliationSession>, organizationId: string): Promise<void>;
  
  // Item matching
  addItems(sessionId: string, items: ReconciliationItem[]): Promise<void>;
  matchItems(sessionId: string, sourceId: string, targetId: string, confidence: number): Promise<void>;
  unmatchItem(sessionId: string, itemId: string): Promise<void>;
  
  // Auto-matching
  autoMatchItems(sessionId: string, threshold: number): Promise<{ matched: number; unmatched: number }>;
...
export interface IReportingRepository {
  // Report generation
  generateReport(reportType: string, organizationId: string, periodStart: string, periodEnd: string): Promise<FinancialReport>;
  ...
  getTransactionSummary(organizationId: string, periodStart: string, periodEnd: string): Promise<any>;
}
...
export interface IAuditRepository {
  ...
  getAuditByUser(userId: string, organizationId: string, startDate: string, endDate: string): Promise<AuditEntry[]>;
