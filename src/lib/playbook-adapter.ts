...
export async function runPlaybook<T = any>(
  ...
  const idempotencyKey =
    ...
    createHash('sha256')
      .update(
        `${normalizedSmartCode}-${JSON.stringify(payload)}-${opts.organizationId || 'default'}`
      )
      .digest('hex')
      .substring(0, 16)
    ...
    if (normalizedSmartCode === 'HERA.UNIV.TXN.HEADER.CREATE.V1') {
      try {
        ...
        if (header_smart_code.includes('.POS.CART.')) {
          ...
          header_smart_code.includes('.APT.') ||
          header_smart_code.includes('.APPOINTMENT.')
        ) {
      ...
      try {
        ...
        if (idempotencyKey) {
          ...
          if (existingExecution?.metadata?.result) {
            console.log(
              '[PLAYBOOK] Returning cached line add result for idempotency key:',
              idempotencyKey
            )
        ...
        if (lineError) {
          ...
          if (idempotencyKey) {
            const { error: logError } = await supabase.from('universal_transactions').insert({
              organization_id,
      ...
      try {
        ...
        if (configEntity && !error) {
          ...
          const { data: dynamicData, error: dynamicError } = await supabase
            ...
            .select(
              'field_name, field_value_text, field_value_number, field_value_boolean, field_value_json'
            )
            .eq('organization_id', organization_id)
      ...
      try {
        ...
        if (configEntity && !error) {
          ...
          if (!error) {
            ...
            const { error: auditError } = await supabase.from('universal_transactions').insert({
              organization_id,
...
export const salonPlaybooks = {
  appointments: {
    ...
    confirm: (appointmentId: string, opts?: PlaybookOptions) =>
      runPlaybook<void>(
        'HERA.SALON.APPOINTMENT.CONFIRM.V1',
  ...
  pos: {
    ...
    addItem: (cartId: string, serviceId: string, quantity: number = 1, opts?: PlaybookOptions) =>
      runPlaybook<any>(
        'HERA.SALON.POS.CART.ADD_ITEM.V1',
        {
          cart_id: cartId,
  ...
  whatsapp: {
    sendAppointmentReminder: (appointmentId: string, opts?: PlaybookOptions) =>
      runPlaybook<void>(
        'HERA.SALON.COMMS.WHATSAPP.APPOINTMENT_REMINDER.V1',
        {
          appointment_id: appointmentId
        },
        opts
    ...
    sendReceipt: (saleId: string, opts?: PlaybookOptions) =>
      runPlaybook<void>(
        'HERA.SALON.COMMS.WHATSAPP.SEND_RECEIPT.V1',
        {
          sale_id: saleId
        },
        opts
