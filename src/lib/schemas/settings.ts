...
export const SalesPolicy = z.object({
  vat_rate: z.number().min(0, 'VAT rate cannot be negative').max(1, 'VAT rate cannot exceed 100%').default(0.05),
  commission_rate: z.number().min(0, 'Commission rate cannot be negative').max(1, 'Commission rate cannot exceed 100%').default(0.35),
  ...
  payment_methods: z.array(z.enum(['cash', 'card', 'digital_wallet', 'bank_transfer'])).default(['cash', 'card']),
...
export const Branch = z.object({
  ...
  entity_code: z.string().min(2, 'Branch code must be at least 2 characters').regex(/^[A-Z0-9_]+$/, 'Branch code must be uppercase alphanumeric with underscores'),
...
export const NotificationPolicy = z.object({
  ...
  appointment_confirmed: z.object({
  ...
  low_stock_alert: z.object({
    enabled: z.boolean().default(true),
    channels: z.array(z.enum(['email', 'sms', 'whatsapp', 'in_app'])).default(['email', 'in_app']),
    threshold_quantity: z.number().min(0).default(5),
    recipients: z.array(z.string().email()).optional()
  }).default({}),
  
  // Daily reports
  end_of_day_report: z.object({
    enabled: z.boolean().default(true),
    channels: z.array(z.enum(['email', 'sms', 'whatsapp', 'in_app'])).default(['email']),
  ...
  // Marketing
  promotional_campaigns: z.object({
...
export const SETTINGS_DYNAMIC_DATA_KEYS = {
  ...
  NOTIFICATION_POLICY: 'NOTIFICATIONS_POLICY.v1',
