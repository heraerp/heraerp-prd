...
export class LeadScoringEngine {
  ...
  async calculateLeadScore(email: string): Promise<LeadScore> {
    try {
      ...
      const totalScore = Math.min(100, Math.round(
        ...
        (frequencyScore * 0.10)         // 10% weight on frequency
      ))
      ...
      const recommendedAction = this.getRecommendedAction(leadGrade, qualificationStatus, engagements)
  ...
  private async calculateBehavioralScore(email: string, engagements: any[]): Promise<number> {
    ...
    const engagementDays = new Set(
      engagements.map(e => new Date(e.timestamp).toDateString())
    ).size
    
    if (engagementDays >= 7) score += 20      // Very consistent
    ...
    if (opens > 0) {
      ...
      if (ctr >= 0.1) score += 25      // Excellent CTR
      else if (ctr >= 0.05) score += 15  // Good CTR
      else if (ctr >= 0.02) score += 5   // Average CTR
    }
    
    // Multi-campaign engagement bonus
    const uniqueCampaigns = new Set(
      engagements.map(e => e.campaign_id).filter(Boolean)
    ).size
  ...
  private calculateRecencyScore(engagements: any[]): number {
    ...
    if (daysSinceLastEngagement <= 1) return 100      // Today
    else if (daysSinceLastEngagement <= 7) return 80  // This week
    else if (daysSinceLastEngagement <= 30) return 50 // This month
    else if (daysSinceLastEngagement <= 90) return 25 // This quarter
  ...
  private calculateFrequencyScore(engagements: any[]): number {
    ...
    else if (frequency >= 5) return 75  // Frequent
    else if (frequency >= 2) return 50  // Moderate
    else if (frequency >= 1) return 25  // Occasional
   ...
   */
  private getQualificationStatus(score: number, engagements: any[]): LeadScore['qualification_status'] {
  ...
  private calculateConfidenceLevel(engagements: any[], email: string): number {
    ...
    const daysSinceLastEngagement = engagements.length > 0 
  ...
  private analyzeEngagementPattern(engagements: any[]): LeadInsights['engagement_pattern'] {
    ...
    const recentAvgScore = recentEngagements.reduce((sum, e) => sum + (e.score_value || 0), 0) / recentEngagements.length
    const olderAvgScore = olderEngagements.reduce((sum, e) => sum + (e.score_value || 0), 0) / olderEngagements.length
  ...
  private async analyzeCampaignResponsiveness(engagements: any[]): Promise<LeadInsights['campaign_responsiveness']> {
  ...
  private calculateOptimalFrequency(engagements: any[]): number {
    ...
    const avgDailyEngagements = Array.from(dailyEngagements.values())
      .reduce((sum, count) => sum + count, 0) / dailyEngagements.size
