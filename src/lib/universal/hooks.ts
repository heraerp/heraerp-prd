...
export function useUniversalApi(options?: UseUniversalApiOptions) {
  ...
  const entities = {
    ...
    recover: (id: string) =>
      request(`/entities/${id}/recover`, { method: 'POST' })
  };
  ...
  const transactions = {
    ...
    reverse: (id: string, data: { p_reason: string; p_user_id: string; p_reversal_date?: string }) =>
    ...
    validate: (id: string) =>
      request(`/transactions/${id}/validate`, { method: 'POST' }),
