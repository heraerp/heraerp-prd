...
async function fromDynamic(fn: string): Promise<FnSignature[] | FnSignature | null> {
  ...
  const ent = await sb.from('core_entities').select('id').match(eqs).limit(1).maybeSingle()
...
export async function getFnSignatures(fn: string): Promise<FnSignature[] | FnSignature | null> {
  // 1) cache
  const now = Date.now()
