...
import { Guardrails, parseSmartCode, type SmartCodeParts } from './guardrails'
...
export class SmartCodeEngine {
   ...
   */
  constructor(supabaseUrl: string, supabaseKey: string, options: { cacheTimeout?: number } = {}) {
  ...
  async loadSmartCode(
    ...
    Guardrails.guardSmartCode(smartCode)
   ...
   */
  private async loadUCRRules(smartCode: string, organizationId: string): Promise<UCRRule[]> {
   ...
   * Load UCR bundle
   */
  private async loadUCRBundle(smartCode: string, organizationId: string): Promise<any | null> {
   ...
   */
  private async loadProcedures(smartCode: string, organizationId: string): Promise<Procedure[]> {
   ...
   */
  private async loadPlaybooks(smartCode: string, organizationId: string): Promise<Playbook[]> {
   ...
   */
  async getValidationRules(smartCode: string, organizationId: string): Promise<UCRRule[]> {
   ...
   */
  async getCalculationRules(smartCode: string, organizationId: string): Promise<UCRRule[]> {
    const context = await this.loadSmartCode(smartCode, organizationId)
   ...
   */
  async getDerivationRules(smartCode: string, organizationId: string): Promise<UCRRule[]> {
    const context = await this.loadSmartCode(smartCode, organizationId)
  ...
  async executeProcedure(
    ...
    const { data, error } = await this.supabase.rpc(procedure.database_function, enrichedParameters)
  ...
  private validateProcedureParameters(procedure: Procedure, parameters: Record<string, any>): void {
 ...
 * );
 */
export function getSmartCodeEngine(supabaseUrl?: string, supabaseKey?: string): SmartCodeEngine {
  if (!engineInstance) {
...
export const loadSmartCode = async (smartCode: string, organizationId: string) => {
  const engine = getSmartCodeEngine()
