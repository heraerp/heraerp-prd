...
export class EntityBuilder {
  ...
  private static buildFieldValidator(fieldSchema: FieldSchema): ZodSchema {
    ...
    switch (fieldSchema.field_type) {
      ...
      case 'enum':
        if (fieldSchema.validations.allowed_values && fieldSchema.validations.allowed_values.length > 0) {
  ...
  static async create(data: Record<string, any>, orgId: string): Promise<EntityCreationResult> {
    ...
    try {
      ...
      const dynamicFields = Object.entries(validated).filter(
        ([key]) => !key.startsWith('p_') && dynamicSchema.field_schemas.some((f) => f.field_name === key)
      );
   ...
   */
  static async delete(entityId: string, orgId: string): Promise<{ success: boolean; procedures_executed: string[] }> {
    guardOrganization(orgId);
  ...
  static async get(entityId: string, orgId: string): Promise<any> {
    ...
    // Get dynamic fields
    const { data: dynamicFields, error: fieldsError } = await supabase.rpc('hera_dynamic_data_get_v1', {
      p_entity_id: entityId,
      p_organization_id: orgId,
    });
    ...
    if (dynamicFields && Array.isArray(dynamicFields)) {
      for (const field of dynamicFields) {
        const value = field.field_value_number ?? field.field_value_boolean ?? field.field_value_date ?? field.field_value_json ?? field.field_value;
