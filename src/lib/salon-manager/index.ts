...
export class SalonManagerService implements ISalonManagerService {
    ...
    try {
      ...
      await universalApi.createTransactionLine({
        ...
        metadata: {
          ...
          commission_rate: stylist.commission_rate || 0.30
        }
      });
  ...
  async findAvailableSlots(date: Date, serviceName?: string): Promise<any> {
    try {
      console.log('Finding available slots:', { date, serviceName, organizationId: this.organizationId });
  ...
  async getAIInsights(context: string): Promise<any> {
    try {
      ...
      if (recentBookings && recentBookings.length > 0) {
        ...
        const popularService = Array.from(serviceCounts.entries())
          .sort((a, b) => b[1] - a[1])[0];
      ...
      if (revenue.totalRevenue > 0 && lastMonthRevenue.totalRevenue > 0) {
        const growthRate = ((revenue.totalRevenue - lastMonthRevenue.totalRevenue) / lastMonthRevenue.totalRevenue) * 100;
  ...
  async getClientBirthdays(month: number): Promise<any> {
    ...
    const birthdayClients = birthdays?.filter(record => {
  ...
  private async findStylist(name: string): Promise<any> {
    ...
    if (stylist) {
      ...
      stylist.commission_rate = commissionField?.field_value_number || 0.30;
    }
  ...
  private async findService(name: string): Promise<ServiceInfo> {
    ...
    const priceField = service.core_dynamic_data?.find(
...
export function createSalonManagerService(organizationId: string, userId: string): ISalonManagerService {
