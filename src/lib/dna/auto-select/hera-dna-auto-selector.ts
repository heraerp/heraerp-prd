...
export class HeraDNAAutoSelector {
  ...
  analyzeBuildRequest(request: BuildRequest): DNASelection {
    ...
    const enterpriseIndicators = [
      'enterprise', 'professional', 'business', 'corporate',
      'dashboard', 'analytics', 'admin', 'management',
      'saas', 'platform', 'system', 'application'
    ];

    const uiIndicators = [
      'ui', 'interface', 'frontend', 'component', 'design',
      'glass', 'glassmorphism', 'modern', 'premium'
    ];

    const fioriIndicators = [
      'fiori', 'sap', 'worklist', 'object page', 'shell bar',
      'responsive table', 'smart table', 'enterprise table'
    ];

    // Check if request warrants HERA DNA
    const hasEnterpriseNeeds = enterpriseIndicators.some(indicator => 
      prompt.includes(indicator)
    );
    
    const hasUIRequirements = uiIndicators.some(indicator => 
      prompt.includes(indicator)
    );
    
    const hasFioriPatterns = fioriIndicators.some(indicator => 
      prompt.includes(indicator)
    );

    // Auto-select HERA DNA for enterprise builds
    const useHeraDNA = hasEnterpriseNeeds || hasUIRequirements || hasFioriPatterns ||
      request.techStack?.includes('React') || request.techStack?.includes('Next.js');
...
export function analyzeAndEnhanceBuildRequest(prompt: string, options?: Partial<BuildRequest>): {
