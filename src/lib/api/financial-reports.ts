 ...
 */
export async function generatePnLData({
  organizationId,
  startDate,
  endDate,
  branchId
...
export async function getDailySalesReport(organizationId: string, date: string) {
  ...
  data?.forEach(txn => {
    ...
    if (txn.target_entity?.entity_name) {
      ...
      const current = summary.salesByStylist.get(stylistName) || {
        name: stylistName,
        revenue: 0,
        count: 0
      }
  ...
  summary.averageTicket =
    summary.totalTransactions > 0 ? summary.totalSales / summary.totalTransactions : 0
