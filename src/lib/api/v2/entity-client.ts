...
export class EntityClientV2 {
  ...
  async update(
    entityId: string,
    updates: Partial<EntityV2>
  ): Promise<ApiResponse<{ entity_id: string }>> {
    return this.upsert({
      ...updates,
      id: entityId,
      entity_id: entityId,
   ...
   */
  async batchUpsertDynamicFields(
    entityId: string,
    fields: Partial<DynamicField>[]
  ): Promise<ApiResponse<any[]>> {
    const orgId = this.organizationId
    ...
    switch (detectedType) {
      ...
      case 'json':
        fieldData.field_value_json =
          typeof value === 'object' ? (value as Record<string, any>) : { value }
        break
    }
