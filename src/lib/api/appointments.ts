...
export class MockAppointmentsApi extends AppointmentsApi {
  ...
  async list(params: AppointmentFilters) {
    ...
    let appointments = Array.from(this.mockAppointments.values())
      .filter(appt => appt.organization_id === params.organization_id)
  ...
  async getUpcomingCount(organizationId: string) {
    ...
    const now = new Date()
    const upcoming = Array.from(this.mockAppointments.values())
      .filter(appt => 
...
export function useAppointmentsList({ organizationId, filters }: { 
