  ...
  const analyticsResults = await Promise.all(
    campaigns.map(async campaign => {
      ...
      const analytics: EmailCampaignAnalytics = {
        ...
        email_to_opportunity_rate:
          emailsDelivered > 0
            ? Math.round((opportunities.length / emailsDelivered) * 10000) / 100
            : 0,
        email_to_revenue_rate:
          emailsDelivered > 0
            ? Math.round((revenueData.total_revenue / emailsDelivered) * 100) / 100
            : 0
      }
  ...
  return NextResponse.json({
    ...
    data: {
      ...
      summary: {
        ...
        total_revenue_attributed: analyticsResults.reduce(
          (sum, c) => sum + c.revenue_attributed,
          0
        ),
        average_open_rate:
          analyticsResults.length > 0
            ? Math.round(
                (analyticsResults.reduce((sum, c) => sum + c.open_rate, 0) /
...
async function getLeadGenerationFunnel(searchParams: URLSearchParams) {
  ...
  for (let i = 0; i < funnelStages.length; i++) {
    ...
    const conversionRate =
      i === 0 ? 100 : previousStageCount > 0 ? (stageCount / previousStageCount) * 100 : 0
    ...
    data: {
      ...
      overall_conversion_rate:
        funnelData.length > 0 && funnelData[0].count > 0
          ? Math.round((funnelData[funnelData.length - 1].count / funnelData[0].count) * 10000) /
            100
          : 0
    }
  })
...
async function getRevenueAttribution(searchParams: URLSearchParams) {
  ...
  const finalAttributionData = attributionData.map(data => ({
    ...
    attribution_percentage:
      totalRevenue > 0 ? Math.round((data.total_revenue / totalRevenue) * 10000) / 100 : 0
  }))
  ...
  return NextResponse.json({
    ...
    data: {
      ...
      performance_indicators: {
        email_to_lead_rate:
          campaignSummary.emails_sent_period > 0
            ? Math.round(
                (conversionMetrics.total_leads / campaignSummary.emails_sent_period) * 10000
              ) / 100
            : 0,
...
async function getEmailCampaignSummary() {
  ...
  return {
    ...
    average_open_rate:
      totalEmailsSent > 0 ? Math.round((totalOpens / totalEmailsSent) * 10000) / 100 : 0,
    average_click_rate:
      totalEmailsSent > 0 ? Math.round((totalClicks / totalEmailsSent) * 10000) / 100 : 0
  }
}
