...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // Filter by type
    if (type === 'media') {
      messagesQuery = messagesQuery.in('metadata->>message_type', [
        'image',
        'video',
        'document',
        'audio'
    ...
    messagesQuery = messagesQuery.or('metadata->>deleted.is.null,metadata->>deleted.eq.false')
    ...
    if (type === 'starred') {
      filteredMessages = filteredMessages.filter(msg => starredMessageIds.includes(msg.id))
    }
    ...
    const enhancedMessages = filteredMessages.map(msg => {
      const conversation = conversations?.find(
        conv => conv.id === msg.source_entity_id || conv.id === msg.target_entity_id
      )
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        results:
          type === 'all' || type === 'media' ? enhancedMessages : Object.values(groupedResults),
        query,
        type
      }
    })
