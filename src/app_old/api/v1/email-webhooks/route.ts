...
export async function POST(request: NextRequest) {
  try {
    ...
    const organizationId =
      request.headers.get('x-organization-id') || searchParams.get('organization_id')
    ...
    const signature =
      request.headers.get('webhook-signature') || request.headers.get('x-signature') || ''
    ...
    for (const eventData of events) {
      try {
        ...
        const parser =
          webhookParsers[provider as keyof typeof webhookParsers] || webhookParsers.generic
...
async function updateCampaignMetrics(
  ...
  try {
    ...
    const currentCount =
      (await universalApi.getDynamicField(campaignId, `emails_${eventType}`)) || '0'
    ...
    if (eventType === 'clicked') {
      const opensCount = parseInt(
        (await universalApi.getDynamicField(campaignId, 'emails_opened')) || '0'
      )
  ...
  try {
    ...
    if (existingContacts.length === 0) {
      ...
      if (campaignId) {
        ...
        const currentLeads =
          (await universalApi.getDynamicField(campaignId, 'leads_generated')) || '0'
      ...
      const currentScore = parseInt(
        (await universalApi.getDynamicField(contact.id, 'engagement_score')) || '0'
      )
...
async function updateContactEngagementScore(
  ...
  try {
    ...
    if (contacts.length > 0) {
      ...
      const currentScore = parseInt(
        (await universalApi.getDynamicField(contact.id, 'email_engagement_score')) || '0'
      )
