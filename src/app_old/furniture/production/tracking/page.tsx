...
import { Activity, Factory, Package, Clock, CheckCircle, AlertCircle, Play, Pause, RotateCcw, TrendingUp, User,
  Gauge
} from 'lucide-react'
...
const stats ={
  ...
  return (
    <div className="min-h-screen bg-[var(--color-body)]"> <div className="p-6 space-y-6"> 
        {/* Header  */}
        <div className="flex justify-between items-center"> <div> <h1 className="bg-[var(--color-body)] text-3xl font-bold text-[var(--color-text-primary)]">Real-Time Production Tracking</h1> <p className="text-[var(--color-text-secondary)] mt-1">Monitor live production status and performance</p> <OrganizationInfo name={organizationName} id={organizationId} /> </div>
        <div className="bg-[var(--color-body)] flex gap-3"> <Button variant="outline" size="sm" className="gap-2 hover:bg-[var(--color-hover)]"> <RotateCcw className="h-4 w-4" /> Auto-refresh: ON </Button> </div> </div> 
        {/* Live Statistics  */}
        <div className="bg-[var(--color-body)] grid grid-cols-1 md:grid-cols-4 gap-4"> <Card className="bg-[var(--color-surface-raised)] border-[var(--color-border)] p-6 bg-[var(--color-body)]/50 backdrop-blur-sm border-[var(--color-border)]/50"> <div className="flex items-center justify-between mb-2"> <Activity className="h-8 w-8 text-green-500" /> <Badge variant="secondary" className="bg-green-500/10 text-green-400 animate-pulse">
          LIVE 
        </Badge> </div> <p className="text-2xl font-bold text-[var(--color-text-primary)]">{stats.activeOperations}</p> <p className="text-sm text-[var(--color-text-secondary)] mt-1">Active Operations</p> </Card>
        <Card className="bg-[var(--color-surface-raised)] border-[var(--color-border)] p-6 bg-[var(--color-body)]/50 backdrop-blur-sm border-[var(--color-border)]/50"> <div className="flex items-center justify-between mb-2"> <Package className="h-8 w-8 text-[#37353E]" /> <TrendingUp className="h-4 w-4 text-[#37353E]" /> </div> <p className="text-2xl font-bold text-[var(--color-text-primary)]">{stats.unitsInProgress}</p> <p className="text-sm text-[var(--color-text-secondary)] mt-1">Units in Progress</p> </Card>
...
const totalOperations =orderLines.length || 1 const progress = (completedOperations / totalOperations) * 100 return (
    <Card key={center.id} className="p-6 bg-[var(--color-body)]/50 backdrop-blur-sm border-[var(--color-border)]/50" > <div className="flex items-start justify-between mb-4"> <div> <h3 className="bg-[var(--color-body)] font-semibold text-[var(--color-text-primary)]">{center.entity_name}</h3> <p className="text-sm text-[var(--color-text-secondary)]"> {(center.metadata as any)?.location || 'Shop Floor'} </p> </div> <Badge className={ activeOrder ? 'bg-green-500/10 text-green-400' : 'bg-gray-9000/10 text-[var(--color-text-secondary)]' } >
          {activeOrder ? 'Running' : 'Idle'} 
            ...
            <div className="space-y-3"> <div> <p className="text-sm font-medium text-[var(--color-text-primary)]"> {activeOrder.transaction_code} </p> <p className="text-xs text-[var(--color-text-secondary)]"> {product?.entity_name || 'Unknown Product'} </p> </div>
        <div className="space-y-2"> <div className="flex justify-between text-xs"> <span className="text-[var(--color-text-secondary)]">Progress</span> <span className="text-[var(--color-text-primary)]">{Math.round(progress)}%</span> </div> <Progress value={progress} className="h-2 bg-muted-foreground/10" /> </div>
        <div className="flex items-center justify-between text-xs"> <span className="text-[var(--color-text-secondary)]">Current Operation</span> <span className="text-[var(--color-text-primary)]"> {currentOperation?.metadata?.operation || 'Setup'} </span> </div>
        <div className="flex gap-2"> <Button size="sm" variant="outline" className="flex-1 h-8 hover:bg-[var(--color-hover)]"> <Pause className="h-3 w-3 mr-1" /> Pause </Button>
        <Button size="sm" variant="outline" className="flex-1 h-8 hover:bg-[var(--color-hover)]"> <CheckCircle className="h-3 w-3 mr-1" /> Complete </Button> </div>
      </div>
    )
          : (
            <div className="text-center py-8"> <Factory className="h-12 w-12 mx-auto text-[var(--color-text-secondary)] mb-2" /> <p className="text-sm text-[var(--color-text-secondary)]">No active order</p> <Link href="/furniture/production/orders/new"> <Button size="sm" variant="outline" className="mt-2 hover:bg-[var(--color-hover)]"> <Play className="h-3 w-3 mr-1" /> Start Production </Button> </Link>
      </div>
      )
    } </Card>
      )
...
const progress = order.total_amount ? (completedQty / order.total_amount) * 100 : 0 return (
    <div key={order.id} className="p-4 bg-[var(--color-body)]/50 rounded-lg border border-[var(--color-border)]/50" > <div className="flex items-center justify-between"> <div className="flex-1"> <div className="bg-[var(--color-body)] flex items-center gap-4"> <div> <p className="font-semibold text-[var(--color-text-primary)]">{order.transaction_code}</p> <p className="text-sm text-[var(--color-text-secondary)]"> {product?.entity_name} - {order.total_amount} units </p> </div> <Badge className="bg-green-500/10 text-green-400 animate-pulse">
        ...
        <div className="bg-[var(--color-body)] grid grid-cols-1 md:grid-cols-3 gap-4 mt-4"> <div> <p className="text-xs text-[var(--color-text-secondary)]">Work Center</p> <p className="text-sm font-medium text-[var(--color-text-primary)]"> {workCenter?.entity_name || 'Not assigned'} </p> </div>
        ...
        <div> <p className="text-[var(--color-text-primary)]">Operation completed on PRD-FRN-2025-0002</p> <p className="text-xs text-[var(--color-text-secondary)]"> Cutting completed - 15 units • 2 minutes ago </p> </div> </div>
        <div className="bg-[var(--color-body)] flex items-start gap-3 text-sm"> <div className="mt-0.5 p-1.5 bg-[var(--color-body)]/20 rounded"> <Activity className="h-4 w-4 text-[#37353E]" /> </div>
        <div> <p className="text-[var(--color-text-primary)]">Production started on Assembly Line 1</p> <p className="text-xs text-[var(--color-text-secondary)]"> Order PRD-FRN-2025-0002 • 15 minutes ago </p> </div> </div>
        <div className="bg-[var(--color-body)] flex items-start gap-3 text-sm"> <div className="mt-0.5 p-1.5 bg-[var(--color-body)]/10 rounded"> <AlertCircle className="h-4 w-4 text-[#37353E]" /> </div>
        <div> <p className="text-[var(--color-text-primary)]">Quality check required</p> <p className="text-xs text-[var(--color-text-secondary)]"> Batch BATCH-2025-002 ready for inspection • 30 minutes ago </p> </div> </div> </div> </div> </Card> </div> </div>
      </div>
      )
}