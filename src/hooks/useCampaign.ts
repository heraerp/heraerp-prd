...
export function useCampaign(organizationId?: string) {
  ...
  const createCampaign = async (data: any) => {
    ...
    if (data.campaign_type) {
      fieldPromises.push(
        universalApi.setDynamicField(result.data.id, 'campaign_type', data.campaign_type, 'text', organizationId)
      )
    }
    if (data.start_date) {
      fieldPromises.push(
        universalApi.setDynamicField(result.data.id, 'start_date', data.start_date, 'text', organizationId)
      )
    }
    if (data.end_date) {
      fieldPromises.push(
        universalApi.setDynamicField(result.data.id, 'end_date', data.end_date, 'text', organizationId)
      )
    }
    if (data.target_audience) {
      fieldPromises.push(
        universalApi.setDynamicField(result.data.id, 'target_audience', data.target_audience, 'text', organizationId)
      )
    }
    if (data.budget) {
      fieldPromises.push(
    ...
    if (data.channel) {
      fieldPromises.push(
        universalApi.setDynamicField(result.data.id, 'channel', data.channel, 'text', organizationId)
      )
    }
  ...
  const updateCampaign = async (id: string, updates: any) => {
    ...
    if (updates.campaign_type !== undefined) {
      fieldPromises.push(
        universalApi.setDynamicField(id, 'campaign_type', updates.campaign_type, 'text', organizationId)
      )
    }
    ...
    if (updates.target_audience !== undefined) {
      fieldPromises.push(
        universalApi.setDynamicField(id, 'target_audience', updates.target_audience, 'text', organizationId)
      )
    }
