...
export function useSetting(organizationId?: string) {
  ...
  const createSetting = async (data: any) => {
    ...
    if (data.setting_key) {
      fieldPromises.push(
        universalApi.setDynamicField(result.data.id, 'setting_key', data.setting_key, 'text', organizationId)
      )
    }
    if (data.setting_value) {
      fieldPromises.push(
        universalApi.setDynamicField(result.data.id, 'setting_value', data.setting_value, 'text', organizationId)
      )
    }
    if (data.setting_type) {
      fieldPromises.push(
        universalApi.setDynamicField(result.data.id, 'setting_type', data.setting_type, 'text', organizationId)
      )
    }
    if (data.description) {
      fieldPromises.push(
        universalApi.setDynamicField(result.data.id, 'description', data.description, 'text', organizationId)
      )
    }
    if (data.updated_by) {
      fieldPromises.push(
        universalApi.setDynamicField(result.data.id, 'updated_by', data.updated_by, 'text', organizationId)
      )
    }
  ...
  const updateSetting = async (id: string, updates: any) => {
    ...
    if (updates.setting_value !== undefined) {
      fieldPromises.push(
        universalApi.setDynamicField(id, 'setting_value', updates.setting_value, 'text', organizationId)
      )
    }
    if (updates.setting_type !== undefined) {
      fieldPromises.push(
        universalApi.setDynamicField(id, 'setting_type', updates.setting_type, 'text', organizationId)
      )
    }
