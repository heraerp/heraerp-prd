...
export function usePatient(organizationId?: string) {
  ...
  const createPatient = async (data: any) => {
    ...
    if (data.medical_record_number) {
      fieldPromises.push(
        universalApi.setDynamicField(result.data.id, 'medical_record_number', data.medical_record_number, 'text', organizationId)
      )
    }
    if (data.date_of_birth) {
      fieldPromises.push(
        universalApi.setDynamicField(result.data.id, 'date_of_birth', data.date_of_birth, 'text', organizationId)
      )
    }
    if (data.blood_type) {
      fieldPromises.push(
        universalApi.setDynamicField(result.data.id, 'blood_type', data.blood_type, 'text', organizationId)
      )
    }
    if (data.allergies) {
      fieldPromises.push(
        universalApi.setDynamicField(result.data.id, 'allergies', data.allergies, 'text', organizationId)
      )
    }
    if (data.medications) {
      fieldPromises.push(
        universalApi.setDynamicField(result.data.id, 'medications', data.medications, 'text', organizationId)
      )
    }
    if (data.insurance_info) {
      fieldPromises.push(
        universalApi.setDynamicField(result.data.id, 'insurance_info', data.insurance_info, 'text', organizationId)
      )
    }
  ...
  const updatePatient = async (id: string, updates: any) => {
    ...
    if (updates.medical_record_number !== undefined) {
      fieldPromises.push(
        universalApi.setDynamicField(id, 'medical_record_number', updates.medical_record_number, 'text', organizationId)
      )
    }
    if (updates.date_of_birth !== undefined) {
      fieldPromises.push(
        universalApi.setDynamicField(id, 'date_of_birth', updates.date_of_birth, 'text', organizationId)
      )
    }
    ...
    if (updates.insurance_info !== undefined) {
      fieldPromises.push(
        universalApi.setDynamicField(id, 'insurance_info', updates.insurance_info, 'text', organizationId)
      )
    }
