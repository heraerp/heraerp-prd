 ...
 */
export function useTodayStats(organizationId?: string) {
  ...
  const loadStats = useCallback(async () => {
    try {
      ...
      const todayTransactions = allTransactions.filter(
        t => t.transaction_date && t.transaction_date.startsWith(today)
      )
      ...
      const tipTotal = payments.reduce(
        (sum, t) => sum + (((t.metadata as any)?.tip_amount as number) || 0),
        0
      )
