...
export function useReport(organizationId?: string) {
  ...
  const createReport = async (data: any) => {
    ...
    if (data.report_type) {
      fieldPromises.push(
        universalApi.setDynamicField(result.data.id, 'report_type', data.report_type, 'text', organizationId)
      )
    }
    if (data.frequency) {
      fieldPromises.push(
        universalApi.setDynamicField(result.data.id, 'frequency', data.frequency, 'text', organizationId)
      )
    }
    if (data.parameters) {
      fieldPromises.push(
        universalApi.setDynamicField(result.data.id, 'parameters', data.parameters, 'text', organizationId)
      )
    }
    if (data.last_run) {
      fieldPromises.push(
        universalApi.setDynamicField(result.data.id, 'last_run', data.last_run, 'text', organizationId)
      )
    }
    if (data.recipients) {
      fieldPromises.push(
        universalApi.setDynamicField(result.data.id, 'recipients', data.recipients, 'text', organizationId)
      )
    }
    if (data.format) {
      fieldPromises.push(
