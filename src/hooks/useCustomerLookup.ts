...
export function useCustomerLookup(organizationId: string) {
  ...
  const loadCustomer = useCallback(async (customerId: string): Promise<CustomerData | null> => {
  ...
  const searchCustomers = useCallback(async (options: CustomerSearchOptions = {}): Promise<CustomerData[]> => {
    ...
    try {
      ...
      const customers: CustomerData[] = customersResponse.data.map(customer => {
        ...
        if (options.phone && dynamicFields.phone && !dynamicFields.phone.includes(options.phone)) {
          return null
        }
        if (options.email && dynamicFields.email && !dynamicFields.email.toLowerCase().includes(options.email.toLowerCase())) {
          return null
        }
  ...
  const quickSearchCustomers = useCallback(async (query: string): Promise<CustomerData[]> => {
  ...
  const getCustomerAppointments = useCallback(async (customerId: string, limit: number = 10) => {
  ...
  const updateCustomerVisit = useCallback(async (customerId: string, visitAmount: number) => {
    if (!organizationId || !customerId) return
