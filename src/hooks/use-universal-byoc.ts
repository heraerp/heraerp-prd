...
export function useUniversalBYOC({
  ...
  // Save configuration
  const saveConfiguration = useCallback(async (config: Partial<BYOCConfiguration>): Promise<boolean> => {
    try {
      setLoading(true)
      setError(null)
  ...
  const activateConfiguration = useCallback(async (configId: string): Promise<boolean> => {
    try {
      setLoading(true)
      setError(null)
      
      const result = await byocService.activateConfiguration(configId, applicationId, organizationId, userId)
  ...
  // Test connection
  const testConnection = useCallback(async (configId: string): Promise<BYOCTestResult | null> => {
    try {
  ...
  // Test permissions
  const testPermissions = useCallback(async (configId: string): Promise<BYOCTestResult | null> => {
    try {
  ...
  // Test performance
  const testPerformance = useCallback(async (configId: string): Promise<BYOCTestResult | null> => {
    try {
  ...
  // Test security
  const testSecurity = useCallback(async (configId: string): Promise<BYOCTestResult | null> => {
    try {
      const response = await fetch('/api/v1/universal/byoc/test', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  // Run full test suite
  const runFullTest = useCallback(async (configId: string): Promise<BYOCTestResult | null> => {
    try {
      const response = await fetch('/api/v1/universal/byoc/test', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const exportConfiguration = useCallback((configId: string, includeSecrets = false): string | null => {
  ...
  const importConfiguration = useCallback(async (jsonData: string): Promise<boolean> => {
    try {
      setLoading(true)
      setError(null)
  ...
  const getConfigByProvider = useCallback((provider: string): BYOCConfiguration | null => {
