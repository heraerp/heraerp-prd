...
export function useCustomers(organizationId?: string) {
  ...
  const calculateStats = (customersData: CustomerData[]) => {
    const totalRevenue = customersData.reduce((sum, customer) => {
      const customerRevenue = customer.transactions.reduce(
        (txSum, tx) => txSum + (tx.total_amount || 0),
        0
      )
    ...
    const vipCount = customersData.filter(customer => {
      const loyaltyRel = customer.relationships.find(
        r => r.relationship_type === 'has_status' && (r.metadata as any)?.status_name === 'Platinum'
      )
      return !!loyaltyRel
    }).length
    ...
    try {
      ...
      for (const field of fields) {
        if (updates[field] !== undefined) {
          fieldUpdates.push(updateDynamicField(customerId, field, updates[field], organizationId))
        }
      }
