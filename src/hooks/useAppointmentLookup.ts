...
export function useAppointmentLookup(organizationId: string) {
  ...
  const loadAppointment = useCallback(async (appointmentId: string): Promise<AppointmentData | null> => {
  ...
  const loadAppointmentServices = useCallback(async (serviceIds: string[]): Promise<AppointmentService[]> => {
  ...
  const searchAppointments = useCallback(async (options: {
    date_from?: string
    date_to?: string
    status?: string
    stylist_id?: string
    ...
    try {
      ...
      const appointments: AppointmentData[] = appointmentsResponse.data.slice(0, options.page_size || 20).map(appt => ({
        id: appt.id,
