...
export function useTransaction(organizationId?: string) {
  ...
  const createTransaction = async (data: any) => {
    ...
    if (data.payment_method) {
      fieldPromises.push(
        universalApi.setDynamicField(result.data.id, 'payment_method', data.payment_method, 'text', organizationId)
      )
    }
    if (data.amount) {
      fieldPromises.push(
    ...
    if (data.reference_number) {
      fieldPromises.push(
        universalApi.setDynamicField(result.data.id, 'reference_number', data.reference_number, 'text', organizationId)
      )
    }
    if (data.status) {
      fieldPromises.push(
  ...
  const updateTransaction = async (id: string, updates: any) => {
    ...
    if (updates.payment_method !== undefined) {
      fieldPromises.push(
        universalApi.setDynamicField(id, 'payment_method', updates.payment_method, 'text', organizationId)
      )
    }
    ...
    if (updates.reference_number !== undefined) {
      fieldPromises.push(
        universalApi.setDynamicField(id, 'reference_number', updates.reference_number, 'text', organizationId)
      )
    }
