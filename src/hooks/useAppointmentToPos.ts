...
export function useAppointmentToPos(): UseAppointmentToPosReturn {
  ...
  const loadAppointment = useCallback(
    async (appointmentId: string): Promise<AppointmentToPos> => {
      setIsLoading(true)
      setError(null)
  ...
  const markServiceComplete = useCallback(
  ...
  const convertToPos = useCallback(
    (appointment: AppointmentToPos, additionalItems: PosCartItem[] = []): PosCartItem[] => {
  ...
  const processAppointmentCheckout = useCallback(
    async (appointment: AppointmentToPos, payments: any[], additionalItems: PosCartItem[] = []) => {
      setIsLoading(true)
      setError(null)
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : 'Failed to process appointment checkout'
        setError(errorMessage)
        throw err
      } finally {
        setIsLoading(false)
