...
export function useLoyalty_program(organizationId?: string) {
  ...
  const createLoyalty_program = async (data: any) => {
    ...
    if (data.points_ratio) {
      fieldPromises.push(
        universalApi.setDynamicField(result.data.id, 'points_ratio', data.points_ratio, 'text', organizationId)
      )
    }
    if (data.tier_benefits) {
      fieldPromises.push(
        universalApi.setDynamicField(result.data.id, 'tier_benefits', data.tier_benefits, 'text', organizationId)
      )
    }
    if (data.expiry_days) {
      fieldPromises.push(
        universalApi.setDynamicField(result.data.id, 'expiry_days', data.expiry_days, 'text', organizationId)
      )
    }
    if (data.tier_name) {
      fieldPromises.push(
        universalApi.setDynamicField(result.data.id, 'tier_name', data.tier_name, 'text', organizationId)
      )
    }
    if (data.minimum_spend) {
      fieldPromises.push(
        universalApi.setDynamicField(result.data.id, 'minimum_spend', data.minimum_spend, 'text', organizationId)
      )
    }
  ...
  const updateLoyalty_program = async (id: string, updates: any) => {
    ...
    if (updates.points_ratio !== undefined) {
      fieldPromises.push(
        universalApi.setDynamicField(id, 'points_ratio', updates.points_ratio, 'text', organizationId)
      )
    }
    if (updates.tier_benefits !== undefined) {
      fieldPromises.push(
        universalApi.setDynamicField(id, 'tier_benefits', updates.tier_benefits, 'text', organizationId)
      )
    }
    ...
    if (updates.minimum_spend !== undefined) {
      fieldPromises.push(
        universalApi.setDynamicField(id, 'minimum_spend', updates.minimum_spend, 'text', organizationId)
      )
    }
