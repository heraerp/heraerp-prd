...
export function useService(organizationId?: string) {
  ...
  const createService = async (data: any) => {
    ...
    if (data.requires_license) {
      fieldPromises.push(
        universalApi.setDynamicField(result.data.id, 'requires_license', data.requires_license, 'text', organizationId)
      )
    }
  ...
  const updateService = async (id: string, updates: any) => {
    ...
    if (updates.requires_license !== undefined) {
      fieldPromises.push(
        universalApi.setDynamicField(id, 'requires_license', updates.requires_license, 'text', organizationId)
      )
    }
