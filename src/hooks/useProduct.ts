...
export function useProduct(organizationId?: string) {
  ...
  const createProduct = async (data: any) => {
    ...
    if (data.stock_level) {
      fieldPromises.push(
        universalApi.setDynamicField(result.data.id, 'stock_level', data.stock_level, 'text', organizationId)
      )
    }
    if (data.reorder_point) {
      fieldPromises.push(
        universalApi.setDynamicField(result.data.id, 'reorder_point', data.reorder_point, 'text', organizationId)
      )
    }
    if (data.category) {
      fieldPromises.push(
        universalApi.setDynamicField(result.data.id, 'category', data.category, 'text', organizationId)
      )
    }
    if (data.location) {
      fieldPromises.push(
        universalApi.setDynamicField(result.data.id, 'location', data.location, 'text', organizationId)
      )
    }
  ...
  const updateProduct = async (id: string, updates: any) => {
    ...
    if (updates.reorder_point !== undefined) {
      fieldPromises.push(
        universalApi.setDynamicField(id, 'reorder_point', updates.reorder_point, 'text', organizationId)
      )
    }
