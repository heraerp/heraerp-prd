...
export function useLeaveManagement(options: UseLeaveManagementOptions = {}) {
  ...
  const submitLeaveRequest = useCallback(async (request: any) => {
    if (!organizationId) {
      toast({
        title: 'Error',
    ...
    try {
      const result = await leaveManagementApi.createLeaveRequest(
        request,
        organizationId
      )
  ...
  const cancelLeaveRequest = useCallback(async (requestId: string, reason: string) => {
    if (!organizationId || !user?.id) {
      toast({
        title: 'Error',
        description: 'Authentication required',
  ...
  const getEmployeeBalance = useCallback(async (employeeId: string, leaveType: string) => {
  ...
  const createBalanceAdjustment = useCallback(async (
    employeeId: string,
    leaveType: string,
    days: number,
    reason: string
  ...
  const getTeamOverview = useCallback(async (
    managerId: string,
    startDate: string,
    endDate: string
  ) => {
  ...
  const generateAnnualReport = useCallback(async (
  ...
  const refreshLeaveData = useCallback(async () => {
    ...
    try {
      ...
      if (requests.success && requests.data) {
        ...
        const leaveRequests = requests.data.filter(r => 
