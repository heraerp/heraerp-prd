 ...
 */
export async function getBranchPnL(params: BranchPnLParams): Promise<BranchPnLResult> {
  ...
  try {
    ...
    if (Object.keys(aggregates).length > 0) {
      const branchIds = [...new Set(Object.keys(aggregates).map(k => k.split('::')[0]))]
 ...
 */
export async function getBranchComparison(params: Omit<BranchPnLParams, 'branch_id'>): Promise<{
  branches: Array<{
    branch_id: string
    branch_name: string
    revenue: number
