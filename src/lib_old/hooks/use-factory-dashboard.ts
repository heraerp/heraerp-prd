...
export function useFactoryDashboard() {
  ...
  const {
    ...
    error: txnError,
    mutate: mutateTxns
  } = useSWR(['transactions', filters], () =>
    client.getTransactions(filters.organization_id, filters.from_date!, filters.to_date!, filters)
  )
  ...
  } = useSWR(['modules', filters.organization_id], () => client.getModules(filters.organization_id))
