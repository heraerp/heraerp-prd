...
export class IceCreamChatStorage {
  ...
  async getChatHistory(options?: {
    ...
    try {
      ...
      const transactions = await universalApi.searchTransactions(filters, {
      ...
      return transactions.map(tx => ({
        ...
        message_type:
          tx.smart_code === ICECREAM_CHAT_SMART_CODES.USER_QUERY
            ? 'user'
            : tx.smart_code === ICECREAM_CHAT_SMART_CODES.AI_RESPONSE
              ? 'assistant'
              : 'system',
      ...
      allMessages.forEach(msg => {
        if (!sessionMap.has(msg.session_id)) {
          ...
          sessionMap.set(msg.session_id, {
            ...
            preview:
              firstUserMessage.substring(0, 150) + (firstUserMessage.length > 150 ? '...' : ''),
