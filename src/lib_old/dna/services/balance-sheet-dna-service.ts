...
export const BALANCE_SHEET_DNA_CONFIG = {
  ...
  industries: {
    salon: {
      ...
      daily_metrics: ['cash_position', 'current_ratio', 'total_assets', 'total_equity'],
      alerts: {
...
export class BalanceSheetDNAService {
  ...
  constructor(api: UniversalAPIClient, organizationId: string, industryType: string = 'salon') {
    ...
    this.industryConfig =
      BALANCE_SHEET_DNA_CONFIG.industries[
        industryType as keyof typeof BALANCE_SHEET_DNA_CONFIG.industries
   ...
   */
  async generateDailyBalanceSheet(
    options: Partial<BalanceSheetOptions> = {}
  ): Promise<DailyBalanceSheetReport> {
    const {
    ...
    try {
      ...
      const accounts: BalanceSheetAccount[] =
        balanceSheetResponse.data?.map((account: any) => ({
      ...
      const summary: BalanceSheetSummary[] =
        summaryResponse.data?.map((item: any) => ({
      ...
      if (includeRatios) {
        ...
        ratios =
          ratiosResponse.data?.map((ratio: any) => ({
  ...
  async generateConsolidatedBalanceSheet(
    ...
    const { asOfDate = new Date().toISOString().split('T')[0] } = options
    ...
    try {
      ...
      for (const orgId of organizationIds) {
        const service = new BalanceSheetDNAService(
          this.api,
          orgId,
          this.industryConfig.name.toLowerCase()
        )
   ...
   */
  private generateBalanceSheetAlerts(
    accounts: BalanceSheetAccount[],
    ratios: FinancialRatio[]
  ): Array<{
    type: 'info' | 'warning' | 'error'
    ...
    const cashAccounts = accounts.filter(
      acc => acc.accountGroup.toLowerCase().includes('cash') || acc.accountCode.startsWith('11')
    )
    ...
    if (
      debtRatio &&
      config.alerts?.high_debt_ratio &&
      debtRatio.ratioValue > config.alerts.high_debt_ratio
    ) {
      alerts.push({
        type: 'error' as const,
   ...
   */
  private calculateConsolidatedSummary(
    organizationSummaries: DailyBalanceSheetReport[]
  ): BalanceSheetSummary[] {
   ...
   */
  private calculateConsolidatedRatios(
    organizationSummaries: DailyBalanceSheetReport[]
  ): FinancialRatio[] {
    // For simplicity, this returns weighted average ratios
    const consolidatedRatios: FinancialRatio[] = []

    const totalAssets = organizationSummaries.reduce(
      (sum, org) => sum + org.metadata.totalAssets,
      0
    )
    const totalLiabilities = organizationSummaries.reduce(
      (sum, org) => sum + org.metadata.totalLiabilities,
      0
    )
    const totalEquity = organizationSummaries.reduce(
      (sum, org) => sum + org.metadata.totalEquity,
      0
    )
