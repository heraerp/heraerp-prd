...
export class AnalyticsChatStorage {
  ...
  async saveMessage(message: ChatMessage): Promise<string> {
    try {
      const smartCode =
        message.message_type === 'user' ? CHAT_SMART_CODES.USER_QUERY : CHAT_SMART_CODES.AI_RESPONSE
   ...
   */
  async getChatSessions(options?: { limit?: number; offset?: number }): Promise<ChatSession[]> {
    try {
