...
export interface CreateDynamicFieldRequest
  extends Omit<DynamicDataComplete, 'id' | 'created_at' | 'updated_at' | 'version'> {
...
export class UniversalAPIDynamicDataComplete extends UniversalAPISchemaComplete {
  ...
  async createDynamicFieldComplete(request: CreateDynamicFieldRequest): Promise<any> {
    ...
    return this.execute({
      ...
      ai_requests: request.ai_processing
        ? {
            enrich: request.ai_processing.auto_enhance
              ? ['field_enhancement', 'value_insights']
              : [],
   ...
   */
  async queryDynamicFieldsComplete(
    organizationId: string,
    filters: DynamicFieldQueryFilters = {}
  ): Promise<any> {
    ...
    if (filters.entity_id)
      queryFilters.entity_id = Array.isArray(filters.entity_id)
        ? { in: filters.entity_id }
        : filters.entity_id
    if (filters.field_name)
      queryFilters.field_name = Array.isArray(filters.field_name)
        ? { in: filters.field_name }
        : filters.field_name
    if (filters.field_type)
      queryFilters.field_type = Array.isArray(filters.field_type)
        ? { in: filters.field_type }
        : filters.field_type
    ...
    if (filters.validation_status)
      queryFilters.validation_status = Array.isArray(filters.validation_status)
        ? { in: filters.validation_status }
    ...
    if (filters.is_system_field !== undefined)
      queryFilters.is_system_field = filters.is_system_field
    ...
    if (filters.field_value_text) {
      if (typeof filters.field_value_text === 'string') {
        ...
        if (filters.field_value_text.contains)
          queryFilters.field_value_text = { contains: filters.field_value_text.contains }
        if (filters.field_value_text.starts_with)
          queryFilters.field_value_text = { starts_with: filters.field_value_text.starts_with }
        if (filters.field_value_text.regex)
          queryFilters.field_value_text = { regex: filters.field_value_text.regex }
      }
    }
    ...
    if (filters.field_value_number) {
      if (typeof filters.field_value_number === 'number') {
        ...
        if (filters.field_value_number.min !== undefined)
          queryFilters.field_value_number = { '>=': filters.field_value_number.min }
        if (filters.field_value_number.max !== undefined)
          queryFilters.field_value_number = {
            ...queryFilters.field_value_number,
            '<=': filters.field_value_number.max
          }
        if (filters.field_value_number.equals !== undefined)
          queryFilters.field_value_number = filters.field_value_number.equals
      }
    }

    if (filters.field_value_boolean !== undefined)
      queryFilters.field_value_boolean = filters.field_value_boolean
    ...
    if (filters.field_value_date) {
      if (typeof filters.field_value_date === 'string') {
        ...
        if (filters.field_value_date.from)
          queryFilters.field_value_date = { '>=': filters.field_value_date.from }
        if (filters.field_value_date.to)
          queryFilters.field_value_date = {
            ...queryFilters.field_value_date,
            '<=': filters.field_value_date.to
          }
      }
    }

    if (filters.field_value_json)
      queryFilters.field_value_json = { jsonb_contains: filters.field_value_json }
    if (filters.field_value_file_url)
      queryFilters.field_value_file_url = filters.field_value_file_url
    ...
    if (filters.ai_enhanced_value)
      queryFilters.ai_enhanced_value = { contains: filters.ai_enhanced_value }
    ...
    if (filters.field_order) {
      if (typeof filters.field_order === 'number') {
        ...
        if (filters.field_order.min !== undefined)
          queryFilters.field_order = { '>=': filters.field_order.min }
        if (filters.field_order.max !== undefined)
          queryFilters.field_order = { ...queryFilters.field_order, '<=': filters.field_order.max }
      }
    }
    ...
    // JSONB field queries
    if (filters.calculated_value_query)
      queryFilters.calculated_value = { jsonb_contains: filters.calculated_value_query }
    ...
    return this.query({
      ...
      performance: {
        cache_ttl: 300,
        use_indexes: [
          'idx_dynamic_data_entity_field',
          'idx_dynamic_data_field_type',
          'idx_dynamic_data_searchable'
        ]
      }
    })
  }
    ...
    return {
      ...
      rebuildSearchIndex: async (organizationId: string, entityId?: string): Promise<any> => {
        const filters = entityId
          ? { entity_id: entityId, is_searchable: true }
          : { is_searchable: true }
  ...
  async bulkCreateDynamicFieldsComplete(
    organizationId: string,
    fields: CreateDynamicFieldRequest[]
  ): Promise<any> {
...
export function createUniversalAPIDynamicDataComplete(
  config: any
): UniversalAPIDynamicDataComplete {
