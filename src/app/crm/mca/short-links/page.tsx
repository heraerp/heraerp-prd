...
/**
 * MCA ShortLinks - Click Analytics Hub
 * Mobile-First Enterprise Page for Click Tracking Management
 * 
 * Module: MCA
 * Entity: SHORT_LINK
 * Smart Code: HERA.CRM.MCA.ENTITY.SHORT_LINK.V1
 * Path: /crm/mca/short-links
 * Description: Click tracking and UTM parameter management with real-time analytics
 */
...
/**
 * ShortLink Entity Interface
 * Extends TableRecord for HERA compliance
 */
interface ShortLink extends TableRecord {
  id: string
  entity_id?: string
  entity_name: string
  smart_code: string
...
/**
 * ShortLinks Main Page Component
...
export default function ShortLinksPage() {
  ...
  const [selectedShortLinks, setSelectedShortLinks] = useState<(string | number)[]>([])
  const [showAddModal, setShowAddModal] = useState(false)
  const [showEditModal, setShowEditModal] = useState(false)
  const [showDeleteModal, setShowDeleteModal] = useState(false)
  const [currentShortLink, setCurrentShortLink] = useState<ShortLink | null>(null)
  const [short_linkToDelete, setShortLinkToDelete] = useState<ShortLink | null>(null)
  ...
  const short_links: ShortLink[] = short_linkData.entities?.map((entity: any) => {
    return {
      id: entity.id,
      entity_id: entity.id,
  ...
  const columns: TableColumn[] = [
    { key: 'entity_name', label: 'ShortLink Name', sortable: true },
  ...
  const filterFields: FilterField[] = [
    { key: 'search', label: 'Search ShortLinks', type: 'search' },
  ...
  const handleAddShortLink = async (short_linkData: any) => {
    try {
  ...
  const handleEditShortLink = async (short_linkData: any) => {
    if (!currentShortLink) return
    
    try {
      await short_linkData.update(currentShortLink.entity_id!, {
        entity_name: short_linkData.entity_name
      }, short_linkData)

      // Emit update event
      await short_linkData.emitEvent(SHORT_LINK_SMART_CODES.EVENT_UPDATED, {
        entity_id: currentShortLink.entity_id!,
        user_id: user?.id,
        timestamp: new Date().toISOString(),
  ...
  if (!isAuthenticated) {
    return (
      <div className="p-4 text-center">
        ...
        <p>Please log in to access ShortLinks.</p>
      </div>
    )
  }
  ...
  return (
    ...
    >
      ...
      <MobileDataTable
        data={short_links}
        columns={columns}
        selectedRows={selectedShortLinks}
        onRowSelect={setSelectedShortLinks}
        onRowClick={(short_link) => {
          setCurrentShortLink(short_link)
          setShowEditModal(true)
        }}
        showBulkActions={selectedShortLinks.length > 0}
        bulkActions={[
          {
            label: 'Delete Selected',
            action: async () => {
              // Bulk delete with events
              for (const id of selectedShortLinks) {
          ...
          <MobileCard key={short_link.id} className="p-4 hover:shadow-md transition-shadow">
            <div className="flex justify-between items-start mb-3">
              ...
              <div className="flex space-x-2">
                ...
                <button
                  onClick={() => {
                    setShortLinkToDelete(short_link)
      ...
      {showAddModal && (
        <ShortLinkModal
          title="Add New ShortLink"
          isOpen={showAddModal}
          onClose={() => setShowAddModal(false)}
          onSave={handleAddShortLink}
      ...
      {showEditModal && currentShortLink && (
        <ShortLinkModal
          ...
          onSave={handleEditShortLink}
          initialData={currentShortLink}
          dynamicFields={short_linkData.dynamicFieldsConfig || []}
          businessRules={{"click_tracking":true,"utm_attribution":true,"real_time_analytics":true,"audit_trail":true}}
        />
      )}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg max-w-md w-full p-6">
            <div className="flex items-center mb-4">
              ...
              <h3 className="text-lg font-semibold">Delete ShortLink</h3>
            </div>
...
/**
 * Enterprise ShortLink Modal Component
...
function ShortLinkModal({ 
  ...
  const [formData, setFormData] = useState(() => {
    ...
    dynamicFields.forEach(field => {
      initial[field.name] = initialData?.[field.name as keyof ShortLink] || (field.type === 'number' ? 0 : '')
    })
  ...
  const validateForm = () => {
    ...
    if (!formData.entity_name?.trim()) {
      newErrors.entity_name = 'ShortLink name is required'
    }
