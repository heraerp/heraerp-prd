  ...
  return (
    <div className="print:text-black print:bg-white p-8">
        ...
        <tbody>
          {logsData?.items?.map((log: any) => (
            <tr key={log.id} className="border-b">
              <td className="py-2">{format(new Date(log.created_at), 'MMM d, h:mm a')}</td>
              <td className="py-2">{log.transaction_type}</td>
              <td className="py-2">{log.entity_name || log.entity_code}</td>
              <td className="py-2 max-w-xs truncate">{JSON.stringify(log.metadata || {})}</td>
      ...
      <div className="mt-8 pt-4 border-t text-xs text-gray-600">
        ...
        <p className="mt-2">
          This is an official audit log generated from CivicFlow Communications. All timestamps are
