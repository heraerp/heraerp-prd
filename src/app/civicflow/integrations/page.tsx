      ...
      <div className="flex-1 overflow-hidden">
        <div className="h-full overflow-y-auto">
          {activeView === 'dashboard' && (
            <IntegrationDashboard organizationId={organizationId} />
          )}
          
          {activeView === 'connectors' && (
            <ConnectorList organizationId={organizationId} />
          )}
          
          {activeView === 'mappings' && (
            <MappingList organizationId={organizationId} />
          )}
          
          {activeView === 'sync-jobs' && (
            <SyncJobList organizationId={organizationId} />
          )}
          
          {activeView === 'sync-runs' && (
            <SyncRunHistory organizationId={organizationId} />
          )}
          
          {activeView === 'data-flow' && (
            <DataFlowDiagram organizationId={organizationId} />
          )}
        </div>
      </div>
    </div>
