            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className={`px-2 py-1 rounded text-sm ${
                    sessionType === 'demo' ? 'bg-blue-100 text-blue-800' : 
                    sessionType === 'real' ? 'bg-purple-100 text-purple-800' : 
            ...
            <div className="space-y-4">
              ...
              {user ? (
                <div className="space-y-2 text-sm">
                  ...
                  <div><span className="font-medium">Entity ID:</span> {user.entity_id}</div>
                  <div><span className="font-medium">Name:</span> {user.name}</div>
                  <div><span className="font-medium">Email:</span> {user.email}</div>
                  <div><span className="font-medium">Role:</span> {user.role}</div>
                  <div><span className="font-medium">Session:</span> {user.session_type}</div>
                </div>
              ) : (
          ...
          {organization && (
            <div className="mb-6">
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div><span className="font-medium">ID:</span> {organization.id}</div>
                <div><span className="font-medium">Name:</span> {organization.name}</div>
                <div><span className="font-medium">Type:</span> {organization.type}</div>
                <div><span className="font-medium">Industry:</span> {organization.industry}</div>
              </div>
            </div>
          )}
