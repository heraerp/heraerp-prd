...
import { DollarSign, Calendar, Users, Activity, Crown, Star, Package, Scissors, Receipt, ChevronRight, AlertCircle } from 'lucide-react'
import { LineChart, Line, CartesianGrid, XAxis, YAxis, Tooltip as ReTooltip, ResponsiveContainer, BarChart, Bar } from 'recharts'
import { useHERAAuth } from '@/components/auth/HERAAuthProvider'
import { Alert, AlertDescription } from '@/components/ui/alert'
// Sidebar is provided by /salon layout; no local sidebar import

// --- Types ---
interface KPIData { revenueToday: number; upcomingAppts: number; avgTicket: number; utilization: number }
interface RevenuePoint { week: string; revenue: number }
interface BookingSvc { name: string; value: number }
interface Recent { id: string; when: string; client: string; service: string; total: number; stylist: string }
interface Stylist { id: string; name: string; revenue: number; utilization: number; rating: number }
interface StockRow { sku: string; name: string; stock: number; min: number }
interface MemberTier { tier: string; count: number; arpu: number }
interface NPS { score: number; promoters: number; passives: number; detractors: number }
interface TxnHeader {
  id: string
...
function KPI({ label, value, icon, hint }: { label: string; value: string; icon: React.ReactNode; hint?: string }) {
  return (
    <Card className="bg-transparent border-0 shadow-none" style={{ backgroundColor: COLORS.charcoal, borderRadius: '1rem' }}>
    ...
    async function load() {
      ...
      try {
        const [kpis, rev, recent, styl, inv, mship, nps, txns] = await Promise.all([
          ...
          fetchAPI('/universal', { action: 'read', table: 'universal_transactions', limit: 10, organization_id: organization.id }),
        ])
  ...
  return (
    <TooltipProvider>
      <div className="min-h-screen" style={{ backgroundColor: 'var(--hera-black)' }}>
        ...
        <div className="relative" style={{ minHeight: '100vh' }}>
          ...
          <SoftCard>
            <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
              <Tabs value={range} onValueChange={setRange} className="w-full md:w-auto">
                <TabsList className="bg-transparent p-1 rounded-full border" style={{ borderColor: COLORS.bronze }}>
                  {[
                    ...
                    <TabsTrigger key={t.id} value={t.id} className="rounded-full data-[state=active]:text-black">
                      <span
                        className="px-3 py-1.5 rounded-full"
                        style={{ background: range === t.id ? `linear-gradient(90deg, ${COLORS.gold} 0%, ${COLORS.goldDark} 100%)` : 'transparent', color: COLORS.champagne }}
                      >
                        {t.label}
                      </span>
                    </TabsTrigger>
              ...
              <div className="text-xs opacity-70">{loading ? 'Refreshing…' : 'Last updated: just now'}</div>
            </div>
          </SoftCard>

          {/* KPIs */}
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <KPI label="Revenue" value={loading ? "Loading..." : fmtCurrency(data.kpis.revenueToday)} icon={<DollarSign size={18} color={COLORS.gold} />} />
            <KPI label="Upcoming" value={loading ? "Loading..." : `${data.kpis.upcomingAppts}`} icon={<Calendar size={18} color={COLORS.gold} />} />
            <KPI label="Avg Ticket" value={loading ? "Loading..." : fmtCurrency(data.kpis.avgTicket)} icon={<Users size={18} color={COLORS.gold} />} />
            <KPI label="Utilization" value={loading ? "Loading..." : `${data.kpis.utilization}%`} icon={<Activity size={18} color={COLORS.gold} />} />
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <SoftCard className="lg:col-span-2">
              <SectionTitle title="Revenue – Last 12 Weeks" icon={<DollarSign size={18} color={COLORS.gold} />} right={<Badge style={{ backgroundColor: COLORS.emerald, color: 'white' }}>AI insight</Badge>} />
              <div className="h-64">
                {loading ? (
                  ...
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={data.revenueSeries} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                      <CartesianGrid stroke="#222" strokeDasharray="3 3" />
                      <XAxis dataKey="week" stroke={COLORS.lightText} tick={{ fill: COLORS.lightText, fontSize: 12 }} />
                      <YAxis stroke={COLORS.lightText} tick={{ fill: COLORS.lightText, fontSize: 12 }} />
                      <ReTooltip contentStyle={{ background: COLORS.charcoal, border: `1px solid ${COLORS.black}`, color: COLORS.champagne }} />
                      <Line type="monotone" dataKey="revenue" stroke={chartStroke} strokeWidth={2} dot={false} />
                    </LineChart>
                  </ResponsiveContainer>
                )}
              </div>
              <div className="mt-3 text-sm" style={{ color: COLORS.champagne }}>
                Suggestion: <span style={{ color: COLORS.emerald }}>Launch midweek color promo</span> – last 4 Wednesdays under-index vs avg by 12%.
              </div>
            </SoftCard>

            <SoftCard>
              <SectionTitle title="Bookings by Service" icon={<Calendar size={18} color={COLORS.gold} />} />
              <div className="h-64">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={data.bookingsByService}>
                    <CartesianGrid stroke="#222" strokeDasharray="3 3" />
                    <XAxis dataKey="name" stroke={COLORS.lightText} tick={{ fill: COLORS.lightText }} />
              ...
              <div className="mt-3 text-xs" style={{ color: COLORS.lightText }}>
                Top driver: <span style={{ color: COLORS.champagne, fontWeight: 600 }}>Cut</span> · Opportunity: Upsell treatment with Color.
              </div>
            </SoftCard>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <SoftCard className="lg:col-span-2">
              <SectionTitle title="Recent Bookings" icon={<Calendar size={18} color={COLORS.gold} />} right={<Badge style={{ backgroundColor: COLORS.champagne, color: COLORS.black }}>Latest 5</Badge>} />
            ...
            <SoftCard>
              <SectionTitle title="Top Stylists" icon={<Users size={18} color={COLORS.gold} />} />
              <div className="space-y-4">
                {loading ? (
                  ...
                  <div key={s.id} className="p-3 rounded-xl" style={{ backgroundColor: '#141414', border: `1px solid ${COLORS.black}` }}>
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <Avatar className="h-8 w-8">
                          <AvatarFallback style={{ backgroundColor: COLORS.bronze, color: COLORS.black }}>
                    ...
                    <div className="mt-2">
                      ...
                      <div className="text-xs mt-1 opacity-70">Revenue {fmtCurrency(s.revenue)}</div>
                    </div>
                  </div>
                  ))
                )}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <SoftCard>
              <SectionTitle title="Memberships" icon={<Receipt size={18} color={COLORS.gold} />} right={<Badge style={{ backgroundColor: COLORS.emerald, color: 'white' }}>Emerald Focus</Badge>} />
              <div className="grid grid-cols-3 gap-3">
                {data.membership.map((m: MemberTier) => (
                  <div key={m.tier} className="p-4 rounded-xl text-center" style={{ backgroundColor: '#141414', border: `1px solid ${COLORS.black}` }}>
                    <div className="text-xs uppercase tracking-wide" style={{ color: m.tier === 'Emerald' ? COLORS.emerald : m.tier === 'Gold' ? COLORS.gold : COLORS.rose }}>
                      {m.tier}
                    </div>
                    <div className="text-2xl font-semibold" style={{ color: COLORS.champagne }}>
                      {m.count}
              ...
              <div className="text-xs mt-3 opacity-80">
                Tip: Highlight <span style={{ color: COLORS.emerald }}>Emerald</span> perks at checkout to lift conversion.
              </div>
            </SoftCard>

            <SoftCard>
              <SectionTitle title="Low Stock Alerts" icon={<Package size={18} color={COLORS.gold} />} right={<Badge style={{ backgroundColor: COLORS.rose, color: 'white' }}>Critical</Badge>} />
              <div className="space-y-3">
                {loading ? (
                  ...
                  data.inventory.map((p: StockRow) => {
                    ...
                    return (
                      <div key={p.sku} className="p-3 rounded-xl flex items-center justify-between" style={{ backgroundColor: '#141414', border: `1px solid ${COLORS.black}` }}>
                        <div>
                          <div className="font-medium" style={{ color: COLORS.champagne }}>
                            {p.name}
                          </div>
                        ...
                        <div className="text-right">
                          <div className="text-sm" style={{ color: danger ? COLORS.rose : COLORS.champagne }}>
                            Stock {p.stock}
                          </div>
                          <div className="text-xs opacity-70">Min {p.min}
            ...
            <SoftCard>
              <SectionTitle title="Client Sentiment (NPS)" icon={<Star size={18} color={COLORS.gold} />} />
              <div className="flex items-center justify-center h-40">
                <div
                  className="relative h-32 w-32 rounded-full flex items-center justify-center"
                  style={{ background: `conic-gradient(${COLORS.emerald} ${Math.max(0, data.nps.score)}%, ${COLORS.rose} 0)` }}
                >
                  <div className="absolute inset-2 rounded-full" style={{ backgroundColor: COLORS.charcoal }} />
                  <div className="relative text-3xl font-semibold" style={{ color: COLORS.champagne }}>
                    {data.nps.score}
                  </div>
                </div>
              </div>
          ...
          <SoftCard>
            <SectionTitle title="Recent Transactions" icon={<Receipt size={18} color={COLORS.gold} />} right={<div className="text-xs" style={{ color: COLORS.bronze }}>Last 10 • Smart Code routed</div>} />
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="text-left" style={{ color: COLORS.bronze }}>
                    ...
                    <tr key={t.id} className="border-t" style={{ borderColor: '#1f1f1f' }}>
                      ...
                      <td className="text-right">
                        <Button
                          ...
                          size="sm"
                          className="border"
                          style={{ borderColor: COLORS.bronze, background: 'transparent', color: COLORS.champagne }}
          ...
          <div className="flex items-center justify-between py-4">
            <div className="text-xs opacity-70">
              Smart Code: <span style={{ color: COLORS.bronze }}>HERA.SALON.DASHBOARD.OWNER.V1</span>
            </div>
            <div className="flex gap-2">
              <GradientButton>New Booking</GradientButton>
              <Button variant="outline" className="border" style={{ borderColor: COLORS.bronze, color: COLORS.champagne, backgroundColor: 'transparent' }}>
                Export Report
              </Button>
            </div>
          </div>
        ...
        {/* Drill-down Dialog */}
        <Dialog open={!!openTxn} onOpenChange={(v) => { if (!v) { setOpenTxn(null); setOpenTxnLines(null) } }}>
          <DialogContent className="sm:max-w-2xl z-50" style={{ backgroundColor: COLORS.charcoal, color: COLORS.champagne, border: `1px solid ${COLORS.black}` }}>
            <DialogHeader>
              ...
              <DialogDescription className="text-xs" style={{ color: COLORS.bronze }}>
                {openTxn?.smart_code} • {openTxn && shortDate(openTxn.transaction_date || openTxn.created_at)}
              </DialogDescription>
            </DialogHeader>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
            ...
            {openTxn && (
              <div className="flex items-center justify-between text-xs mt-3">
                ...
                <div>
                  Total: <span style={{ color: COLORS.champagne }}>{fmtCurrency(openTxn.total_amount, openTxn.currency)}</span>
                </div>
              </div>
            )}
          </DialogContent>
