...
export default function SalonStaffV2Page() {
  ...
  const getYearsOfService = (hireDate: string) => {
    ...
    const years = Math.floor((Date.now() - new Date(hireDate).getTime()) / (365 * 24 * 60 * 60 * 1000))
    return years
  }
  ...
  return (
    <SalonLuxeCRUDPage
      ...
      renderCard={(staff, handlers) => {
        ...
        const specialties = staff.dynamic_fields?.specialties?.value?.split(',').map(s => s.trim()) || []
            ...
            footer={
              <div className="space-y-1">
                {staff.dynamic_fields?.email?.value && (
                  <div className="text-xs flex items-center gap-2" style={{ color: LUXE_COLORS.bronze }}>
                    <span>✉️</span>
