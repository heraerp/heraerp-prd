...
import { Users, Phone, Mail, Calendar, DollarSign, Star, Heart, MapPin, Clock, Gift } from 'lucide-react'
  ...
  return (
    <SalonLuxeCRUDPage
      ...
      renderCard={(customer, handlers) => {
        ...
        const tags = customer.dynamic_fields?.tags?.value?.split(',').map(t => t.trim()).filter(Boolean) || []
        ...
        return (
          <SalonLuxeCard
            ...
            subtitle={customer.dynamic_fields?.preferred_stylist?.value ? 
              `Prefers ${customer.dynamic_fields.preferred_stylist.value}` : undefined}
            icon={Users}
            ...
            footer={
              <div className="space-y-2">
                <div className="flex items-center gap-4 text-xs" style={{ color: LUXE_COLORS.bronze }}>
                ...
                {lastVisit && (
                  <div className="flex items-center gap-1 text-xs" style={{ color: LUXE_COLORS.bronze }}>
                    <Clock className="h-3 w-3" />
                    <span>Last visit: {daysSince === 0 ? 'Today' : 
                      daysSince === 1 ? 'Yesterday' : 
                      daysSince < 7 ? `${daysSince} days ago` :
                      daysSince < 30 ? `${Math.floor(daysSince / 7)} weeks ago` :
                      `${Math.floor(daysSince / 30)} months ago`}</span>
                  </div>
                )}
                ...
                {customer.dynamic_fields?.date_of_birth?.value && (
                  <div className="flex items-center gap-1 text-xs" style={{ color: LUXE_COLORS.bronze }}>
                    ...
                    <span>Birthday: {format(new Date(customer.dynamic_fields.date_of_birth.value), 'MMM d')}</span>
                  </div>
                )}
              </div>
            }
