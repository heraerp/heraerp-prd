/**
 * HERA Salon Access - Redirect to /salon/auth
 *
 * This page redirects to the canonical authentication URL: /salon/auth
 *
 * Enterprise-grade authentication flow:
 * - All authentication happens at /salon/auth (within salon namespace)
 * - This maintains consistent URL structure
 * - Backward compatibility for any external links
 *
 * Migration Note:
 * - OLD: /salon-access (outside salon namespace)
 * - NEW: /salon/auth (canonical, enterprise-grade)
 */
...
export default function SalonAccessRedirectPage() {
  const router = useRouter()

  useEffect(() => {
    // Redirect to canonical salon auth page
    console.log('🔀 Redirecting /salon-access → /salon/auth (canonical URL)')
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          Redirecting to Enterprise Login
        </h2>
        <p className="text-sm" style={{ color: SALON_LUXE_COLORS.bronze }}>
          Taking you to the secure authentication portal...
        </p>
      </div>
