...
export default function SalonDashboard() {
  ...
  const { organization: orgContext, isLoading: orgLoading } = useOrganizationContext()

  // Determine which organization to use
  // Priority: 1. Authenticated org, 2. Subdomain org, 3. Demo org
  const organizationId = currentOrganization?.id || orgContext?.organizationId || null
  const organizationName = currentOrganization?.name || orgContext?.organizationName || 'Demo Salon & Spa'
  const isDemo = !currentOrganization && orgContext?.isDemo
  ...
  // If no organization context is available
  if (!organizationId) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <Card className="max-w-md">
          <CardContent className="pt-6">
            <Alert>
              <AlertDescription>
                No organization context found. Please select an organization or use the demo.
              </AlertDescription>
            </Alert>
            <div className="mt-4 space-y-2">
              <Button onClick={() => router.push('/auth/organizations')} className="w-full">
                Select Organization
              </Button>
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-4">
              {isDemo && (
                <Badge variant="outline" className="bg-amber-50 border-amber-300 text-amber-700 px-4 py-2">
