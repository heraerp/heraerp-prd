...
export default function TransactionEntryPage() {
  const [messages, setMessages] = useState<Message[]>([
    {
      ...
      type: 'assistant',
      content:
        'Hi! I\'m your digital accountant. Tell me about any financial transaction in plain English, and I\'ll handle the accounting for you. For example: "Paid stylist Sarah $2,500 salary" or "Received $180 for hair color service".',
      timestamp: new Date()
    }
  ])
  ...
  const parseNaturalLanguage = async (text: string): Promise<ParsedTransaction | null> => {
    ...
    let smartCode = 'HERA.SALON.FINANCE.TXN.EXPENSE.GENERAL.V1'

    if (text.toLowerCase().includes('salary') || text.toLowerCase().includes('payroll')) {
      category = 'salary'
      smartCode = 'HERA.SALON.FINANCE.TXN.EXPENSE.SALARY.V1'
    } else if (text.toLowerCase().includes('commission')) {
      category = 'commission'
      smartCode = 'HERA.SALON.FINANCE.TXN.EXPENSE.COMMISSION.V1'
    } else if (text.toLowerCase().includes('rent')) {
      category = 'rent'
      smartCode = 'HERA.SALON.FINANCE.TXN.EXPENSE.RENT.V1'
    } else if (text.toLowerCase().includes('product') || text.toLowerCase().includes('supplies')) {
      category = 'supplies'
      smartCode = 'HERA.SALON.FINANCE.TXN.EXPENSE.SUPPLIES.V1'
    } else if (
      ...
      type = 'revenue'
      category = 'service'
      smartCode = 'HERA.SALON.FINANCE.TXN.REVENUE.SERVICE.V1'
    } else if (
      ...
      category = 'utilities'
      smartCode = 'HERA.SALON.FINANCE.TXN.EXPENSE.UTILITIES.V1'
    }
  ...
  const processTransaction = async (transaction: ParsedTransaction): Promise<boolean> => {
    // Mock API call to MDA system
    try {
      const ufe = {
        apiVersion: 'v2',
        organization_id: 'demo-salon-org-uuid',
        transaction_type:
          transaction.type === 'revenue' ? 'TX.FINANCE.REVENUE.V1' : 'TX.FINANCE.EXPENSE.V1',
        ...
        metadata: {
          ingest_source: 'NL_Interface',
          original_ref: `NL-${Date.now()}`
        },
        lines: []
      }

      // In production, this would call the actual MDA API
      console.log('Would post to MDA:', ufe)
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      // Process transaction
      const success = await processTransaction(parsed)
      ...
      const resultMessage: Message = {
        ...
        type: 'assistant',
        content: success
          ? `✅ Successfully posted ${parsed.type} transaction of $${parsed.amount} for ${parsed.category}. The GL entries have been created automatically.`
          : `❌ Failed to process the transaction. Please check the details and try again.`,
        timestamp: new Date(),
        transaction: parsed,
        ...
        <div className="px-4 md:px-6 lg:px-8">
          ...
          >
            ...
            <div className="lg:col-span-3">
              <LuxeCard variant="glass" className="h-[600px] flex flex-col">
                {/* Messages */}
                  ...
                  <div className="flex items-center space-x-3">
                    <div className="flex-1 relative">
                      <Input
                        ...
                        className="pr-12 rounded-xl border-gray-300 dark:border-gray-600"
