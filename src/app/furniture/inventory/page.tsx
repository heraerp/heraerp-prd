...
import { InventoryPageSkeleton } from '@/components/furniture/InventoryPageSkeleton'
...
const stockColumns = [
  ...
  },
  {
    ...
    sortable: true,
    align: 'right' as const,
    render: (value: number) => <span className="font-mono text-muted-foreground">{value || 0}</span>
  },
  {
    key: 'available',
    label: 'Available',
    ...
    render: (value: number, row: any) => {
      ...
      return <span className="font-mono font-medium text-foreground">{available}</span>
    }
  },
  {
    key: 'in_transit',
    ...
    sortable: true,
    align: 'right' as const,
    render: (value: number) => <span className="font-mono text-cyan-400">{value || 0}</span>
  },
  {
    key: 'reorder_point',
    label: 'Reorder Point',
...
const movementColumns = [
  ...
  },
  {
    ...
    render: (value: string, row: any) => (
      <div>
        ...
        <p className="text-sm text-muted-foreground">{row.entity_code}</p>
      </div>
    )
  },
  {
  ...
  },
  {
    ...
    align: 'right' as const,
    render: (value: number, row: any) => {
      const isIncoming = ['purchase_receipt', 'production_output', 'stock_adjustment'].includes(
...
export default function FurnitureInventory() {
  ...
  // Use the optimized inventory data hook
  const { stockData, movements, stats, loading, error, refresh, filterStock, filterMovements } =
    useInventoryData(organizationId)

  // Memoize filtered data
  const filteredStock = useMemo(() => {
    return filterStock(searchTerm, selectedLocation, selectedCategory)
  }, [stockData, searchTerm, selectedLocation, selectedCategory, filterStock])

  const filteredMovements = useMemo(() => {
    return filterMovements(searchTerm)
  }, [movements, searchTerm, filterMovements])
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-center">
          <p className="text-red-400 mb-4">Error loading inventory data: {error}</p>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-4">
          <div className="flex justify-between items-center">
            <TabsList className="bg-muted border-border">
              <TabsTrigger value="overview" className="data-[state=active]:bg-muted-foreground/10">
                <Warehouse className="h-4 w-4 mr-2" />
                Stock Overview
              </TabsTrigger>
              <TabsTrigger value="movements" className="data-[state=active]:bg-muted-foreground/10">
          ...
          <TabsContent value="movements" className="space-y-4">
            <EnterpriseDataTable
              ...
              emptyState={{
                ...
                description:
                  'Stock movements will appear here when products are received or shipped.'
              }}
              className="bg-card/50 border-border"
            />
          </TabsContent>
          ...
          <TabsContent value="analytics" className="space-y-4">
            <div className="grid grid-cols-2 gap-6">
              ...
              <Card className="p-6 bg-card/50 border-border">
                <h3 className="text-lg font-semibold mb-4 text-foreground">Movement Trends</h3>
                <div className="h-64 flex items-center justify-center text-muted-foreground">
