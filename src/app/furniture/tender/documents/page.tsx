...
const documentCategories = [
  ...
  { value: 'bid_template', label: 'Bid Templates' },
  ...
  { value: 'experience', label: 'Experience Certificates' },
...
const mockDocuments: Document[] = [
  {
    id: '1',
    entity_code: 'DOC/001',
    entity_name: 'Standard Bid Template',
    metadata: {
      file_name: 'standard_bid_template_v3.docx',
      file_size: 245760,
      file_type: 'application/vnd.openxmlformats-officedocument.wordprocessingml.document',
      category: 'bid_template',
      description: 'Standard template for furniture tender bids',
      uploaded_by: 'Admin',
      version: 3,
      tags: ['template', 'bid', 'standard'],
      is_template: true
    },
  ...
  },
  {
    id: '2',
    entity_code: 'DOC/002',
    entity_name: 'Company Registration Certificate',
    metadata: {
      file_name: 'company_registration_2024.pdf',
      file_size: 1048576,
      file_type: 'application/pdf',
      category: 'compliance',
      description: 'Kerala Furniture Works registration certificate',
      uploaded_by: 'Admin',
  ...
  },
  {
    id: '4',
    entity_code: 'DOC/004',
    entity_name: 'Technical Capability Statement',
    metadata: {
      file_name: 'technical_capability_2025.pdf',
      file_size: 2097152,
      file_type: 'application/pdf',
      category: 'technical_docs',
      description: 'Detailed technical capabilities and machinery list',
      uploaded_by: 'Operations',
      tags: ['technical', 'capability'],
      is_template: false
    },
  ...
  },
  {
    id: '5',
    entity_code: 'DOC/005',
    entity_name: 'Financial Statement Template',
    metadata: {
      file_name: 'financial_statement_template.xlsx',
      file_size: 327680,
      file_type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
      category: 'financial_docs',
      description: 'Template for financial statements submission',
      uploaded_by: 'Finance Team',
      version: 2,
      tags: ['template', 'financial'],
      is_template: true
    },
...
export default function DocumentsPage() {
  ...
  const handleDuplicate = (doc: Document) => {
    const newDoc: Document = {
      ...
      entity_code: `DOC/${Date.now()}`,
      entity_name: `Copy of ${doc.entity_name}`,
      metadata: {
        ...doc.metadata,
        file_name: `copy_of_${doc.metadata.file_name}`,
        version: (doc.metadata.version || 1) + 1
      },
  ...
  const categoryStats = documentCategories.map(cat => ({
    ...cat,
    count: documents.filter(doc => doc.metadata.category === cat.value).length
  }))
  ...
  return (
    <div>
      <FurniturePageHeader
        ...
        subtitle="Centralized repository for all tender documents and templates"
        actions={
          <Button>
      ...
      {/* Category Stats */}
      <div className="bg-[var(--color-body)] grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-4 mb-6">
        {categoryStats.map(cat => (
          ...
          >
            <div className="bg-[var(--color-body)] text-center">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          <TabsList>
            ...
            <TabsTrigger value="templates">
              Templates ({documents.filter(d => d.metadata.is_template).length})
            </TabsTrigger>
            <TabsTrigger value="documents">
              Documents ({documents.filter(d => !d.metadata.is_template).length})
            </TabsTrigger>
          </TabsList>
                    ...
                    <TableRow key={doc.id}>
                          ...
                          <div>
                            ...
                            <div className="flex gap-2 mt-2">
                              ...
                              {doc.metadata.version && doc.metadata.version > 1 && (
                                <Badge variant="outline" className="bg-[var(--color-body)] text-xs">
                      ...
                      <TableCell>
                        <div className="bg-[var(--color-body)] flex items-center gap-2">
                          <User className="h-4 w-4 text-[var(--color-icon-secondary)]" />
      ...
      <Card className="bg-[var(--color-surface-raised)] border-[var(--color-border)] p-6 mt-6">
        <h3 className="bg-[var(--color-body)] text-lg font-semibold mb-4">Quick Actions</h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <Button variant="outline" className="justify-start hover:bg-[var(--color-hover)]">
            <FileCheck className="h-4 w-4 mr-2" />
            Verify All Compliance Documents
          </Button>
          <Button variant="outline" className="justify-start hover:bg-[var(--color-hover)]">
            <FolderOpen className="h-4 w-4 mr-2" />
            Create Document Package for Bid
          </Button>
          <Button variant="outline" className="justify-start hover:bg-[var(--color-hover)]">
