...
const monthlyBidData = [
  { month: 'Sep', submitted: 12, won: 5, lost: 7, value: 45 },
  { month: 'Oct', submitted: 15, won: 7, lost: 8, value: 62 },
...
const winRateByCategory = [
  ...
  { category: 'Rosewood', winRate: 45, totalBids: 12 },
...
const competitorAnalysis = [
  ...
  { name: 'XYZ Wood', share: 22, color: '#4ecdc4' },
      ...
      <div className="min-h-screen bg-[var(--color-body)] flex items-center justify-center">
        <div className="text-center">
          <div className="inline-flex items-center space-x-2">
            ...
            <p className="text-[var(--color-text-secondary)]">Loading tender dashboard...</p>
          </div>
        </div>
      </div>
    )
    ...
    <div className="min-h-screen bg-[var(--color-body)]">
      <div className="p-6 space-y-6">
        ...
        <FurniturePageHeader
          ...
          subtitle="Performance insights and competitive analysis"
          actions={
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="bg-[var(--color-body)] space-y-4">
          ...
          <TabsContent value="overview" className="bg-[var(--color-body)] space-y-6">
                  ...
                  {winRateByCategory.map(item => (
                    <div key={item.category} className="bg-[var(--color-body)] space-y-2">
                      ...
                      <div className="h-2 bg-muted-foreground/10 rounded-full overflow-hidden">
                        <div
                          className="h-full bg-gradient-to-r from-[var(--color-accent-teal)] to-[var(--color-accent-teal)] transition-all"
                          style={{ width: `${item.winRate}%` }}
                        />
                      </div>
                      <p className="text-xs text-[var(--color-text-secondary)]">
                        {item.totalBids} bids submitted
                      </p>
                    </div>
                  ))}
                </div>
              ...
              <Card className="bg-[var(--color-surface-raised)] border-[var(--color-border)] p-6 bg-[var(--color-body)]/70 backdrop-blur-sm border-[var(--color-border)]/50">
                ...
                <div className="grid grid-cols-2 gap-2 mt-4">
                  {competitorAnalysis.map(item => (
                    <div key={item.name} className="bg-[var(--color-body)] flex items-center gap-2">
                      <div
                        className="bg-[var(--color-body)] w-3 h-3 rounded-full"
            ...
            {/* Recent Alerts */}
            <Card className="bg-[var(--color-surface-raised)] border-[var(--color-border)] p-6 bg-gradient-to-br from-amber-900/20 to-orange-900/20 border-amber-700/50">
              <div className="flex items-start gap-4">
                <div className="p-3 rounded-xl bg-[var(--color-accent-indigo)]/20">
                  <AlertCircle className="h-6 w-6 text-[var(--color-icon-secondary)]" />
                </div>
                <div className="flex-1">
                  <h4 className="bg-[var(--color-body)] font-semibold text-[var(--color-text-primary)]">
          ...
          <TabsContent value="trends" className="bg-[var(--color-body)] space-y-6">
            <Card className="bg-[var(--color-surface-raised)] border-[var(--color-border)] p-6 bg-[var(--color-body)]/70 backdrop-blur-sm border-[var(--color-border)]/50">
              <h3 className="bg-[var(--color-body)] text-lg font-semibold text-[var(--color-text-primary)] mb-4">
          ...
          <TabsContent value="ai-insights" className="bg-[var(--color-body)] space-y-6">
            <Card className="bg-[var(--color-surface-raised)] border-[var(--color-border)] p-6 bg-gradient-to-br from-[var(--color-accent-teal)]/20 to-[var(--color-accent-teal)]/20 border-[var(--color-accent-teal)]/50">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="p-4 rounded-lg bg-muted-foreground/10">
                  <p className="text-sm text-[var(--color-text-secondary)]">Prediction Accuracy</p>
                  <p className="text-2xl font-bold text-[var(--color-text-primary)] mt-1">78%</p>
          ...
          <TabsContent value="all-tenders" className="bg-[var(--color-body)] space-y-6">
            <TenderListTable />
          </TabsContent>
        </Tabs>
      </div>
