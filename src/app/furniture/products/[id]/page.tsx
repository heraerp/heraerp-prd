...
const productId = params.id as string const { currentOrganization } = useMultiOrgAuth()

const [loading, setLoading] = useState(true)

const [product, setProduct] = useState<any>(null)

const [dynamicData, setDynamicData] = useState<any[]>([]) useEffect(() => { if (currentOrganization?.id && productId) { loadProductData() } }, [currentOrganization?.id, productId])

const loadProductData = async () => { try { setLoading(true) // Get product details const { data: entities } = await universalApi.read({ table: 'core_entities', organizationId: currentOrganization!.id })

const productData = entities?.find((e: any) => e.id === productId) if (!productData) throw new Error('Product not found') setProduct(productData) // Get dynamic data const { data: dynamicFields } = await universalApi.read({ table: 'core_dynamic_data', organizationId: currentOrganization!.id })
