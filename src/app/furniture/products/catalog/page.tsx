...
// Use the optimized products data hook
  ...
  const Icon = categoryIcons[row.category as keyof typeof categoryIcons] || Package 
    ...
    } }, { key: 'category', label: 'Category', sortable: true, render: (value: string) => { const colorClass = categoryColors[value as keyof typeof categoryColors] || 'bg-gray-9000/20 text-[var(--color-text-secondary)]' 
    return (
  ...
  const level =value || 0
  const color = level > 20 ? 'text-green-600 dark:text-green-400' : level > 10 ? 'text-[var(--color-accent-indigo)] dark:text-[var(--color-text-secondary)]' : 'text-red-600 dark:text-red-400' 
...
const handleExport = () => { // Export functionality console.log('Exporting products...'  );
  const stats = [ { label: 'Total Products', value: products.length, icon: Package, color: 'text-[var(--color-text-primary)]' }, { label: 'Active SKUs', value: products.filter(p => p.status === 'active').length, icon: BarChart, color: 'text-green-500' }, { label: 'Low Stock', value: products.filter(p => (p.stock_quantity || 0) < 10).length, icon: AlertCircle, color: 'text-[var(--color-text-primary)]' }, { label: 'Total Value', value: `$${products.reduce((sum, p) => sum + (p.stock_quantity || 0) * (p.cost || 0), 0).toLocaleString()}`, icon: BarChart, color: 'text-[var(--color-text-primary)]' } ] // Show loading state while organization is loading if (orgLoading) {  return <FurnitureOrgLoading /> }
