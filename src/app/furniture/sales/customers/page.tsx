...
export default function CustomersPage() {
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="p-5">
            <div className="flex items-center">
              <div className="flex-shrink-0 bg-[var(--color-body)] rounded-md p-3">
                <Building2 className="h-6 w-6 text-[var(--color-icon-secondary)]" />
              </div>
      ...
      {/* Search and Filters */}
      <div className="bg-[var(--color-surface-raised)] shadow rounded-lg">
        <div className="p-4 border-b border-[var(--color-border)]">
          <div className="flex flex-col sm:flex-row gap-4">
            <div className="flex-1 relative">
              <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                <Search className="h-5 w-5 text-[var(--color-icon-secondary)]" />
              </div>
              <input
                type="text"
                value={searchTerm}
                onChange={e => setSearchTerm(e.target.value)}
                className="block w-full pl-10 pr-3 py-2 border border-[var(--color-border)] rounded-md leading-5 bg-[var(--color-body)] placeholder-gray-500 focus:outline-none focus:placeholder-gray-400 focus:ring-1 focus:ring-[var(--color-accent-indigo)] focus:border-[var(--color-accent-teal)] sm:text-sm"
              ...
              <button
                ...
                className={`px-4 py-2 text-sm font-medium rounded-md ${
                  filterType === 'all'
                    ...
                    : 'text-[var(--color-text-primary)] bg-muted-foreground/10 hover:bg-[var(--color-body)]'
                }`}
              >
                All
              </button>
        ...
        <div className="overflow-x-auto furniture-scrollbar">
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-[var(--color-body)]">
              <tr>
            ...
            <tbody className="bg-[var(--color-surface-raised)] divide-y divide-gray-200">
              {customersLoading ? (
                <tr>
                  <td colSpan={7} className="px-6 py-4 text-center">
                    <div className="animate-spin rounded-full h-6 w-6 border-b-2 border-[var(--color-accent-indigo)] mx-auto"></div>
                  </td>
                </tr>
                    ...
                    <tr key={customer.id} className="hover:bg-[var(--color-body)]">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <div className="flex items-center justify-end gap-2">
                          <Link
                            href={`/furniture/sales/customers/${customer.id}`}
                            className="text-[var(--color-accent-indigo)] hover:text-amber-900"
                          >
      ...
      {showDeleteModal && (
        <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center p-4 z-50">
          <div className="bg-[var(--color-surface-raised)] rounded-lg max-w-md w-full p-6 space-y-4">
            ...
            <div className="flex justify-end gap-3">
              <button
                ...
                className="px-4 py-2 text-sm font-medium text-[var(--color-text-primary)] bg-muted-foreground/10 border border-[var(--color-border)] rounded-md hover:bg-[var(--color-body)]"
              >
                Cancel
              </button>
              <button
