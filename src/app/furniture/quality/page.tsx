...
import { cn } from '@/src/lib/utils' // Inspection status configurations
...
// Inspection columns
  ...
  const types = { incoming: 'Incoming', in_process: 'In-Process', final: 'Final', random: 'Random Sample' } 
    return <span>{types[value as keyof typeof types] || value}</span> } }, { key: 'inspector_name', label: 'Inspector', sortable: true, render: (value: string) => ( <div className="flex items-center gap-2"> <User className="h-4 w-4 text-[#37353E]" /> <span>{value}</span>
      </div>
  ...
  const config =inspectionStatuses[value as keyof typeof inspectionStatuses] || inspectionStatuses.pending const Icon = config.icon 
    return (
...
const status = statuses[value as keyof typeof statuses] || statuses.open 
    return <span className={status.color}>{status.label}</span> } }, { key: 'corrective_action', label: 'Corrective Action', render: (value: string) => <span className="text-sm">{value || '-'}</span> }
]
...
const [selectedStatus, setSelectedStatus] = useState('all')
          // Quality metrics
  const [metrics, setMetrics] = useState({ overallQualityRate: 0, inspectionsToday: 0, openIssues: 0, avgResolutionTime: 0 }) useEffect(() => { if (organizationId && !orgLoading) {
...
const statCards = [ { label: 'Quality Rate', value: `${metrics.overallQualityRate}%`, icon: Award, color: 'text-green-500', trend: '+2%', trendUp: true, description: 'Overall pass rate' }, { label:"Today's Inspections", value: metrics.inspectionsToday, icon: ClipboardCheck, color: 'text-[var(--color-text-primary)]', trend: '+5', trendUp: true, description: 'Completed today' }, { label: 'Open Issues', value: metrics.openIssues, icon: AlertTriangle, color: 'text-[var(--color-text-primary)]', trend: '-3', trendUp: false, description: 'Pending resolution' }, { label: 'Avg Resolution Time', value: `${metrics.avgResolutionTime} days`, icon: Clock, color: 'text-[var(--color-text-primary)]', trend: '-0.5 days', trendUp: false, description: 'Time to resolve' } ] // Quality standards data 
  const qualityStandards = [  { standard: 'ISO 9001:2015', status: 'Certified', expiry: '2026-12-31' },`
  ...
  const count =issues.filter(i => i.issue_type === key).length const percentage = issues.length > 0 ? Math.round((count / issues.length) * 100) : 0 
    return (
