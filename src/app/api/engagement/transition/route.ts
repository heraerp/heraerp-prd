...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!body.skip_criteria_check) {
      ...
      const getToStageFieldValue = (fieldName: string, type: 'text' | 'number' | 'json' = 'text') => {
      ...
      if (entryCriteria) {
        ...
        if (entryCriteria.required_actions && entryCriteria.required_actions.length > 0) {
          ...
          const completedActions = new Set(
            (scoreHistory as any[]).map(event => event.action)
          );
    ...
    const toStageOrdinal = toStageDynamicData?.find(
      (d: any) => d.field_name === 'ordinal'
