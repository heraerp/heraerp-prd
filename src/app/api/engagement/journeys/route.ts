...
export async function GET(request: NextRequest) {
  try {
    ...
    const scoreRange = searchParams.get('score_range') ? JSON.parse(searchParams.get('score_range')!) : null;
    const dateRange = searchParams.get('date_range') ? JSON.parse(searchParams.get('date_range')!) : null;
    ...
    const transformedJourneys: EngagementJourney[] = await Promise.all(
      (journeys || []).map(async (journey) => {
        ...
        return {
          ...
          subject_type: getFieldValue('subject_type') as 'constituent' | 'organization' || 'constituent',
    ...
    if (stageIds?.length) {
      filteredJourneys = filteredJourneys.filter(j => 
        stageIds.includes(j.current_stage_id)
      );
    }
    
    if (subjectType) {
      filteredJourneys = filteredJourneys.filter(j => 
        j.subject_type === subjectType
      );
    }
    
    if (scoreRange) {
      filteredJourneys = filteredJourneys.filter(j => 
        j.score >= scoreRange.min && j.score <= scoreRange.max
      );
    }
    ...
    return NextResponse.json(
      { error: 'Failed to fetch engagement journeys' },
      { status: 500 }
    );
  }
}