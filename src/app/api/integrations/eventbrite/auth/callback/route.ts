...
export async function GET(request: NextRequest) {
  try {
    ...
    if (isDemo) {
      ...
      const connectorResponse = await fetch(`${request.nextUrl.origin}/api/v2/universal/entity-upsert`, {
        method: 'POST',
        headers: {
    ...
    return NextResponse.redirect(
      new URL('/civicflow/communications/integrations?error=production_not_implemented', request.url)
    );
  } catch (error) {
