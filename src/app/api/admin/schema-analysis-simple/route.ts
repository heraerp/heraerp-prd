  ...
  try {
    ...
    for (const tableName of CORE_TABLES) {
      try {
        ...
        if (sampleData && sampleData.length > 0 && 'smart_code' in sampleData[0]) {
          const smartCodes = sampleData.map(row => row.smart_code).filter(code => code !== null)
    ...
    if (analysis.insights.branches.length === 0) {
      analysis.summary.recommendations.push({
        type: 'info',
        message:
          'No branch entities found. Branches might be stored as dynamic data or relationships.'
      })
    }
