...
import { EntityUpsertBody, EntityProfilesQuery, SearchParams } from '@/lib/universal/schemas'
    ...
    if (createProcedures.length > 0) {
      ...
      if (dynamicFields.length > 0 && result.data?.entity_id) {
        ...
        for (const [field_name, value] of dynamicFields) {
          const fieldType =
            dynamicSchema.field_schemas.find(f => f.field_name === field_name)?.field_type || 'text'
...
export async function GET(request: NextRequest) {
  try {
    ...
    const queryParams: any = {
      ...
      p_parent_entity_id:
        searchParams.get('p_parent_entity_id') || searchParams.get('parent_entity_id'),
    ...
    return NextResponse.json({
      ...
      _links: {
        ...
        next:
          page < totalPages
            ? `/api/universal/entities?${new URLSearchParams({ ...Object.fromEntries(searchParams), page: String(page + 1) }).toString()}`
            : null,
        prev:
          page > 1
            ? `/api/universal/entities?${new URLSearchParams({ ...Object.fromEntries(searchParams), page: String(page - 1) }).toString()}`
