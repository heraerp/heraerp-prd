...
import { GuardrailViolation, isValidUUID } from '@/lib/universal/guardrails';
  ...
  try {
    ...
    if (validatedBase.p_smart_code) {
      ...
      if (updateProcedures.length > 0) {
        ...
        const result = await EntityBuilder.update(
          entityId,
          body,
          validatedBase.p_organization_id
        );
...
export async function DELETE(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const entityResult = await callRPC('hera_entity_get_v1', 
