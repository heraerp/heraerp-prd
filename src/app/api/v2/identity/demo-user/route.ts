...
const SVC_KEY = (process.env.SUPABASE_SERVICE_ROLE as string) || (process.env.SUPABASE_SERVICE_ROLE_KEY as string) as string; // service key required
...
export async function POST(req: NextRequest) {
  if (!SVC_URL || !SVC_KEY) {
    return NextResponse.json({ error: "SERVER_MISCONFIGURED: missing Supabase service keys" }, { status: 500 });
  }
...
async function ensureUserAppRel(sb: any, orgId: string, userEntityId: string, appEntityId: string, role?: string) {
  const { data: exists, error } = await sb
    .from("core_relationships")
    .select("id")
    .eq("organization_id", orgId)
...
async function resolveBranchEntity(sb: any, orgId: string, branch: BranchInput): Promise<string | null> {
...
async function ensureUserBranchRel(sb: any, orgId: string, userEntityId: string, branchEntityId: string, role?: string) {
  const { data: exists, error } = await sb
    .from("core_relationships")
    .select("id")
    .eq("organization_id", orgId)
...
async function ensureAuthToEntityMap(sb: any, orgId: string, authUserId: string, entityUserId: string) {
