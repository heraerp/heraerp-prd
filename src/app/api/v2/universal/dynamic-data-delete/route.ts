...
export async function DELETE(req: NextRequest) {
  ...
  const { organization_id, entity_id, field_name, field_id, delete_all_fields = false } = body
  ...
  if (!field_id && !entity_id) {
    return NextResponse.json({ error: 'Either field_id or entity_id is required' }, { status: 400 })
  }
  ...
  try {
    ...
    if (field_id) {
      ...
      return NextResponse.json(
        {
          error:
            'Invalid delete parameters. Provide field_id, or entity_id with field_name, or entity_id with delete_all_fields=true'
        },
        { status: 400 }
      )
    }
