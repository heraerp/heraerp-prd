...
interface UniversalLearningRequest {
  action: 'process_content' | 'analyze_content' | 'create_entities' | 'generate_paths' | 'specialize_domain' | 'cross_domain_intelligence' | 'complete_pipeline';
  domain: string;
  content?: string;
  files?: File[];
  options?: {
...
export async function POST(request: NextRequest): Promise<NextResponse<UniversalLearningResponse>> {
  try {
    ...
    if (contentType.includes('multipart/form-data')) {
      ...
      const finalContent = textContent + (extractedContent ? '\n\n--- FILE CONTENT ---\n\n' + extractedContent : '');
    ...
    console.log('🔍 [DEBUG] Request body parsed:', { action: body.action, domain: body.domain, contentLength: body.content?.length });
    
    const { action, domain, content, files, options = {}, metadata = {} } = body;

    console.log(`🚀 HERA Universal Learning API - ${action.toUpperCase()} for ${domain.toUpperCase()}`);
    console.log(`📊 Options:`, Object.keys(options).filter(key => options[key]).join(', '));
    ...
    switch (action) {
      case 'process_content':
        ...
        result = await processor.processTextContent(
          content || '',
          domain,
          {
      ...
      case 'analyze_content':
        ...
        const processedContent = await processor.processTextContent(
          content,
          domain,
          {
        ...
        processingMetadata = {
          ...
          universal_applicability: (result.universalElements?.length > 0) 
            ? result.universalElements.reduce((avg, el) => avg + (el.universalApplicability || 0), 0) / result.universalElements.length
            : 0.7,
          domain_specialization: (result.domainSpecificElements?.length || 0) / ((result.universalElements?.length || 0) + (result.domainSpecificElements?.length || 0) + 1),
      ...
      case 'create_entities':
        ...
        const entityProcessedContent = await processor.processTextContent(
          content,
          domain,
          { source: 'api_entity_creation', author: 'API', subject: domain, grade_level: 'Professional', language: 'English' }
        );
      ...
      case 'generate_paths':
        ...
        const pathProcessedContent = await processor.processTextContent(
          content,
          domain,
          { source: 'api_path_generation', author: 'API', subject: domain, grade_level: 'Professional', language: 'English' }
        );
        ...
        processingMetadata = {
          ...
          total_learning_steps: result.reduce((total, path) => total + path.learningSequence.length, 0),
      ...
      case 'specialize_domain':
        ...
        const specProcessedContent = await processor.processTextContent(
          content,
          domain,
          { source: 'api_specialization', author: 'API', subject: domain, grade_level: 'Professional', language: 'English' }
        );
        ...
        const specPathResult = await pathGenerator.generateUniversalLearningPaths(
          specAnalysisResult,
          specEntityResult,
          { targetAudience: 'mixed', learningStyle: 'adaptive', timeConstraint: 'flexible', assessmentFrequency: 'adaptive', domainFocus: domain, crossDomainInsights: true, personalization: true, gamification: true }
        );
      ...
      case 'complete_pipeline':
        console.log(`🔄 Running complete Universal Learning Pipeline for ${domain.toUpperCase()}...`);
        ...
        const pipelineProcessedContent = await processor.processTextContent(
          content,
          domain,
          {
        ...
        if (options.cross_domain_insights !== false) {
          ...
          const crossDomainData = new Map([[domain, {
            analysis: pipelineAnalysisResult,
            specialization: pipelineSpecialization,
            learningPaths: pipelineLearningPaths
          }]]);
          pipelineCrossDomainIntelligence = await crossDomainEngine.learnFromAllDomains(crossDomainData);
        }
        ...
        result = {
          ...
          pipeline_summary: {
            ...
            total_learning_steps: pipelineLearningPaths.reduce((total, path) => total + path.learningSequence.length, 0),
    ...
    console.log(`✅ HERA Universal Learning API - ${action.toUpperCase()} completed in ${processingMetadata.processing_time}ms`);
    console.log(`📊 Confidence: ${processingMetadata.confidence_score?.toFixed(2)}, Universal: ${processingMetadata.universal_applicability?.toFixed(2)}, Domain: ${processingMetadata.domain_specialization?.toFixed(2)}`);
    ...
    switch (action) {
        ...
        return NextResponse.json({
          ...
          supported_domains: {
            'CA': {
              ...
              specializations: ['Audit', 'Taxation', 'Financial Reporting', 'Management Accounting', 'Corporate Law']
            },
            'MED': {
              name: 'Medical Education',
              description: 'Healthcare and clinical learning',
              certifications: ['USMLE', 'Medical Board Exams', 'Specialty Certifications'],
              specializations: ['Clinical Practice', 'Medical Research', 'Patient Care', 'Medical Ethics', 'Evidence-Based Medicine']
            },
            'LAW': {
              ...
              specializations: ['Constitutional Law', 'Corporate Law', 'Criminal Law', 'Civil Procedure', 'Legal Research']
            },
            'ENG': {
              ...
              specializations: ['Design Engineering', 'Systems Analysis', 'Project Management', 'Safety Standards', 'Innovation']
            },
            'LANG': {
              name: 'Language Learning',
              description: 'Linguistic and cultural education',
              certifications: ['TOEFL', 'IELTS', 'Language Proficiency Certificates'],
              specializations: ['Communication Skills', 'Cultural Context', 'Professional Language', 'Translation', 'Literature']
            },
            'GENERAL': {
              ...
              specializations: ['Learning Science', 'Cognitive Psychology', 'Educational Technology', 'Assessment Design', 'Curriculum Development']
            }
          },
      ...
      default:
        return NextResponse.json({
          ...
          version: '1.0.0',
          description: 'Universal AI-powered learning system that processes ANY educational content and creates adaptive learning experiences across all domains with 95% code reuse',
          endpoints: {
...
async function getMockAnalysisForDomain(domain: string): Promise<any> {
  return {
    universalElements: Array.from({ length: 5 }, (_, i) => ({
      ...
      aiEnhancements: { explanation: '', examples: [], analogies: [], mnemonics: [], visualizations: [] },
      assessmentQuestions: [],
      prerequisites: [],
      relatedConcepts: []
    })),
