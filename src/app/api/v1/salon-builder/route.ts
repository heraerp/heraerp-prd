...
export async function POST(req: NextRequest) {
  try {
    ...
    const { salonName, desiredSlug, useSubdomainRouting, location } = inputSchema.parse(body)
    ...
    if (createError || !org) {
      ...
      return NextResponse.json(
        { error: createError?.message || 'Failed to create salon organization' },
        { status: 400 }
      )
    }
    ...
    await supabase.from('universal_transactions').insert({
      ...
      metadata: {
        ...
        slug,
        domains: [`${slug}.${base}`],
        created_by: 'salon_builder_api'
      }
    })

    // 3) Create comprehensive seed data using the enhanced function
    await createBasicSalonData(supabase, org.id)

    // Return success with redirect URL
    const redirectUrl = useSubdomainRouting
      ? `http://${slug}.${base}/salon-data`
      : `/org/${slug}/salon-data`
    ...
    console.error('Salon builder error:', error)
    return NextResponse.json({ error: error.message || 'Failed to build salon' }, { status: 500 })
  }
}
