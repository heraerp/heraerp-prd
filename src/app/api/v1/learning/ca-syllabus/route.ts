...
export async function GET(request: NextRequest) {
  try {
    ...
    const group = searchParams.get('group') ? parseInt(searchParams.get('group')!) as 1 | 2 : undefined
 ...
 */
async function generateLearningPath(studentLevel: string, targetPaper: string, weakAreas: string[]) {
  // Get paper details
  let paperDetails = {}
  ...
  return {
    ...
    priority_topics: weakAreas.length > 0 ? weakAreas : ['GST Basics', 'Input Tax Credit', 'Customs Valuation']
  }
}
...
async function analyzeSyllabusCoverage(completedTopics: string[], targetLevel: string) {
  ...
  return {
    ...
    recommendation: completedTopics.length < 10 ? 'Focus on completing more topics' : 'Good progress, maintain consistency'
  }
}
