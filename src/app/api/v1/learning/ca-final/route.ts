    ...
    switch (action) {
      ...
      case 'complete_quiz':
        ...
        return NextResponse.json({
          ...
          ai_feedback: {
            performance_rating:
              data.score > 0.8 ? 'excellent' : data.score > 0.6 ? 'good' : 'needs_improvement',
        ...
        if (topicConfig.requiresDynamicUpdate) {
          ...
          try {
            const universalResponse = await fetch(
              'http://localhost:3002/api/v1/universal-learning',
              {
                method: 'POST',
      ...
      case 'save_generated_content':
        ...
        try {
          ...
          const dynamicData = [
            ...
            {
              field_name: 'secondary_smart_codes',
              field_value: JSON.stringify(contentSmartCode.secondary)
            },
            {
              field_name: 'cross_subject_applicable',
              field_value: contentSmartCode.crossSubject.toString()
            },
            {
              field_name: 'reuse_domains',
              field_value: JSON.stringify(contentSmartCode.reuseDomains)
            },
            // Enhanced Classification
            { field_name: 'bloom_taxonomy_level', field_value: data.bloom_level || 'application' },
            {
              field_name: 'cognitive_complexity',
              field_value: assessCognitiveComplexity(data.question)
            },
            {
              field_name: 'universal_concepts',
              field_value: JSON.stringify(extractUniversalConcepts(data.question, data.tags))
            }
          ]
      ...
      case 'generate_fresh_questions':
        ...
        try {
          ...
          if (!chatGPTResponse.ok) {
            throw new Error(
              `AI API returned ${chatGPTResponse.status}: ${chatGPTResponse.statusText}`
            )
          }
          ...
          if (testQuestions.length === 0) {
            ...
            testQuestions = [
              ...
              },
              {
                ...
                correct_answer: 1,
                explanation:
                  'GSTR-3B is the monthly summary return filed by regular GST taxpayers.',
                difficulty: 'medium',
                topic: 'indirect_tax_gst'
              },
              {
      ...
      learning_elements: [
        {
          ...
          content:
            'Professional skepticism is an attitude of professional doubt that auditors must maintain throughout the audit process.',
    ...
    auditing_standards: [
      {
        ...
        answer:
          'Professional skepticism is an attitude of professional doubt that includes a questioning mind and critical assessment of audit evidence. It is crucial because it helps auditors remain objective and detect potential misstatements or fraud.',
        explanation:
          'Professional skepticism is a fundamental principle in auditing that requires auditors to maintain an attitude of professional doubt throughout the audit engagement.',
        difficulty: 'medium',
      ...
      },
      {
        ...
        answer:
          'Materiality refers to the significance of an amount, transaction, or discrepancy that could influence economic decisions of users. For example, a $1000 error in a company with $10 million revenue may not be material, but the same error in a $50,000 revenue company would be material.',
        explanation:
          'Materiality is both quantitative (monetary threshold) and qualitative (nature of the item) and guides audit procedures.',
        difficulty: 'medium',
    ...
    cost_management: [
      {
        ...
        answer:
          'ABC assigns costs to products based on activities consumed rather than volume-based allocation. Unlike traditional costing which uses single cost drivers like direct labor hours, ABC identifies multiple cost drivers for each activity.',
        explanation:
          'ABC provides more accurate product costing by recognizing that products consume activities and activities consume resources.',
        difficulty: 'hard',
    ...
    questionBank[topicId] || [
      {
        ...
        answer:
          'This topic covers fundamental principles and practical applications relevant to CA Final examination.',
    ...
    CA: [
      {
        ...
        answer:
          'Input Tax Credit (ITC) is the tax paid on inputs that can be set off against the tax liability on outputs under GST.',
      ...
      },
      {
        ...
        question:
          'Calculate the depreciation using straight-line method for an asset costing ₹100,000 with 10-year life.',
        answer:
          'Annual depreciation = (Cost - Salvage Value) / Useful Life = (₹100,000 - 0) / 10 = ₹10,000 per year',
        explanation:
          "Straight-line depreciation allocates equal amounts over the asset's useful life.",
...
function getCrossSubjectReusableQuestions(filters: any) {
  const universalQuestionBank = [
    // === BUSINESS & ECONOMICS (High Reuse) ===
    {
      ...
      answer:
        'Fixed costs remain constant regardless of production volume (rent, salaries), while variable costs change with production levels (raw materials, direct labor).',
      explanation:
        'Understanding cost behavior is fundamental to business decision-making across all domains.',
    ...
    // === CRITICAL THINKING (Universal Skill) ===
    {
      ...
      answer:
        '1) Define the problem clearly, 2) Gather relevant information, 3) Generate alternative solutions, 4) Evaluate options, 5) Implement the best solution, 6) Monitor results.',
      explanation:
        'Systematic problem-solving is a universal skill applicable across all disciplines and contexts.',
    ...
    // === RESEARCH METHODOLOGY (Academic Universal) ===
    {
      ...
      answer:
        'Primary sources are original, first-hand evidence (interviews, surveys, experiments), while secondary sources analyze or interpret primary sources (books, articles, reviews).',
      explanation:
        'Understanding source types is essential for academic research across all disciplines.',
    ...
    // === COMMUNICATION (Universal Skill) ===
    {
      ...
      answer:
        'Clarity (clear message), Conciseness (brief and focused), Accuracy (correct information), Courtesy (respectful tone), and Completeness (all necessary details).',
      explanation:
        'Effective communication principles apply across all professional contexts and industries.',
    ...
    // === ETHICS (Universal Across All Professions) ===
    {
      ...
      answer:
        'Integrity (honesty and moral uprightness), Responsibility (accountability for actions), Respect (for persons and rights), and Fairness (just and equitable treatment).',
      explanation:
        'These ethical principles form the foundation of professional conduct across all disciplines.',
    ...
    // === DATA ANALYSIS (STEM + Business) ===
    {
      ...
      answer:
        "Correlation means two variables change together but one doesn't necessarily cause the other. Causation means one variable directly influences another.",
      explanation:
        'This fundamental concept prevents misinterpretation of data across research, business, and scientific contexts.',
    ...
    // === PROJECT MANAGEMENT (Cross-Industry) ===
    {
      ...
      answer:
        '1) Initiation (define project scope), 2) Planning (detailed roadmap), 3) Execution (implement the plan), 4) Monitoring (track progress), 5) Closure (finalize deliverables).',
      explanation:
        'Project management methodology applies to projects across all industries and contexts.',
    ...
    // === DIGITAL LITERACY (Modern Universal) ===
    {
      ...
      answer:
        'Technical skills (using software/hardware), Information literacy (finding/evaluating online info), Communication skills (digital collaboration), and Digital citizenship (ethical online behavior).',
      explanation:
        'Digital literacy is essential across all modern professions and educational contexts.',
  ...
  return universalQuestionBank
    .filter(q => {
      if (filters.difficulty_level && q.difficulty_level !== filters.difficulty_level) return false
      if (
        filters.subject_domain &&
        !q.reuse_domains.some(
          domain =>
        ...
        return false
      if (
        filters.applicability_threshold &&
        q.applicability_score < filters.applicability_threshold
      )
        return false
      return true
    })
    ...
    customs_valuation: [
      {
        ...
        correct_answer: 0,
        explanation:
          'Transaction Value Method is the primary method for customs valuation under WTO rules.'
      },
      {
        ...
        correct_answer: 1,
        explanation:
          'Section 14 of the Customs Act, 1962 deals with valuation of imported and export goods.'
      }
    ],
    ftp_schemes: [
      {
        ...
        options: [
          ...
          'Export Production Capital Grant',
          'Export Policy Capital Guarantee',
          'Export Product Capital Growth'
        ],
        correct_answer: 0,
      ...
      },
      {
        ...
        correct_answer: 1,
        explanation:
          'RoDTEP (Remission of Duties and Taxes on Export Products) replaced MEIS scheme.'
      }
    ]
  }
...
function generateMockTestQuestions(config: any) {
  const comprehensiveQuestionPool = [
    ...
    // === UNIVERSAL BUSINESS CONCEPTS ===
    {
      ...
      correct_answer: 1,
      explanation:
        'SWOT analysis helps organizations evaluate internal strengths/weaknesses and external opportunities/threats.',
      difficulty: 'medium',
    ...
    // === STEM UNIVERSAL ===
    {
      ...
      correct_answer: 1,
      explanation:
        'Control groups provide a baseline to measure the effect of the experimental variable.',
      difficulty: 'medium',
    ...
    // === ETHICS (Universal Across All Professions) ===
    {
      id: 'MT_ETHICS_001',
      question:
        'What should be the primary consideration when facing an ethical dilemma in professional practice?',
      options: [
        'Personal benefit',
        'Company profit',
        'Stakeholder impact and professional standards',
        'Speed of resolution'
      ],
      correct_answer: 2,
      explanation:
        'Ethical decisions should prioritize stakeholder welfare and adherence to professional standards.',
      difficulty: 'medium',
    ...
    // === DATA ANALYSIS (Cross-Disciplinary) ===
    {
      id: 'MT_DATA_001',
      question:
        'When interpreting statistical data, what does a p-value of 0.03 typically indicate?',
      options: [
        '3% margin of error',
      ...
      correct_answer: 1,
      explanation:
        'A p-value of 0.03 is less than 0.05, indicating statistical significance at the 95% confidence level.',
      difficulty: 'hard',
    ...
    // === PROJECT MANAGEMENT (Cross-Industry) ===
    {
      ...
      correct_answer: 1,
      explanation:
        'The critical path determines the minimum project duration and shows tasks that cannot be delayed.',
      difficulty: 'medium',
    ...
    // === DIGITAL LITERACY (Modern Universal) ===
    {
      id: 'MT_DIGITAL_001',
      question:
        'What is the most important factor when evaluating the credibility of online information?',
      options: [
      ...
      correct_answer: 1,
      explanation:
        'Author expertise and source reliability are key indicators of information credibility.',
      difficulty: 'easy',
  ...
  console.log(
    `🎯 generateMockTestQuestions returning ${finalQuestions.length} questions out of ${comprehensiveQuestionPool.length} available`
  )
  console.log(
    '📊 Final questions:',
