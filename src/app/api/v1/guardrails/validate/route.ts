...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (table === 'universal_transactions' && payload.smart_code?.includes('.GL.') && payload.lines) {
    ...
    const result = await guardrailAutoFix.autoFixPayload(
      table,
      payload,
      {
    ...
    // Record metrics
    if (result.fixed) {
      heraMetrics.recordGuardrailAutofix(
    ...
    // Log validation
    heraLogger.info('Guardrail validation completed', {
      table,
    ...
    return NextResponse.json({
      ...
      ai_confidence: result.fixes_applied.length > 0
        ? result.fixes_applied.reduce((sum, f) => sum + f.confidence, 0) / result.fixes_applied.length
        : 1.0
    })
  } catch (error) {
    const duration = Date.now() - startTime
    
    heraLogger.error('Guardrail validation failed', error as Error, {
      duration_ms: duration
    }, requestId)
