...
const MCP_ACCOUNTING_TOOLS = {
  queryGLAccounts: {
    description:
      'Query general ledger accounts with optional filters. Returns account details and current balances.',
    parameters: {
      organizationId: 'string - Organization ID (required)',
      accountCode: 'string - Specific account code to query (optional)',
      accountType:
        'string - Filter by account type (asset, liability, equity, revenue, expense) (optional)',
  ...
  createJournalEntry: {
    description:
      'Create a journal entry with debits and credits. Automatically validates that debits equal credits.',
    parameters: {
      organizationId: 'string - Organization ID (required)',
  ...
  recordSalonTransaction: {
    description:
      'Record a salon-specific transaction (sale, expense, commission). Handles VAT calculation and categorization.',
    parameters: {
      organizationId: 'string - Organization ID (required)',
  ...
  getTransactionHistory: {
    description:
      'Retrieve transaction history with filters. Useful for summaries, reports, and finding specific transactions.',
    parameters: {
      organizationId: 'string - Organization ID (required)',
  ...
  calculateDailySummary: {
    description:
      'Calculate daily summary including revenue, expenses, commissions, and net profit for a salon.',
    parameters: {
      organizationId: 'string - Organization ID (required)',
  ...
  processCommissionPayment: {
    description:
      'Process a commission payment for staff. Updates pending commission to paid status.',
    parameters: {
      organizationId: 'string - Organization ID (required)',
      transactionId:
        'string - Commission transaction ID to process (optional, provide this or staffName)',
  ...
  postTransactionToGL: {
    description:
      'Post a draft transaction to the general ledger. Creates journal entries and updates GL balances.',
    parameters: {
      organizationId: 'string - Organization ID (required)',
...
Available MCP tools:
${Object.entries(MCP_ACCOUNTING_TOOLS)
...
async function handleMCPRequest(
  ...
  if (
    lowerMessage.match(
      /^(pay now|yes pay|process payment|confirm payment|pay commission|ok pay|pay it)$/
    )
  ) {
    return {
...
The commission has been paid successfully! The staff member will receive their payment via the selected method.`,
  ...
  if (
    ...
    (lowerMessage.includes('for') || lowerMessage.includes('$'))
  ) {
...
Your daily sales total has been updated!`,
  ...
  if (
    ...
    (lowerMessage.includes('paid') && lowerMessage.includes('for'))
  ) {
    const amountMatch = message.match(/\$?([\d,]+(?:\.\d{2})?)/g)
    const amount = amountMatch ? parseFloat(amountMatch[0].replace(/[$,]/g, '')) : 0
...
export async function POST(request: NextRequest) {
  try {
    ...
    const mockMCPResponse = await handleMCPRequest(
      message,
      organizationId,
      context,
      conversationHistory
