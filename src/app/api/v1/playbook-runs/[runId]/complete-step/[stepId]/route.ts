...
export async function POST(
  ...
  try {
    ...
    const stepLine = stepLines.find(line => 
      line.metadata?.step_id === params.stepId
    );
    ...
    if (stepLine.metadata?.step_type === 'human') {
      ...
      if (assignedUserId && assignedUserId !== auth.userId) {
        ...
        if (!canOverride) {
          return NextResponse.json({ 
            success: false, 
            error: 'This step is assigned to another user and you do not have override permissions' 
          }, { status: 403 });
        }
      }
    }
    ...
    const actualDurationMinutes = stepStartTime 
      ? Math.round((new Date(completionTime).getTime() - new Date(stepStartTime).getTime()) / 1000 / 60)
      : undefined;
  ...
  try {
    ...
    for (const line of allLines) {
      ...
      if (dependsOnCompleted && line.metadata?.status === 'pending') {
        ...
        const allDependenciesMet = checkAllDependencies(
          dependencies,
          allLines
        );
