  ...
  try {
      ...
      for (const line of stepLines) {
        ...
        if (metadata.started_at) {
          timelineEvents.push({
            ...
            sequence: line.line_number,
            step_name: metadata.step_name,
            actor: metadata.worker_type === 'human' ? {
        ...
        if (metadata.completed_at) {
          const eventType = metadata.status === 'completed' ? 'step_completed' :
                           metadata.status === 'failed' ? 'step_failed' :
                           metadata.status === 'cancelled' ? 'step_cancelled' :
                           metadata.status === 'skipped' ? 'step_skipped' : null;
          ...
          if (eventType) {
            timelineEvents.push({
              ...
              duration_ms: metadata.started_at ? 
                new Date(metadata.completed_at).getTime() - new Date(metadata.started_at).getTime() : 
                undefined,
              actor: {
        ...
        if (metadata.expected_duration_minutes && metadata.actual_duration_minutes) {
          if (metadata.actual_duration_minutes > metadata.expected_duration_minutes) {
            const breachPercentage = ((metadata.actual_duration_minutes - metadata.expected_duration_minutes) / 
                                     metadata.expected_duration_minutes) * 100;
    ...
    if (relatedEvents) {
      for (const event of relatedEvents) {
        ...
        // Approval events
        if (event.transaction_type === 'playbook_step_completion' && 
        ...
        // Error events
        if (event.transaction_type === 'playbook_ai_error' || 
    ...
    if (run.metadata?.completed_at) {
      const eventType = run.metadata?.status === 'completed' ? 'run_completed' :
                       run.metadata?.status === 'failed' ? 'run_failed' :
                       run.metadata?.status === 'cancelled' ? 'run_cancelled' : null;
      
      if (eventType) {
        timelineEvents.push({
          timestamp: run.metadata.completed_at,
          event_type: eventType as any,
          duration_ms: new Date(run.metadata.completed_at).getTime() - 
    ...
    // Build steps summary
    const stepsSummary: StepSummary[] = stepLines?.map(line => {
      const metadata = line.metadata || {};
...
function identifyBottlenecks(
  ...
  const bottlenecks: Array<{ step_sequence: number; step_name: string; wait_time_ms: number; reason: string }> = [];
  ...
  for (const step of stepsSummary) {
    ...
    if (metadata.activated_at && metadata.started_at) {
      const waitTime = new Date(metadata.started_at).getTime() - 
                      new Date(metadata.activated_at).getTime();
      
      if (waitTime > 5 * 60 * 1000) { // More than 5 minutes wait
        bottlenecks.push({
    ...
    if (metadata.expected_duration_minutes && metadata.actual_duration_minutes) {
      ...
      if (actualMs > expectedMs * 1.5) { // 50% over expected
        bottlenecks.push({
          step_sequence: step.sequence,
          step_name: step.step_name,
...
function calculateCriticalPath(stepLines: any[]): number[] {
  ...
  const startSteps = stepLines.filter(line => 
    !line.metadata?.dependencies || line.metadata.dependencies.length === 0
  );
