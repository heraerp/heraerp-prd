  ...
  try {
    ...
    const formattedAgents =
      agents?.map(agent => {
        ...
        const status =
          dynamicData.find((d: any) => d.field_name === 'availability_status')?.field_value_text ||
          'available'
        const activeChats =
          dynamicData.find((d: any) => d.field_name === 'active_chats')?.field_value_text || '0'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { error: updateError } = await supabase
      .from('core_entities')
      .update({
        metadata: supabase.rpc('jsonb_set', ['metadata', '{agent_id}', `"${agentId}"`, true])
      })
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const { error: updateError } = await supabase
      .from('core_entities')
      .update({
        metadata: supabase.rpc('jsonb_set', ['metadata', '{agent_id}', 'null', false])
      })
      .eq('id', conversationId)
      .eq('organization_id', organizationId)
