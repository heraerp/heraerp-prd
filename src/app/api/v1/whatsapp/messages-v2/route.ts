  ...
  try {
    ...
    if (transactionIds.length > 0) {
      const { data: relData, error: relError } = await supabase
        ...
        .select('*')
        .or(
          `from_entity_id.in.(${transactionIds.join(',')}),to_entity_id.in.(${transactionIds.join(',')})`
        )
    ...
    const messages =
      transactions?.map(txn => {
        ...
        const txnRelationships =
          relationships?.filter(r => r.from_entity_id === txn.id || r.to_entity_id === txn.id) || []
        ...
        const direction =
          txn.smart_code?.includes('RECEIVED') || (txn.metadata as any)?.direction === 'inbound'
            ? 'inbound'
            : 'outbound'
        ...
        return {
          ...
          waba_message_id:
            (txn.metadata as any)?.waba_message_id ||
            dynamicFields.waba_message_id ||
            txn.external_id,
