...
// GET /restaurant-reservations/transactions - Fetch all restaurant-reservations items
export async function GET(request: NextRequest) {
  try {
    const supabaseAdmin = getSupabaseAdmin()
    const { searchParams } = new URL(request.url)
    const organizationId = searchParams.get('organization_id')
    const entityType = searchParams.get('entity_type') || 'restaurant-reservations_item'
    ...
    if (entitiesError) {
      console.error('Error fetching restaurant-reservations entities:', entitiesError)
      return NextResponse.json(
        { success: false, message: 'Failed to fetch restaurant-reservations entities' },
        { status: 500 }
      )
    }
    ...
    console.error('Restaurant-reservations API error:', error)
    return NextResponse.json(
...
// POST /restaurant-reservations/transactions - Create restaurant-reservations item
  ...
  try {
    ...
    const { data: entity, error: entityError } = await supabaseAdmin
      ...
      .insert({
        ...
        entity_type,
        entity_name,
        entity_code: entity_code || `RES-${Date.now().toString().slice(-6)}`,
        status: 'active',
    ...
    if (entityError) {
      console.error('Error creating restaurant-reservations entity:', entityError)
      return NextResponse.json(
        { success: false, message: 'Failed to create restaurant-reservations entity', error: entityError },
        { status: 500 }
      )
    }
    ...
    return NextResponse.json({
      ...
      data: {
        ...entity,
        message: 'Restaurant-reservations created successfully'
      }
    })

  } catch (error) {
    console.error('Restaurant-reservations creation error:', error)
    return NextResponse.json(
