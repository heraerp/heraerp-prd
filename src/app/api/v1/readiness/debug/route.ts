...
export async function GET(request: NextRequest) {
  try {
    ...
    if (answersError) {
      return NextResponse.json({ error: 'Failed to fetch answers', details: answersError }, { status: 500 })
    }
    ...
    const debugData = {
      ...
      sessions_without_answers: sessions?.filter(s => !answersBySession[s.id] || answersBySession[s.id].length === 0).map(s => s.id),
