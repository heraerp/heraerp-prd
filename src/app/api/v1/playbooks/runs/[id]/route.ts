...
export async function GET(
  ...
  try {
    ...
    if (!playbookAuthService.canExecutePlaybook(execution.playbook_id) && 
    ...
    if (queryParams.include_logs) {
      enrichedExecution.execution_logs = await getExecutionLogs(
        executionId,
        queryParams.log_level
      );
    }
    ...
    enrichedExecution.progress = {
      percentage: execution.total_steps > 0 ? 
...
function canCancelExecution(execution: any, authState: any): boolean {
  ...
  return execution.initiated_by === authState.user?.id || 
         playbookAuthService.canManagePlaybooks();
}

function canRetryExecution(execution: any, authState: any): boolean {
  return execution.status === 'failed' && 
         (execution.initiated_by === authState.user?.id || 
          playbookAuthService.canManagePlaybooks());
}
