    ...
    if (body.policies && body.policies.length > 0) {
      for (const policyData of body.policies) {
        const policySmartCode = PlaybookSmartCodes.forPolicy(
          policyData.type.replace('_policy', '') as
            | 'sla'
            | 'quorum'
            | 'segregation'
            | 'approval'
    ...
    await playbookDataLayer.updatePlaybookDefinition(playbook.id, {
      metadata: {
        ...
        creation_summary: {
          ...
          policies: createdPolicies.map(p => ({
            type: p.code,
            rules_count: Object.keys(p.value_json.rules || {}).length
          }))
        }
      }
    })
...
export async function GET(request: NextRequest) {
  try {
    ...
    const summary = {
      ...
      avg_duration_hours:
        enrichedPlaybooks.reduce((sum, p) => sum + (p.metadata.estimated_duration_hours || 0), 0) /
        enrichedPlaybooks.length,
