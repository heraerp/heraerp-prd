...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    // Check permissions
    if (
      !playbookAuthService.canExecutePlaybook(playbookId) &&
    ...
    for (const step of sortedSteps) {
      ...
      if (includeContracts) {
        enriched.input_contract = await playbookDataLayer.getContract(
          step.id,
          'step_input_contract'
        )
        enriched.output_contract = await playbookDataLayer.getContract(
          step.id,
  ...
  switch (playbook.status) {
    ...
    case 'active':
      return {
        ...
        allowed_operations: ['view', 'execute', 'create_new_version', 'deprecate']
      }
    ...
    case 'deprecated':
      return {
        ...
        allowed_operations: ['view', 'create_new_version', 'reactivate']
      }
 ...
 */
async function createNewPlaybookVersion(originalPlaybook: any, userId?: string): Promise<any> {
...
async function addStepsToPlaybook(
  ...
  for (const stepRequest of request.steps) {
    if (
      existingStepNumbers.includes(stepRequest.step_number) &&
    ...
    if (stepRequest.dependencies) {
      for (const dep of stepRequest.dependencies) {
        if (
          !existingStepNumbers.includes(dep.depends_on_step_number) &&
          !request.steps.some(s => s.step_number === dep.depends_on_step_number)
        ) {
          errors.push(
            `Step ${stepRequest.step_number}: dependency on non-existent step ${dep.depends_on_step_number}`
          )
        }
      }
    }
    ...
    try {
      ...
      if (stepRequest.dependencies) {
        for (const dependency of stepRequest.dependencies) {
          ...
          const dependsOnStep = [...existingSteps, ...createdSteps].find(
            s => s.metadata.step_number === dependency.depends_on_step_number
          )
...
function analyzeStepFlow(steps: any[]): any {
  return {
    ...
    estimated_total_duration_minutes: steps.reduce(
      (sum, s) => sum + (s.metadata.estimated_duration_minutes || 0),
      0
    ),
...
function identifyPotentialBottlenecks(steps: any[]): string[] {
  ...
  const longSteps = steps.filter(s => (s.metadata.estimated_duration_minutes || 0) > 60)
