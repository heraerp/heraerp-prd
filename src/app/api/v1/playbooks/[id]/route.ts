...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    if (
      !playbookAuthService.canExecutePlaybook(playbook.id) &&
    ...
    if (queryParams.include_contracts) {
      enrichedPlaybook.contracts = await getPlaybookContracts(
        playbook.id,
        queryParams.expand_relationships
      )
    }

    // Include policies
    if (queryParams.include_policies) {
      enrichedPlaybook.policies = await getPlaybookPolicies(
        playbook.id,
        queryParams.expand_relationships
      )
    }
    ...
    // Get version history
    const versionHistory = await getPlaybookVersionHistory(
      playbook.name,
    ...
    const metadata = {
      ...
      user_permissions: {
        ...
        can_view_analytics:
          queryParams.include_analytics && playbookAuthService.hasPermission('playbooks:analytics')
      },
      version_info: {
        ...
        available_versions: versionHistory.map(v => ({
          id: v.id,
          version: v.version,
          status: v.status
        }))
    ...
    execution_stats: {
      ...
      average_duration_hours:
        completedRuns.length > 0
          ? completedRuns.reduce((sum, run) => sum + calculateRunDurationHours(run), 0) /
...
function analyzeJsonSchema(schema: any) {
  return {
    ...
    optional_fields: schema.properties
...
function analyzePolicyRules(policyData: any) {
  ...
  return {
    ...
    has_thresholds: Object.values(rules).some(
      (rule: any) => rule.threshold || rule.minimum || rule.maximum || rule.duration
    ),
    complexity_score:
...
function getPerformanceTrend(runs: any[], days: number) {
  ...
  return {
    ...
    average_duration:
      recentRuns.length > 0
        ? recentRuns.reduce((sum, run) => sum + calculateRunDurationHours(run), 0) /
          recentRuns.length
        : 0
  }
}
