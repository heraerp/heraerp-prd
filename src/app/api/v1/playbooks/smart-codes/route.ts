  ...
  try {
    ...
    switch (action) {
      ...
      case 'generate-contract':
        ...
        if (!contractType || !['input', 'output', 'step_input', 'step_output'].includes(contractType)) {
          return NextResponse.json({
      ...
      case 'generate-policy':
        ...
        if (!policyType || !['sla', 'quorum', 'segregation', 'approval', 'retry'].includes(policyType)) {
          return NextResponse.json({
      ...
      default:
        return NextResponse.json({
          error: 'Invalid action. Supported actions: generate-playbook, generate-step, generate-contract, generate-policy, next-version, batch-validate'
        }, { status: 400 });
    }
