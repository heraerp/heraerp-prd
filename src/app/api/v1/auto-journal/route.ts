        ...
        return NextResponse.json(
          {
            success: false,
            error:
              'Invalid action. Supported actions: process_transaction, run_batch_processing, check_relevance, generate_journal'
          },
          { status: 400 }
        )
    }
        ...
        return NextResponse.json(
          {
            success: false,
            error:
              'Invalid action. Supported actions: history, statistics, pending_batch, transaction_status'
          },
          { status: 400 }
        )
    }
...
async function getAutoJournalStatistics(organizationId: string, days: number) {
  ...
  return {
    ...
    automation_rate:
      totalProcessed > 0 ? ((totalJournals / totalProcessed) * 100).toFixed(1) : '0.0'
  }
}
...
async function getTransactionJournalStatus(transactionId: string) {
  ...
  return {
    ...
    status: journalEntry
      ? 'journal_created'
      : (transaction.metadata as any)?.batch_journal_id
        ? 'batched'
        : processingLog?.field_name === 'journal_processing_error'
          ? 'error'
          : 'pending'
  }
}
