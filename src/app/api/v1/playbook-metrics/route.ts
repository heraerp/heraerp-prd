...
interface PlaybookMetrics {
  ...
  sla_metrics: {
    ...
    sla_by_step_type: Record<string, {
      total_steps: number;
      breaches: number;
      breach_rate: number;
    }>;
  ...
  retry_metrics: {
    ...
    retry_by_step_type: Record<string, {
      total_retries: number;
      success_rate: number;
  ...
  try {
    ...
    const relatedEvents = await universalApi.readTransactions({
      filters: {
        organization_id: auth.organizationId!,
        transaction_type: {
          $in: ['playbook_signal', 'playbook_step_completion', 'playbook_ai_error', 
                'playbook_external_error', 'playbook_orchestrator_event']
        },
        created_at: {
          $gte: startDate,
          $lte: endDate
...
function calculateSLAMetrics(stepLines: any[], playbooks: any[]): PlaybookMetrics['sla_metrics'] {
  const stepsWithSLA = stepLines.filter(s => 
    s.metadata?.expected_duration_minutes !== undefined
  );
  ...
  const avgBreachPercentage = breachPercentages.length > 0 ?
    breachPercentages.reduce((a, b) => a + b, 0) / breachPercentages.length : 0;
  ...
  breachedSteps.forEach(step => {
    ...
    const breachMinutes = step.metadata.actual_duration_minutes - 
  ...
  const topBreaches = Array.from(breachMap.values())
    .map(entry => ({
      ...
      average_breach_minutes: entry.breaches.reduce((a: number, b: number) => a + b, 0) / entry.breaches.length,
  ...
  ['human', 'system', 'ai', 'external'].forEach(stepType => {
    ...
    slaByType[stepType] = {
      ...
      breach_rate: typeSteps.length > 0 ? 
        (typeBreaches.length / typeSteps.length) * 100 : 0
    };
  });
  ...
  return {
    ...
    sla_breach_rate: stepsWithSLA.length > 0 ? 
      (breachedSteps.length / stepsWithSLA.length) * 100 : 0,
...
function calculateRetryMetrics(
  ...
  const retrySuccessRate = retriedSteps.length > 0 ?
    (retriedAndSucceeded.length / retriedSteps.length) * 100 : 0;
  
  // Average retries per failure
  const failedSteps = stepLines.filter(s => s.metadata?.status === 'failed');
  const avgRetriesPerFailure = failedSteps.length > 0 ?
    failedSteps.reduce((sum, s) => sum + (s.metadata?.retry_count || 0), 0) / failedSteps.length : 0;
  
  // Max retries reached
  const maxRetriesReached = retriedSteps.filter(s => 
    s.metadata?.retry_count >= (s.metadata?.max_retries || 3)
  ).length;
  ...
  ['human', 'system', 'ai', 'external'].forEach(stepType => {
    ...
    retryByType[stepType] = {
      ...
      success_rate: typeRetries.length > 0 ?
        (typeSuccesses.length / typeRetries.length) * 100 : 0,
      average_attempts: typeRetries.length > 0 ?
        totalTypeRetries / typeRetries.length : 0
    };
  });
  ...
  const topRetrySteps = Array.from(retryMap.values())
    .map(entry => ({
      ...
      success_rate: entry.total_retries > 0 ?
        (entry.successes / entry.total_retries) * 100 : 0,
...
function calculateBottleneckAnalysis(stepLines: any[]) {
  ...
  stepTypes.forEach(type => {
    ...
    const waitTimes = typeSteps
      ...
      .map(s => {
        const wait = new Date(s.metadata.started_at).getTime() - 
                    new Date(s.metadata.activated_at).getTime();
    ...
    analysis[`${type}_steps`] = {
      average_wait_time_minutes: waitTimes.length > 0 ?
        waitTimes.reduce((a, b) => a + b, 0) / waitTimes.length : 0,
      average_execution_time_minutes: execTimes.length > 0 ?
        execTimes.reduce((a, b) => a + b, 0) / execTimes.length : 0
    };
  });
...
function calculateReliabilityMetrics(
  ...
  const mtbfHours = failureTimestamps.length > 1 ?
    (totalTimeBetweenFailures / (failureTimestamps.length - 1)) / 1000 / 60 / 60 : 0;
  ...
  const recoveredRuns = failedRuns.filter(r => {
    ...
    const laterRuns = runs.filter(lr => 
  ...
  const recoveryTimes = recoveredRuns.map(r => {
    const nextSuccess = runs.find(lr => 
      lr.metadata?.playbook_id === r.metadata?.playbook_id &&
      new Date(lr.created_at).getTime() > new Date(r.created_at).getTime() &&
      lr.metadata?.status === 'completed'
    );
    ...
    return (new Date(nextSuccess.created_at).getTime() - 
            new Date(r.metadata?.failed_at || r.created_at).getTime()) / 1000 / 60;
  }).filter(t => t > 0);
  
  const mttrMinutes = recoveryTimes.length > 0 ?
  ...
  const availabilityPercentage = totalRuntime > 0 ?
    ((totalRuntime - failureTime) / totalRuntime) * 100 : 100;
  
  // Error analysis
  const errorEvents = events.filter(e => 
    e.transaction_type === 'playbook_ai_error' || 
    e.transaction_type === 'playbook_external_error'
  );
...
function generateTimeSeries(
  ...
  stepLines.forEach(step => {
    ...
    if (timeGroups.has(groupKey)) {
      ...
      if (step.metadata.expected_duration_minutes && 
  ...
  const timeSeries = Array.from(timeGroups.values())
