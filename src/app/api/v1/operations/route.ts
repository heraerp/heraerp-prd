...
export async function GET(request: NextRequest) {
  try {
    ...
    const transformedTables = tables?.map(table => {
      ...
      const currentOrder = ordersByType.dine_in.find((order: any) => 
        order.table_id === table.id && ['pending', 'processing', 'approved'].includes(order.status)
      )
    ...
    const metrics = {
      ...
      average_order_value: transformedOrders.length > 0 
        ? transformedOrders.reduce((sum, order) => sum + order.total_amount, 0) / transformedOrders.length 
        : 0,
      ...
      table_occupancy_rate: transformedTables.length > 0 
        ? (transformedTables.filter(table => table.status === 'occupied').length / transformedTables.length) * 100 
        : 0,
      ...
      // Pickup operations
      ready_for_pickup: ordersByType.pickup.filter((order: any) => 
        order.status === 'approved' || order.ready_for_pickup
      ).length
    }
    ...
    console.log(`✅ Operations dashboard loaded: ${metrics.total_orders} orders, ${metrics.occupied_tables}/${metrics.total_tables} tables occupied`)
