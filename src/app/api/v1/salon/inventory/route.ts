  ...
  try {
    ...
    const { data: dynamicData, error: dynamicError } = await supabase
      ...
      .in('field_name', [
        ...
        'selling_price', 'supplier', 'location', 'expiry_date',
        'last_restocked', 'units_sold', 'brand', 'sku', 'category'
      ])
    ...
    switch (action) {
      ...
      case 'update':
        ...
        for (const [key, value] of Object.entries(updates)) {
          ...
          const fieldName = key === 'currentStock' ? 'current_stock' :
                           key === 'minStock' ? 'min_stock' :
                           key === 'maxStock' ? 'max_stock' :
                           key === 'unitCost' ? 'unit_cost' :
