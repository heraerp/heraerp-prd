...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (usePlaybook) {
    ...
    logPlaybookExecution('HERA.SALON.POS.CART.REPRICE.V1', result.success, Date.now() - startTime, {
      organizationId,
    ...
    if (!result.success) {
      ...
      const statusCode =
        result.error?.code === 'CART_NOT_FOUND'
          ? 404
          : result.error?.code === 'CART_ALREADY_CHECKED_OUT'
            ? 400
            : result.error?.code === 'INVALID_PROMOTION_CODE'
              ? 400
              : 500
  ...
  return new NextResponse(null, {
    ...
    headers: {
      ...
      'Access-Control-Allow-Headers':
        'Content-Type, Authorization, X-Organization-ID, Idempotency-Key, X-Correlation-ID'
    }
  })
}
