...
export const POST = withErrorHandler(async (request: NextRequest) => {
  ...
  const transactionData = {
    ...
    exchange_rate: currencyCode === 'AED' ? 1.0 : getExchangeRate(currencyCode),
...
export const GET = withErrorHandler(async (request: NextRequest) => {
  ...
  switch (dataType) {
    ...
    case 'products':
      ...
      if (productsError) {
        throw new APIError('Failed to fetch products', 500, 'PRODUCTS_FETCH_ERROR', {
          error: productsError
        })
      }
    ...
    case 'customers':
      ...
      if (customersError) {
        throw new APIError('Failed to fetch customers', 500, 'CUSTOMERS_FETCH_ERROR', {
      ...
      for (const customer of customers || []) {
        ...
        customer.tier =
          dynamicData?.find(f => f.field_name === 'loyalty_tier')?.field_value_text || 'Bronze'
        customer.visits =
          dynamicData?.find(f => f.field_name === 'visit_count')?.field_value_number || 0
      }
