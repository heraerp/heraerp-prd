...
function mockAnalyzeDocument(fileName: string): any {
  ...
  if (
    randomVendor.category === 'materials' ||
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { error: analysisError } = await supabase.from('core_dynamic_data').insert({
      entity_id: fileId,
    ...
    const suggestedMessage = `Paid ${analysis.vendor_name} ${analysis.total_amount.toLocaleString('en-IN')} for ${analysis.items
