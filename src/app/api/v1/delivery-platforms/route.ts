...
export async function GET(request: NextRequest) {
  try {
    ...
    const transformedPlatforms = platforms?.map(platform => {
      const dynamicProps = platform.dynamic_data?.reduce((acc: any, prop: any) => {
      ...
      const platformData = {
        ...
        ...(includeStats && (platformStats as any)[platform.id] && {
    ...
    const overallStats = {
      ...
      active_platforms: transformedPlatforms.filter(p => p.is_active && p.status === 'active').length,
      configured_platforms: transformedPlatforms.filter(p => p.sync_status === 'connected').length,
      total_today_orders: Object.values(platformStats).reduce((sum: number, stats: any) => sum + stats.total_orders, 0),
      total_today_revenue: Object.values(platformStats).reduce((sum: number, stats: any) => sum + stats.total_revenue, 0)
    }
    ...
    const response = {
      ...
      data: {
        ...
        stats: overallStats,
        supported_platforms: [
          'deliveroo', 'ubereats', 'swiggy', 'zomato', 'doordash', 
          'grubhub', 'foodpanda', 'talabat', 'custom'
        ]
      }
    }

    console.log(`✅ Loaded ${transformedPlatforms.length} delivery platforms (${overallStats.active_platforms} active)`)
...
export async function POST(request: NextRequest) {
  try {
    ...
    const platformCode = platformData.code || 
      platformData.platform_type.toUpperCase() + '_' + 
    ...
    const platformProperties = [
      ...
      // Business Configuration
      { name: 'commission_rate', value: (platformData.commission_rate || 0.15).toString(), type: 'number' },
      { name: 'delivery_fee', value: (platformData.delivery_fee || 0).toString(), type: 'number' },
      { name: 'minimum_order_value', value: (platformData.minimum_order_value || 0).toString(), type: 'number' },
      { name: 'max_delivery_distance', value: (platformData.max_delivery_distance || 10).toString(), type: 'number' },
      
      // Operational Settings
      { name: 'is_active', value: 'true', type: 'boolean' },
      { name: 'auto_accept_orders', value: (platformData.auto_accept_orders || false).toString(), type: 'boolean' },
      { name: 'sync_menu', value: (platformData.sync_menu !== false).toString(), type: 'boolean' },
      { name: 'sync_inventory', value: (platformData.sync_inventory !== false).toString(), type: 'boolean' },
      ...
      { name: 'delivery_zones', value: JSON.stringify(platformData.delivery_zones || []), type: 'text' }
    ...
    console.log(`✅ Delivery platform configured: ${platform.entity_name} (${platformData.platform_type})`)
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const dynamicUpdates = [
      'api_endpoint', 'webhook_url', 'api_key', 'secret_key',
      'commission_rate', 'delivery_fee', 'minimum_order_value', 'max_delivery_distance',
      'is_active', 'auto_accept_orders', 'sync_menu', 'sync_inventory',
      'sync_status', 'webhook_verified', 'restaurant_id', 'store_id', 'delivery_zones',
      'last_sync_at'
    ]
    ...
    for (const property of dynamicUpdates) {
      if (updateData[property] !== undefined) {
        ...
        const fieldType = ['commission_rate', 'delivery_fee', 'minimum_order_value', 'max_delivery_distance'].includes(property) ? 'number' :
                         ['is_active', 'auto_accept_orders', 'sync_menu', 'sync_inventory', 'webhook_verified'].includes(property) ? 'boolean' : 'text'
