...
/**
 * Restaurant suppliers that makes money - transactions API
 * Generated by HERA Module Generator
 * 
 * Customer-focused API that just works
 */

// GET /suppliers/transactions - Fetch items using universal schema
export async function GET(request: NextRequest) {
  try {
    ...
    const entityType = searchParams.get('entity_type') || 'suppliers_item'
    ...
    let query = supabaseAdmin
      ...
      .in('entity_type', ['suppliers_item', 'suppliers_category', 'suppliers_supplier'])
      .eq('status', 'active')
      .order('entity_name')
    ...
    if (entitiesError) {
      console.error('Error fetching suppliers entities:', entitiesError)
      return NextResponse.json(
        { success: false, message: 'Failed to fetch suppliers entities' },
        { status: 500 }
      )
    }
...
// POST /suppliers/transactions - Create item using universal schema
export async function POST(request: NextRequest) {
  try {
    ...
    const finalEntityCode = entity_code || `SUP-${entity_type.toUpperCase().slice(0,3)}-${Date.now().toString().slice(-6)}`
    ...
    const { data: entity, error: entityError } = await supabaseAdmin
      ...
      .insert({
        ...
        status: 'active',
        ai_classification: dynamic_fields.category || 'suppliers_item',
    ...
    if (entityError) {
      console.error('Error creating suppliers entity:', entityError)
      return NextResponse.json(
        { success: false, message: 'Failed to create suppliers entity', error: entityError },
        { status: 500 }
      )
    }
    ...
    return NextResponse.json({
      ...
      data: {
        ...entity,
        smart_code: `HERA.SUPPLIERS.ITEM.${finalEntityCode}.v1`,
        dynamic_fields_created: dynamicFieldsData.length
      },
      message: `Suppliers item created successfully`
    })

  } catch (error) {
    console.error('Suppliers creation error:', error)
    return NextResponse.json(
