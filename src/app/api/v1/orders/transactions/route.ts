...
// GET /orders/transactions - Fetch all orders items
export async function GET(request: NextRequest) {
  try {
    const supabaseAdmin = getSupabaseAdmin()
    const { searchParams } = new URL(request.url)
    const organizationId = searchParams.get('organization_id')
    const entityType = searchParams.get('entity_type') || 'orders_item'
    ...
    if (entitiesError) {
      console.error('Error fetching orders entities:', entitiesError)
      return NextResponse.json(
        { success: false, message: 'Failed to fetch orders entities' },
        { status: 500 }
      )
    }
...
// POST /orders/transactions - Create orders item
  ...
  try {
    ...
    const { data: entity, error: entityError } = await supabaseAdmin
      ...
      .insert({
        ...
        entity_type,
        entity_name,
        entity_code: entity_code || `ORD-${Date.now().toString().slice(-6)}`,
        status: 'active',
    ...
    if (entityError) {
      console.error('Error creating orders entity:', entityError)
      return NextResponse.json(
        { success: false, message: 'Failed to create orders entity', error: entityError },
        { status: 500 }
      )
    }
