...
function analyzeQuery(message: string): AnalyticalResponse {
  ...
  if (/book|appointment|schedule/i.test(message) && !/\b(at|for|with|tomorrow|today|\d{1,2}(:\d{2})?\s*(am|pm)?)/i.test(message)) {
  ...
  if (/show|check|view/i.test(message) && /appointment|booking/i.test(message) && !/(today|tomorrow|week|month|for|client)/i.test(message)) {
    analysis.needsClarification = true;
    analysis.clarifyingQuestions = [
...
function parseAppointmentDetails(message: string): any {
  ...
  if (/book\s+appointment\s+for/i.test(message)) {
    ...
    const serviceMatch = message.match(/appointment\s+for\s+(?:a\s+)?([A-Za-z\s]+?)(?:\s+with|\s+at|\s+tomorrow|\s+today|$)/i);
    if (serviceMatch) {
    ...
    const bookForPattern = message.match(/book\s+([A-Za-z\s]+?)\s+for\s+([A-Za-z\s]+?)(?:\s+with|\s+at|\s+tomorrow|\s+today|$)/i);
    if (bookForPattern) {
  ...
  const stylistMatch = message.match(/with\s+([A-Za-z\s]+?)(?:\s+for|\s+at|\s+tomorrow|\s+today|$)/i);
  if (stylistMatch) {
    result.stylistName = stylistMatch[1].trim();
  }
  
  // Extract service (for patterns after stylist)
  const serviceMatch = message.match(/for\s+(?:a\s+)?([A-Za-z\s]+?)(?:\s+tomorrow|\s+today|\s+at|\s+on|$)/i);
...
function parseInventoryQuery(message: string): { productName?: string } {
  ...
  const productMatch = message.match(/(?:check|stock|inventory)\s+(?:of\s+)?([A-Za-z\s]+?)(?:\s+stock|\s+level|$)/i);
    ...
    switch (intent.type) {
      case 'appointment': {
        ...
        if (!details.clientName) {
          ...
          return NextResponse.json({
            ...
            analyticalFramework: {
              stage: 'clarify',
              missingFields: ['clientName', !details.serviceName && 'service', !details.time && 'time'].filter(Boolean)
            },
            suggestions: [
        ...
        return NextResponse.json({
          ...
          analyticalFramework: {
            ...
            nextSteps: result.success ? ['confirm', 'prepare', 'remind'] : ['retry', 'check_availability']
          },
      ...
      case 'birthday': {
        ...
        return NextResponse.json({
          ...
          confidence: 100,
          data: result,
          actions: result.clients.length > 0 ? [
            {
      ...
      case 'searchAppointment': {
        ...
        if (!searchQuery) {
          nameMatch = message.match(/(?:what\s+time\s+is|when\s+is)\s+([A-Za-z]+)(?:'s)?\s+appointment/i);
          if (nameMatch) {
            searchQuery = nameMatch[1].trim();
          }
        }
      ...
      case 'analytics': {
        ...
        const insightType = /revenue|sales|money/i.test(message) ? 'revenue' 
          : /busy|peak|pattern/i.test(message) ? 'patterns'
          : /recommend|suggest/i.test(message) ? 'recommendations'
          : 'all';
        ...
        if (result.insights.recommendations.length > 0) {
          ...
          result.insights.recommendations.forEach((rec: any, idx: number) => {
            const actionIcon = rec.type === 'inventory' ? '📦' 
              : rec.type === 'promotion' ? '📣'
              : rec.type === 'marketing' ? '🎁'
              : '💡';
        ...
        try {
          ...
          for (const pattern of servicePatterns) {
            ...
            if (match && match[1]) {
              ...
              if (['a', 'an', 'the', 'for', 'today', 'tomorrow'].includes(serviceName.toLowerCase())) {
