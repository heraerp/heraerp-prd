...
/**
 * Restaurant staff that makes money - transactions API
 * Generated by HERA Module Generator
 * 
 * Customer-focused API that just works
 */

// GET /staff/transactions - Fetch items using universal schema
export async function GET(request: NextRequest) {
  try {
    ...
    const entityType = searchParams.get('entity_type') || 'staff_item'
    ...
    let query = supabaseAdmin
      ...
      .in('entity_type', ['staff_item', 'staff_category', 'staff_supplier'])
      .eq('status', 'active')
      .order('entity_name')
    ...
    if (entitiesError) {
      console.error('Error fetching staff entities:', entitiesError)
      return NextResponse.json(
        { success: false, message: 'Failed to fetch staff entities' },
        { status: 500 }
      )
    }
...
// POST /staff/transactions - Create item using universal schema
export async function POST(request: NextRequest) {
  try {
    ...
    const finalEntityCode = entity_code || `STA-${entity_type.toUpperCase().slice(0,3)}-${Date.now().toString().slice(-6)}`
    ...
    const { data: entity, error: entityError } = await supabaseAdmin
      ...
      .insert({
        ...
        status: 'active',
        ai_classification: dynamic_fields.category || 'staff_item',
    ...
    if (entityError) {
      console.error('Error creating staff entity:', entityError)
      return NextResponse.json(
        { success: false, message: 'Failed to create staff entity', error: entityError },
        { status: 500 }
      )
    }
    ...
    return NextResponse.json({
      ...
      data: {
        ...entity,
        smart_code: `HERA.STAFF.ITEM.${finalEntityCode}.v1`,
