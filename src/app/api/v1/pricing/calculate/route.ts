...
async function calculateStandardPricing(pricingData: any, organizationId: string): Promise<any> {
  try {
    ...
    marketAdjustment = (competitorBenchmark * marketFactor * demandFactor) - (costBasis + markupAmount)
    ...
    if (pricingData.customer_id) {
      ...
      if (customerEntity) {
        const customerSegment = customerEntity.dynamic_data?.find((d: any) => d.field_name === 'customer_segment')?.field_value_text
        const loyaltyTier = customerEntity.dynamic_data?.find((d: any) => d.field_name === 'loyalty_tier')?.field_value_text
        ...
        switch (customerSegment) {
          ...
          case 'standard':
            ...
            break
          case 'volume':
            customerDiscount = (costBasis + markupAmount) * 0.10 // 10% discount
            break
        }
        ...
        if (pricingData.quantity >= 100) {
          ...
          volumeDiscount = (costBasis + markupAmount) * 0.10 // 10% volume discount
    ...
    return {
      ...
      pricing_breakdown: {
        cost_basis: {
          ...
          cost_components: [
            { component_name: 'Material Costs', cost_amount: bomResult.cost_breakdown?.materials?.total || 0, cost_percentage: 60 },
            { component_name: 'Labor Costs', cost_amount: bomResult.cost_breakdown?.labor?.total || 0, cost_percentage: 25 },
            { component_name: 'Overhead Costs', cost_amount: bomResult.cost_breakdown?.overhead?.total || 0, cost_percentage: 15 }
          ]
        },
        markup_analysis: {
...
async function calculateDynamicPricing(pricingData: any, organizationId: string): Promise<any> {
  try {
    ...
    if (timeOfDay >= 11 && timeOfDay <= 14) { // Lunch rush
      dynamicMultiplier *= 1.15 // 15% premium
    } else if (timeOfDay >= 18 && timeOfDay <= 21) { // Dinner rush
      dynamicMultiplier *= 1.20 // 20% premium
    }

    // Day-based pricing
    if (dayOfWeek === 5 || dayOfWeek === 6) { // Friday/Saturday
      dynamicMultiplier *= 1.10 // 10% weekend premium
    }
    ...
    if (currentDemandLevel > 80) {
      ...
      dynamicMultiplier *= 0.90 // 10% low demand discount
    }
    ...
    return {
      ...
      pricing_breakdown: {
        ...
        dynamic_adjustments: {
          time_multiplier: timeOfDay >= 11 && timeOfDay <= 14 ? 1.15 : timeOfDay >= 18 && timeOfDay <= 21 ? 1.20 : 1.0,
          day_multiplier: (dayOfWeek === 5 || dayOfWeek === 6) ? 1.10 : 1.0,
          demand_multiplier: currentDemandLevel > 80 ? 1.25 : currentDemandLevel < 20 ? 0.90 : 1.0,
      ...
      profitability_analysis: {
        ...
        gross_margin_percent: ((dynamicPrice - standardResult.pricing_breakdown.cost_basis.total_cost) / dynamicPrice) * 100
      }
    }
  ...
  try {
    ...
    const { data: calculationRecord } = await supabase
      ...
      .insert([{
        ...
        ai_insights: {
          pricing_optimization_suggestions: generatePricingOptimizationSuggestions(calculationResult),
          market_positioning: analyzePricePositioning(calculationResult),
          profitability_alerts: generateProfitabilityAlerts(calculationResult.profitability_analysis)
        },
        metadata: {
    ...
    const response: PricingCalculationResponse = {
      ...
      dag_execution_metadata: {
        ...
        dag_path: ['cost_calculation', 'markup_application', 'market_adjustment', 'customer_adjustment'],
...
export async function GET() {
  ...
  return NextResponse.json({
    ...
    hera_advantages: {
      ...
      'unified_cost_and_pricing': 'Single system for cost and price vs separate SAP CO and SD modules',
      'dynamic_pricing_engine': 'Built-in time/demand-based pricing vs complex SAP Variant Configuration',
