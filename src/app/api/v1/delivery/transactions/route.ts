...
// GET /delivery/transactions - Fetch all delivery items
export async function GET(request: NextRequest) {
  try {
    const supabaseAdmin = getSupabaseAdmin()
    const { searchParams } = new URL(request.url)
    const organizationId = searchParams.get('organization_id')
    const entityType = searchParams.get('entity_type') || 'delivery_item'
    ...
    if (entitiesError) {
      console.error('Error fetching delivery entities:', entitiesError)
      return NextResponse.json(
        { success: false, message: 'Failed to fetch delivery entities' },
        { status: 500 }
      )
    }
...
// POST /delivery/transactions - Create delivery item
  ...
  try {
    ...
    const { data: entity, error: entityError } = await supabaseAdmin
      ...
      .insert({
        ...
        entity_type,
        entity_name,
        entity_code: entity_code || `DEL-${Date.now().toString().slice(-6)}`,
        status: 'active',
    ...
    if (entityError) {
      console.error('Error creating delivery entity:', entityError)
      return NextResponse.json(
        { success: false, message: 'Failed to create delivery entity', error: entityError },
        { status: 500 }
      )
    }
