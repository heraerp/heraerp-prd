    ...
    const application: GrantApplicationDetail = {
      ...
      applicant: {
        ...
        name:
          ...
          updatedEntity?.metadata?.applicant_name ||
          `${updatedEntity?.metadata?.applicant_type} - ${updatedEntity?.metadata?.applicant_id}`
      },
      round: {
        id: roundRel?.core_entities?.id || updatedEntity?.metadata?.round_id || '',
        round_code:
          roundRel?.core_entities?.entity_code ||
          updatedEntity?.metadata?.round_code ||
          `ROUND-${updatedEntity?.metadata?.round_id}`
      },
      program: {
