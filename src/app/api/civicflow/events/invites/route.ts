  ...
  try {
    ...
    const transformedInvites: EventInvite[] = await Promise.all(
      (invites || []).map(async (invite) => {
        ...
        const subjectRel = relationships.find((r: any) => r.relationship_type === 'invite_to_subject');
        ...
        const inviteSubjectType = getFieldValue('subject_type') as 'constituent' | 'organization' || 'constituent';
