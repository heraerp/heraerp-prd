...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const stats: EventStats = {
      ...
      registration_rate: totalInvites > 0 ? (registeredCount / totalInvites) * 100 : 0,
      attendance_rate: registeredCount > 0 ? (attendedCount / registeredCount) * 100 : 0,
      no_show_rate: registeredCount > 0 ? (noShowCount / registeredCount) * 100 : 0,
    ...
    if (event) {
      const capacityField = event.core_dynamic_data?.find((d: any) => d.field_name === 'capacity')
