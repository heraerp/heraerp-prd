...
export async function GET(request: NextRequest) {
  try {
    ...
    (events || []).forEach(event => {
      ...
      const endField = event.core_dynamic_data?.find(
        (d: any) => d.field_name === 'end_datetime'
      );
      
      const startDate = startField?.field_value_text 
        ? new Date(startField.field_value_text)
        : new Date(event.created_at);
      const endDate = endField?.field_value_text
        ? new Date(endField.field_value_text)
        : startDate;
    ...
    const trendingRegistrationRate = totalInvitations > 0
    ...
    return NextResponse.json(
      { error: 'Failed to fetch event KPIs' },
      { status: 500 }
    );
  }
}