...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data: constituents, count, error } = await query
    ...
    const transformedConstituents = (constituents || []).map(constituent => {
      const dynamicData = constituent.core_dynamic_data || []
      ...
      return {
        id: constituent.id,
        entity_name: constituent.entity_name,
        email,
        tags
      }
    })
