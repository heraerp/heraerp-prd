...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!emailId || !task) {
      return NextResponse.json(
        { error: 'Email ID and task are required' },
        { status: 400 }
      );
    }
...
async function performReplyGeneration(emailContent: any) {
  ...
  if (!isInbound) {
    return {
      tone: 'professional',
      content: 'This appears to be an outbound email. Reply generation works best with inbound messages.',
      confidence: 0.2,
...
Best regards`;
  ...
  return {
    ...
    confidence: 0.75,
    reasoning: 'Generated professional acknowledgment reply based on email content and standard business practices'
  };
}
...
function extractDates(text: string): string[] {
  const dateRegex = /\b\d{1,2}\/\d{1,2}\/\d{4}\b|\b\d{1,2}-\d{1,2}-\d{4}\b|\b[A-Za-z]+ \d{1,2}, \d{4}\b/g;
...
function extractAmounts(text: string): string[] {
  const amountRegex = /\$\d+(?:,\d{3})*(?:\.\d{2})?|\b\d+(?:,\d{3})*(?:\.\d{2})?\s*(?:dollars?|USD|€|euros?)\b/g;
