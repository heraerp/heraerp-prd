...
export async function GET(req: NextRequest) {
  try {
    ...
    const { data: emailEntities, error: queryError } = await query.range(offset, offset + limit - 1)
    ...
    const emails =
      emailEntities
        ?.map(entity => {
          ...
          if (
            search &&
            ![subject, bodyText, from].some(field =>
    ...
    if (!is_draft && !isDemoMode(organizationId)) {
      try {
        const sendResponse = await fetch(
          `${process.env.NEXTAUTH_URL || 'http://localhost:3000'}/api/integrations/resend/multitenant`,
          {
            method: 'POST',
            headers: {
