  ...
  try {
    ...
    if (!sourceSchema || !targetSchema) {
      return NextResponse.json(
        { error: 'Source and target schemas required' },
        { status: 400 }
      )
    }
    ...
    const stats = {
      ...
      unmappedSourceFields: sourceSchema.fields.filter(
        (sf: SchemaField) => !fieldMappings.some(m => m.source_field === sf.name)
      ).map((f: SchemaField) => f.name),
      unmappedTargetFields: targetSchema.fields.filter(
        (tf: SchemaField) => !fieldMappings.some(m => m.target_field === tf.name)
      ).map((f: SchemaField) => f.name),
      mappingConfidence: (fieldMappings.length / Math.min(
        sourceSchema.fields.length,
    ...
    return NextResponse.json(
      { error: 'Failed to auto-generate mappings' },
      { status: 500 }
    )
  }
}