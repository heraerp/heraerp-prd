  ...
  try {
    ...
    if (params.branch_id) {
      ...
      const { data: relationships } = await supabase
        ...
        .or(
          `from_entity_id.eq.${params.branch_id},to_entity_id.eq.${params.branch_id}`
        )
      ...
      if (allServices && allServices.length > 0) {
        ...
        if (categoryData) {
          filteredServiceIds = categoryData
            .filter((d: any) => d.field_name === 'service.category' && d.field_value === params.category)
    ...
    if (rpcError || !rpcData) {
      ...
      const { data: dynamicDataRows, error: dynamicError } = await supabase
        ...
        .in('field_name', ['service.base_price', 'service.duration_min', 'service.category', 'service.tax', 'service.commission'])
    ...
    const items = services.map(service => {
      ...
      return {
        ...
        duration_minutes: attributes['service.duration_min'] ? Number(attributes['service.duration_min']) : null,
