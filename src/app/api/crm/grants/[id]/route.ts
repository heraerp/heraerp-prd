...
export async function GET(
  ...
  try {
    ...
    const dynamicFields = dynamicData?.reduce((acc, field) => {
      acc[field.field_name] = field.field_value_text || field.field_value_number || field.field_value_boolean;
      return acc;
    }, {} as any) || {};

    // Get tags from dynamic data
    const tags = dynamicData?.filter(d => d.field_name === 'tag').map(d => d.field_value_text).filter(Boolean) || [];
    ...
    const application: GrantApplicationDetail = {
      ...
      applicant: {
        ...
        name: applicantRel?.core_entities?.entity_name || entity.metadata?.applicant_name || `${entity.metadata?.applicant_type} - ${entity.metadata?.applicant_id}`,
      },
      round: {
        id: roundRel?.core_entities?.id || entity.metadata?.round_id || '',
        round_code: roundRel?.core_entities?.entity_code || entity.metadata?.round_code || `ROUND-${entity.metadata?.round_id}`,
      },
      program: {
