...
export async function GET(request: NextRequest) {
  try {
    ...
    const filters: GrantFilters = {
      ...
      tags: searchParams.get('tags')?.split(',').filter(t => t.trim()) || undefined,
...
async function getGrantKpis(orgId: string): Promise<NextResponse> {
  try {
    ...
    const inReview = statusCounts?.filter(app => 
      ['submitted', 'in_review'].includes(app.status || '')
    ).length || 0;
    
    const approvedCount = statusCounts?.filter(app => 
      ['approved', 'awarded'].includes(app.status || '')
    ).length || 0;
    const approvalRate = totalApplications > 0 ? approvedCount / totalApplications : 0;

    // Calculate average award from metadata
    const awardedApplications = statusCounts?.filter(app => 
      app.status === 'awarded' && app.metadata?.amount_awarded
    ) || [];
    const totalAwarded = awardedApplications.reduce((sum, app) => 
      sum + (app.metadata?.amount_awarded || 0), 0
    );
    ...
    return NextResponse.json(
      { error: 'Failed to fetch grant KPIs' },
      { status: 500 }
    );
  }
}
...
async function getGrantApplications(orgId: string, filters: GrantFilters): Promise<NextResponse> {
  try {
    ...
    const applications: GrantApplicationListItem[] = entities?.map(entity => ({
      id: entity.id,
      applicant: {
        id: entity.metadata?.applicant_id || '',
        type: entity.metadata?.applicant_type || 'constituent',
        name: entity.metadata?.applicant_name || `${entity.metadata?.applicant_type} - ${entity.metadata?.applicant_id}`,
      },
      round: {
    ...
    if (filters.round_id) {
      filteredApplications = filteredApplications.filter(app => 
        app.round.id === filters.round_id
      );
    }

    if (filters.program_id) {
      filteredApplications = filteredApplications.filter(app => 
        app.program.id === filters.program_id
      );
    }

    if (filters.amount_min !== undefined) {
      filteredApplications = filteredApplications.filter(app => 
        app.amount_requested && app.amount_requested >= filters.amount_min!
      );
    }

    if (filters.amount_max !== undefined) {
      filteredApplications = filteredApplications.filter(app => 
        app.amount_requested && app.amount_requested <= filters.amount_max!
      );
    }
    ...
    if (filters.tags && filters.tags.length > 0) {
      filteredApplications = filteredApplications.filter(app => 
        filters.tags!.some(tag => 
          JSON.stringify(app).toLowerCase().includes(tag.toLowerCase())
        )
      );
    }
