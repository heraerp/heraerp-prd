...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data: activeRounds } = await supabase
      ...
      .or(`core_dynamic_data.field_code.eq.HERA.PUBLICSECTOR.CRM.GRANT_ROUND.WINDOW_OPEN.v1,core_dynamic_data.field_code.eq.HERA.PUBLICSECTOR.CRM.GRANT_ROUND.WINDOW_CLOSE.v1`);
    ...
    return NextResponse.json(
      { error: 'Failed to fetch program KPIs' },
      { status: 500 }
    );
  }
}