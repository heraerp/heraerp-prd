...
export default function SalesInvoicesPage() {
  ...
  async function refresh() {
    const res = await client.txnQuery({ organization_id: auth.organization_id, transaction_type: 'INVOICE', include_lines: false })
  ...
  async function createInvoice() {
    await emit.mutateAsync({
      ...
      smart_code: 'HERA.ITD.SALES.INVOICE.POST.V1',
  ...
  return (
    <div className="space-y-4">
      ...
      <button className="px-3 py-2 rounded bg-primary text-primary-foreground" onClick={createInvoice}>Create Invoice</button>
