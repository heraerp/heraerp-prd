...
export default function SalesOrdersPage() {
  ...
  async function refresh() {
    const res = await client.txnQuery({ organization_id: auth.organization_id, transaction_type: 'SO', include_lines: false })
    setList(res?.transactions || [])
  }

  React.useEffect(() => {
    if (auth.organization_id) refresh()
  }, [auth.organization_id])

  async function createSO() {
    await emit.mutateAsync({
      organization_id: auth.organization_id,
      smart_code: 'HERA.ITD.SALES.SO.POST.V1',
      transaction_type: 'SO',
      header: { date: new Date().toISOString(), branch_id: branchId, customer_id: customerId, vendor_id: vendorId },
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-2">
        ...
        <input className="border rounded px-2 py-2" placeholder="Vendor (optional)" value={vendorId} onChange={e => setVendorId(e.target.value)} />
      </div>
      ...
      <button className="px-3 py-2 rounded bg-primary text-primary-foreground" onClick={createSO}>Create SO</button>
