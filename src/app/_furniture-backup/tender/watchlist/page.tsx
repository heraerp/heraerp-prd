...
const mockWatchlist: WatchlistItem[] = [
  {
    ...
    tender_name: 'Teak Wood Supply - Nilambur Range',
    ...
    notes: 'Strong competition expected. Prepare aggressive bid.'
  },
  {
    tender_id: '2',
    tender_code: 'KFD/2025/ROSE/002',
    tender_name: 'Rosewood Logs - Wayanad Division',
    department: 'Kerala Forest Department',
    ...
    notes: 'Premium quality rosewood. High margin opportunity.'
  },
  {
    tender_id: '3',
    tender_code: 'KSBC/2025/BAMBOO/001',
    tender_name: 'Bamboo Supply Contract - Annual',
...
export default function WatchlistPage() {
  ...
  const handleRemoveFromWatchlist = (tenderId: string) => {
  ...
  const upcomingDeadlines = watchlist
    .filter(item => getDaysRemaining(item.closing_date) <= 7)
    .sort((a, b) => getDaysRemaining(a.closing_date) - getDaysRemaining(b.closing_date))
  ...
  return (
    <div>
      <FurniturePageHeader
        title="Tender Watchlist"
        subtitle="Monitor important tenders and get timely alerts"
      />
      ...
      {upcomingDeadlines.length > 0 && (
        <Alert className="mb-6">
          ...
          <AlertDescription>
            <strong>{upcomingDeadlines.length} tender(s) closing soon:</strong>
            <ul className="bg-[var(--color-body)] mt-2 space-y-1">
              {upcomingDeadlines.map(item => (
                <li key={item.tender_id}>
                  {item.tender_code} - Closing in {getDaysRemaining(item.closing_date)} days
                </li>
              ))}
            </ul>
          </AlertDescription>
      ...
      <div className="bg-[var(--color-body)] grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <Card className="bg-[var(--color-surface-raised)] border-[var(--color-border)] p-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-[var(--color-text-secondary)]">Watched Tenders</p>
              <p className="text-2xl font-bold">{watchlist.length}</p>
            </div>
            <Eye className="h-8 w-8 text-[var(--color-icon-secondary)]" />
          </div>
        </Card>
        ...
        <Card className="bg-[var(--color-surface-raised)] border-[var(--color-border)] p-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-[var(--color-text-secondary)]">Alerts Enabled</p>
              <p className="text-2xl font-bold">
                {watchlist.filter(item => item.alerts_enabled).length}
              </p>
            </div>
            <Bell className="h-8 w-8 text-[var(--color-icon-secondary)]" />
          </div>
        </Card>
        ...
        <Card className="bg-[var(--color-surface-raised)] border-[var(--color-border)] p-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-[var(--color-text-secondary)]">High Priority</p>
              <p className="text-2xl font-bold">
                  ...
                  <TableRow key={item.tender_id}>
                    <TableCell>
                      <div>
                        <Link
                          ...
                          className="bg-[var(--color-body)] font-medium text-primary hover:underline"
                        >
                          {item.tender_code}
                        </Link>
                    ...
                    <TableCell>
                      <div className={isUrgent ? 'text-red-600 font-medium' : ''}>
                        {format(new Date(item.closing_date), 'dd MMM yyyy')}
                        <p className="text-sm text-[var(--color-text-secondary)]">
                          {isUrgent && <AlertCircle className="inline h-3 w-3 mr-1" />}
                    ...
                    <TableCell>
                      <Switch
                        ...
                        onCheckedChange={() => handleToggleAlerts(item.tender_id)}
                      />
                    </TableCell>
                    <TableCell>
                      <div className="flex items-center gap-2">
                        <Link href={`/furniture/tender/${item.tender_code}`}>
                        ...
                        <Button
                          variant="ghost"
                          size="sm"
                          onClick={() => handleRemoveFromWatchlist(item.tender_id)}
                        >
      ...
      {/* Alert Settings */}
      <Card className="bg-[var(--color-surface-raised)] border-[var(--color-border)] p-6 mt-6">
        <h3 className="bg-[var(--color-body)] text-lg font-semibold mb-4">Alert Preferences</h3>
        <div className="space-y-4">
          <div className="bg-[var(--color-body)] flex items-center justify-between">
            <div>
              <p className="font-medium">Email Notifications</p>
              <p className="text-sm text-[var(--color-text-secondary)]">
                Receive email alerts for watched tenders
              </p>
            </div>
          ...
          <div className="bg-[var(--color-body)] flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-[var(--color-text-secondary)]">
                Get SMS for urgent deadlines (3 days or less)
              </p>
            </div>
            <Switch />
          </div>
          <div className="bg-[var(--color-body)] flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-[var(--color-text-secondary)]">
                Summary of all watched tenders at 9 AM daily
              </p>
            </div>
            <Switch defaultChecked />
          </div>
