...
export default function ReadinessDashboardPage() {
  ...
  const fetchSessions = async () => {
    ...
    setLoading(true)
    try {
      const response = await fetch(`/api/v1/readiness/sessions?organization_id=${orgId}`, {
        headers: {
      ...
      console.log(
        `📊 Sessions with answers: ${sessionsWithAnswers.length} out of ${data.data?.length || 0} total sessions`
      )
    } catch (error) {
  ...
  const filteredSessions = sessions.filter(session => {
    ...
    if (filter.industry !== 'all' && session.metadata.industry_type !== filter.industry)
      return false
    if (
      filter.search &&
      !session.metadata.user_email?.toLowerCase().includes(filter.search.toLowerCase())
    )
      return false
    return true
  })
  ...
  const stats = {
    ...
    avgScore:
      sessions
        .filter(s => s.transaction_status === 'completed')
            ...
            <div className="px-4 sm:px-6 lg:px-8">
              <div className="flex items-center justify-between h-16">
                ...
                <div className="flex items-center gap-2">
                  ...
                  >
                    <RefreshCw
                      className={cn('h-4 w-4 inline mr-2', refreshing && 'animate-spin')}
                    />
                    Refresh
                  </motion.button>
                  <motion.button
                  ...
                  <div className="flex flex-wrap gap-4">
                    ...
                    >
                      ...
                      <SelectContent className="hera-select-content">
                        ...
                        <SelectItem value="in_progress" className="hera-select-item">
                          In Progress
                        </SelectItem>
                      </SelectContent>
                    </Select>
                    ...
                    >
                      ...
                      <SelectContent className="hera-select-content">
                        ...
                        <SelectItem value="general" className="hera-select-item">
                          General
                        </SelectItem>
                        <SelectItem value="manufacturing" className="hera-select-item">
                          Manufacturing
                        </SelectItem>
                        <SelectItem value="retail" className="hera-select-item">
                          Retail
                        </SelectItem>
                        <SelectItem value="healthcare" className="hera-select-item">
                          Healthcare
                        </SelectItem>
                        <SelectItem value="services" className="hera-select-item">
                          Services
                        </SelectItem>
                      </SelectContent>
                    </Select>
                        ...
                        <Tabs defaultValue="overview">
                              ...
                              <>
                                ...
                                >
                                  ...
                                  <Badge
                                    ...
                                    style={{
                                      background:
                                        selectedSession.insights?.readinessLevel === 'High'
                                          ? 'rgba(16, 185, 129, 0.2)'
                                  ...
                                  >
                                    {selectedSession.insights?.readinessLevel || 'Not Analyzed'}{' '}
                                    Readiness
                                  </Badge>
                                </motion.div>
                              </>
                                ...
                                <div className="max-w-xs mx-auto mb-6">
                                  <div className="relative h-2 bg-muted rounded-full overflow-hidden">
                                    <motion.div
                                      initial={{ width: 0 }}
                                      animate={{
                                        width: `${(selectedSession.metadata as any)?.completionRate || 0}%`
                                      }}
                          ...
                          <TabsContent value="categories">
                            <div className="space-y-4">
                              {(selectedSession.metadata as any)?.categoryScores &&
                              Object.keys(selectedSession.metadata.categoryScores).length > 0 ? (
                                Object.entries(selectedSession.metadata.categoryScores).map(
                                  ([category, data], index) => (
                                    <motion.div
                                      key={category}
                                      initial={{ opacity: 0, x: -20 }}
                              ...
                              {selectedSession.answers && selectedSession.answers.length > 0 && (
                                <div className="flex items-center justify-between text-sm text-muted-foreground dark:text-muted-foreground">
                                  <span>
                                    ...
                                    {
                                      selectedSession.answers.filter(
                                        a => a.line_data?.answer_value === 'yes'
                                      ).length
                                    }{' '}
                                    Yes,{' '}
                                    {
                                      selectedSession.answers.filter(
                                        a => a.line_data?.answer_value === 'partial'
                                      ).length
                                    }{' '}
                                    Partial,{' '}
                                    {
                                      selectedSession.answers.filter(
                                        a => a.line_data?.answer_value === 'no'
                                      ).length
                                    }{' '}
                                    No)
                                  </span>
                                ...
                                (() => {
                                  ...
                                  const filteredAnswers = selectedSession.answers.filter(answer => {
                                    ...
                                    return (
                                      ...
                                      answer.line_data?.category
                                        ?.toLowerCase()
                                        .includes(searchLower) ||
                                      answer.line_data?.answer_value
                                        ?.toLowerCase()
                                        .includes(searchLower)
                                    )
                                  })
                                        ...
                                        >
                                          ...
                                          <div className="flex items-center justify-between">
                                            <Badge
                                              ...
                                              style={{
                                                background:
                                                  answer.line_data?.answer_value === 'yes'
                                                    ? 'rgba(16, 185, 129, 0.2)'
