-- =====================================================
-- HERA ENTITIES CRUD V2 - ENTERPRISE RELATIONSHIP EDITION (FIXED)
-- =====================================================
-- 
-- Fixed version that handles existing function conflicts
-- Drops all versions and creates clean enterprise version
-- 
...
DECLARE
    func_record RECORD;
BEGIN
    -- Get all versions of hera_entities_crud_v2 function
    FOR func_record IN 
    ...
    LOOP
        -- Drop each version with its specific signature
    ...
    -- Also try common parameter combinations that might exist
    BEGIN
    ...
    BEGIN
        DROP FUNCTION IF EXISTS public.hera_entities_crud_v2(text, uuid, uuid, jsonb, jsonb, jsonb) CASCADE;
    EXCEPTION WHEN OTHERS THEN
        NULL; -- Ignore if doesn't exist
    END;
    
    BEGIN
        DROP FUNCTION IF EXISTS public.hera_entities_crud_v2(text, uuid, uuid, jsonb, jsonb) CASCADE;
    EXCEPTION WHEN OTHERS THEN
        NULL; -- Ignore if doesn't exist
    END;
    
    BEGIN
        DROP FUNCTION IF EXISTS public.hera_entities_crud_v2(text, uuid, uuid, jsonb) CASCADE;
    EXCEPTION WHEN OTHERS THEN
        NULL; -- Ignore if doesn't exist
    END;
    
    RAISE NOTICE '✅ All existing hera_entities_crud_v2 function versions dropped successfully';
END $$;

-- =====================================================
-- CREATE ENTERPRISE FUNCTION WITH RELATIONSHIP SUPPORT
-- =====================================================
...
BEGIN
    ...
    CASE upper(p_action)
        ...
        WHEN 'CREATE' THEN
            ...
            -- =====================================================
            -- RELATIONSHIP CREATION (FIXED SCHEMA FIELDS)
            -- =====================================================
            ...
            v_result := jsonb_build_object(
                ...
                'metadata', jsonb_build_object(
                    ...
                    'dynamic_fields_created', CASE WHEN v_dynamic_data = '{}'::jsonb THEN '[]'::jsonb ELSE to_jsonb(array(SELECT jsonb_object_keys(v_dynamic_data))) END,
...
Fixed version that properly handles existing function conflicts.

FEATURES:
✅ Full relationship CRUD operations
✅ Bidirectional relationship support using correct schema fields (from_entity_id/to_entity_id)
...
✅ Uses correct core_relationships fields: from_entity_id, to_entity_id
✅ Handles both source_entity_id/target_entity_id and from_entity_id/to_entity_id input formats
✅ Maintains backward compatibility with existing parameter structure
...
BEGIN
    ...
    IF NOT EXISTS (
        ...
        WHERE n.nspname = 'public' 
        AND p.proname = 'hera_entities_crud_v2'
        AND pg_get_function_arguments(p.oid) = 'p_action text, p_actor_user_id uuid, p_organization_id uuid, p_entity jsonb DEFAULT ''{}''::jsonb, p_dynamic jsonb DEFAULT ''{}''::jsonb, p_relationships jsonb DEFAULT ''[]''::jsonb, p_options jsonb DEFAULT ''{}''::jsonb'
    ) THEN
        RAISE EXCEPTION 'DEPLOYMENT_FAILED: Function hera_entities_crud_v2 was not created with correct signature';
    END IF;
    
    RAISE NOTICE '✅ DEPLOYMENT SUCCESS: hera_entities_crud_v2 Enterprise Relationship Edition (Fixed) deployed successfully';
    RAISE NOTICE '🔧 FIXED: Function conflicts resolved, all previous versions dropped';
    RAISE NOTICE '🔗 FEATURES: Full relationship CRUD with correct schema field names';
    ...
    RAISE NOTICE '📋 SCHEMA: Uses correct from_entity_id/to_entity_id fields from core_relationships';
END $$;