...
export function getPostsByCity(city: string): Post[] {
  return getAllPosts().filter(post => 
    post.meta.city.toLowerCase() === city.toLowerCase()
  );
}
...
export function getPostsByPillar(pillar: string): Post[] {
  return getAllPosts().filter(post => 
    post.meta.pillar.toLowerCase() === pillar.toLowerCase()
  );
}
...
// Get unique cities
export function getAllCities(): Array<{ city: string; region: string; country: string; count: number }> {
