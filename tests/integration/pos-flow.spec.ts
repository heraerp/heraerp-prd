...
describe('POS integration hardening', () => {
      ...
      items: [
        {
          ...
          stylist_entity_id: 'test-stylist-main',
          stylist_name: 'Main Stylist'
        }
      ],
      payments: [
        { method: 'Cash', amount: 100 },
        { method: 'Card', amount: 57.5 }
      ],
      ...
      items: [
        {
          ...
          price: 150,
          type: 'SERVICE' as const,
          stylist_entity_id: 'test-stylist-idem',
          stylist_name: 'Idem Stylist'
        }
      ],
      ...
      items: [
        {
          ...
          price: 100,
          type: 'SERVICE' as const,
          stylist_entity_id: 'test-stylist-123',
          stylist_name: 'Alex'
        }
      ],
      payments: [{ method: 'Cash', amount: 100 }]
  ...
  it('does not require a stylist when commissions are disabled (simple POS)', async () => {
    // Note: This test assumes commissions are disabled in organization settings
    // In practice, you would set: { salon: { commissions: { enabled: false } } }
    const ticket = {
      id: 'T-SVC-NO-STYLIST-SIMPLE-' + Date.now(),
      total: 100,
      taxTotal: 0,
      items: [{ name: 'Service A', qty: 1, price: 100, type: 'SERVICE' as const }],
      payments: [{ method: 'Cash', amount: 100 }]
    ...
    // This would fail if commissions are enabled, but should pass if disabled
    try {
      const tx = await postEventWithBranch(ORG, BRANCH, ticket)
      expect(tx.transaction_type).toBe('POS_SALE')
      console.log('✅ Commissions appear to be disabled - service without stylist accepted')
    } catch (error) {
      if (error.message.includes('must have at least one service line with assigned stylist')) {
        console.log('ℹ️ Commissions are enabled - service requires stylist (expected)')
        // Skip this test since commissions are enabled
        return
      }
      throw error
    }
      ...
      items: [
        {
          ...
          entity_id: 'srv-001',
          type: 'SERVICE' as const,
          stylist_entity_id: 'test-stylist-1',
          stylist_name: 'Sarah'
        },
        {
          name: 'Hair Color',
          ...
          entity_id: 'srv-002',
          type: 'SERVICE' as const,
          stylist_entity_id: 'test-stylist-2',
          stylist_name: 'Mike'
        },
        {
