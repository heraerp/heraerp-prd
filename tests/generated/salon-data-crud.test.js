...
describe('Salon - Universal CRUD Operations', () => {
  ...
  describe('CREATE Operations', () => {
    test('should create a new salon-data', async () => {
      const createData = {
      "entity_type": "salon",
      "entity_name": "Test Salon",
      "entity_code": "SALON-001",
      "smart_code": "HERA.SALON.v1"
};
      ...
      expect(result.data.entity_type).toBe('salon');
      expect(result.data.smart_code).toContain('HERA.SALON');
  ...
  describe('READ Operations', () => {
    test('should fetch all salon-data entities', async () => {
      const mockEntities = [
        {
          id: 'salon-data-1',
          entity_type: 'salon',
      ...
      const result = await universalApi.getEntities('salon', TEST_ORG_ID);

      expect(result).toHaveProperty('data');
      expect(result.data).toHaveLength(2);
      expect(result.data[0].entity_type).toBe('salon');
    });

    test('should search salon-data by name', async () => {
      const searchTerm = 'Salon';
      
      // Universal API filters on client side after fetching
      const allEntities = [
        { entity_name: 'Salon Match', entity_type: 'salon' },
        { entity_name: 'Other Entity', entity_type: 'salon' }
      ];
  ...
  describe('UPDATE Operations', () => {
    test('should update salon-data details', async () => {
      const updateData = {
      "entity_name": "Updated Salon",
      "metadata": {
            "updated": true
      }
};
      ...
      const result = await universalApi.updateEntity('salon-data-123', updateData);
  ...
  describe('DELETE Operations', () => {
    test('should delete salon-data', async () => {
      global.fetch.mockResolvedValueOnce({
        ok: true,
        json: async () => ({ success: true, message: 'Entity deleted' })
      });

      await universalApi.deleteEntity('salon-data-123');
    ...
    test('should handle cascade deletion of dynamic fields', async () => {
      ...
      const result = await universalApi.deleteEntity('salon-data-123');
  ...
  describe('Error Handling', () => {
    test('should handle validation errors', async () => {
      ...
      const result = await universalApi.createEntity({ entity_type: 'salon' });
    ...
    test('should handle network errors gracefully', async () => {
      ...
      try {
        await universalApi.getEntities('salon');
  ...
  describe('Multi-tenant Security', () => {
    ...
    test('should enforce organization isolation', async () => {
      await universalApi.getEntities('salon', TEST_ORG_ID);
    ...
    test('should prevent cross-organization access', async () => {
      ...
      const result = await universalApi.getEntities('salon', otherOrgId);
  ...
  describe('HERA Architecture Compliance', () => {
    test('should use universal 6-table structure', () => {
      ...
      expect(tables).toContain('core_entities'); // For salon-data
    ...
    test('should include smart codes in all operations', () => {
      const smartCodePattern = /^HERA\.SALON\..*\.v\d+$/;
      expect('HERA.SALON.CREATE.v1').toMatch(smartCodePattern);
    });
  });
});
