import { Playbook, validate, enforce, branch, post, tx, audit } from '../core/dsl';

export const GRADING_JOB_PLAYBOOK_V1: Playbook = {
  ...
  steps: [
    ...
    branch(
      'when:graded_issue_certificate',
      (ctx) => ctx.util.getDynamic('status') === 'graded' && !!ctx.entity.payload?.metadata?.issue_certificate,
      [
        tx('tx:issue-cert', [
