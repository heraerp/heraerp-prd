...
import type { PlaybookContext } from '../core/dsl';
import { apiV2 } from '@/lib/client/fetchV2';

export const universalApiAdapters: ExecuteOptions['adapters'] = {
  ...
  link(ctx, rel) {
    ...
    const existing = ctx.out.relationships.findIndex(
  ...
  async persist(ctx: PlaybookContext) {
    const results: any = {};
  ...
  async fetchEntityById(id, opts = {}) {
    ...
    const params: any = { organization_id: opts.organizationId };
    if (opts.includeDynamic) params.include_dynamic = true;
    if (opts.includeRelationships) params.include_relationships = true;
