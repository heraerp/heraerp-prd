...
import { businessOracles } from '../oracles/business-oracles';
...
export class TestRunner {
   ...
   */
  async runBusinessProcess(test: BusinessProcessTest): Promise<TestResult> {
   ...
   */
  private async executeAction(action: StepAction): Promise<ActionResult> {
    const startTime = Date.now();
    ...
    try {
      ...
      switch (action.action_type) {
        ...
        case 'set_dynamic_field':
          result = await this.setDynamicField(
            action.entity_id,
            action.field_name,
            action.field_value,
            action.smart_code
   ...
   */
  private async createEntity(data: any): Promise<any> {
    if (!this.supabase) {
  ...
  private async createTransaction(data: any): Promise<any> {
    ...
    if (data.line_items && data.line_items.length > 0) {
      const lineItems = data.line_items.map((line: any) => ({
        ...line,
   ...
   * Create relationship
   */
  private async createRelationship(data: any): Promise<any> {
    if (!this.supabase) {
      return {
   ...
   */
  private async performUIInteraction(action: any): Promise<any> {
   ...
   */
  private async wait(duration: number, condition?: string): Promise<any> {
   ...
   */
  private async executeActions(phase: string, actions: StepAction[]): Promise<void> {
   ...
   */
  private async executeAssertions(assertions: any[]): Promise<AssertionResult[]> {
   ...
   */
  private async executeDatabaseAssertions(assertions: any[]): Promise<{ success: boolean; details: any }> {
    if (!this.supabase) {
   ...
   */
  private async executeBusinessAssertions(assertions: any[]): Promise<{ success: boolean; details: any }> {
    const results = [];
   ...
   */
  private async executeUIAssertions(assertions: any[]): Promise<{ success: boolean; details: any }> {
