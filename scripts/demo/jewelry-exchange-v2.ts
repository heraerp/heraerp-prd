...
async function main(){
  ...
  const item = (list as any)?.data?.[0] || (list as any)?.[0]; const netW = item.dynamic_data?.net_weight ?? item.dynamic_fields?.net_weight?.value; const purity = item.dynamic_data?.purity_karat ?? item.dynamic_fields?.purity_karat?.value
  ...
  const payload = {
    ...
    lines: [
      { line_number:1, line_type:'ITEM', smart_code:'HERA.JEWELRY.LINE.ITEM.RETAIL.V1', entity_id:item.id || item.entity_id, quantity:1,
        line_data:{ net_weight:netW, purity_karat:purity, gold_rate_per_gram:rate, making_charge_type:'per_gram', making_charge_rate:550 } },
      { line_number:2, line_type:'EXCHANGE', smart_code:'HERA.JEWELRY.LINE.EXCHANGE.OLDGOLD.V1', line_amount:-14500,
        line_data:{ assayed_weight:2.35, purity_karat:22 } },
      { line_number:3, line_type:'TAX', smart_code:'HERA.JEWELRY.LINE.TAX.GST.V1', line_data:{ gst_slab:3, mode:'CGST_SGST' } }
    ]
  }
