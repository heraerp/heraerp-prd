...
const PAGE_CONFIGS = {
  ...
  marketing: {
    entityType: 'campaign',
    entityPrefix: 'CAMP',
    dynamicFields: ['campaign_type', 'start_date', 'end_date', 'target_audience', 'budget', 'status'],
    relationships: [
  ...
  reports: {
    entityType: 'report',
    entityPrefix: 'RPT',
    dynamicFields: ['report_type', 'frequency', 'parameters', 'last_run', 'recipients'],
    relationships: []
  },
  settings: {
    entityType: 'setting',
    entityPrefix: 'SET',
    dynamicFields: ['setting_key', 'setting_value', 'setting_type', 'description'],
    relationships: []
  }
}

// Hook template
const HOOK_TEMPLATE = (entityName, entityType, fields) => `import { useState, useEffect } from 'react'
...
const TRANSFORMER_TEMPLATE = (entityName, fields) => `import { format } from 'date-fns'
...
const TEST_DATA_TEMPLATE = (entityType, entityName, fields) => `#!/usr/bin/env node
/**
