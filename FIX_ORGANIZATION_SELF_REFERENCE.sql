-- Fix organization creation with self-referencing organization_id
...
BEGIN
    ...
    -- 1. Create organization entity (with self-reference for organization_id)
    INSERT INTO core_entities (
        id,
        organization_id,  -- Self-reference for organization entities
        entity_type, 
        entity_name, 
        entity_code, 
        metadata, 
        ...
        v_org_id,
        v_org_id,  -- Organization entities reference themselves
        'organization',
        p_org_name,
    ...
    IF v_user_entity_id IS NULL THEN
        -- Create user entity with the organization_id
        INSERT INTO core_entities (
            id,
            organization_id,  -- User belongs to the created organization
            entity_type, 
            entity_name, 
            entity_code, 
            metadata, 
    ...
    ELSE
        -- If user exists, update their organization_id to the new org
...
-- Verify the fix
SELECT 'Organization function updated with self-reference pattern!' as status;