-- Fix hera_txn_query_v1 to include source_entity_id and target_entity_id filters
-- This was causing customer transaction history to show ALL transactions instead of filtering by customer
...
AS $$
DECLARE
  -- ✅ CRITICAL FIX: Add entity ID filters (were missing!)
...
BEGIN
  ...
  WITH base AS (
    ...
    WHERE t.organization_id = p_org_id
      ...
      -- ✅ CRITICAL FIX: Add entity ID filters
  ...
  ELSE
    -- Fetch transactions WITHOUT lines (faster)
    WITH page AS (
      SELECT
        t.id,
        t.organization_id,
...
COMMENT ON FUNCTION hera_txn_query_v1 IS 'HERA Transaction Query v1 - WITH source_entity_id and target_entity_id filtering (FIXED)';
