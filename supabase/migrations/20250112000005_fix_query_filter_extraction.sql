-- ========================================
-- CRITICAL FIX: hera_txn_crud_v1 QUERY Filter Extraction
-- ========================================
-- Issue: Client sends { filters: { source_entity_id: 'x' } }
--        Orchestrator passes entire p_payload to hera_txn_query_v1
--        Query function reads p_filters->>'source_entity_id' (expects flat structure)
--        Result: Filters are ignored, all transactions returned
--
-- Fix: Extract p_payload->'filters' before passing to hera_txn_query_v1
-- Date: 2025-01-12
-- ========================================
...
BEGIN
  ...
  ELSIF v_action = 'query' THEN
    -- ✅ CRITICAL FIX: Extract filters from p_payload.filters
    -- Client sends: { filters: { source_entity_id: 'x', date_from: '...', ... } }
    -- Query expects: { source_entity_id: 'x', date_from: '...', ... } (flat structure)
    v_resp := hera_txn_query_v1(
      p_organization_id,
      COALESCE(p_payload->'filters', p_payload)  -- Extract filters if nested, fallback to root
    );
    v_ok := true;
...
COMMENT ON FUNCTION public.hera_txn_crud_v1 IS 'HERA Transaction CRUD Orchestrator v1 - FIXED: Extracts p_payload.filters before delegating to hera_txn_query_v1';
...
SELECT hera_txn_crud_v1(
  ...
  'your-org-uuid'::uuid,
  '{"filters": {"source_entity_id": "customer-uuid", "limit": 5, "include_lines": true}}'::jsonb
);

-- Should return transactions filtered by source_entity_id
-- Check that data.data.items array contains only transactions for that customer
*/
